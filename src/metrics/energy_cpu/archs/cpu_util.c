...
state_t energy_cpu_util_read(ctx_t *c, ullong *values)
{
    ...
    float CPU_Power = ((float) (cpu_util_topo.tdp * socket_count) * ear_min(1.0, ratio) * (1.0 - IDLE_POWER_PERC)) +
                      (IDLE_POWER_PERC * (float) cpu_util_topo.tdp * (float) socket_count);
    ...
    debug("Usage %%User time %f page faults %ld BI %ld BO %ld Total IPC %ld",
          (float) (utime_usec) / (float) (stime_usec + utime_usec), my_usage.ru_minflt + my_usage.ru_majflt,
          my_usage.ru_inblock, my_usage.ru_oublock, my_usage.ru_msgsnd + my_usage.ru_msgrcv);
#endif
