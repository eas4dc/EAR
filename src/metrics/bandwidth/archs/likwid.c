...
BWIDTH_F_LOAD(likwid)
{
    ...
    if (state_fail(s = topology_select(tp, &tp_own, TPSelect.socket, TPGroup.merge, 0))) {
        return;
    }
    ...
    devs_count = tp->socket_count * evs_count;
    // Debugging
    debug("sockets   : %u", tp_own.cpu_count);
    debug("total cpus: %u", cpus_count);
    debug("counters  : %u", devs_count);
    //
    apis_put(ops->unload, bwidth_likwid_unload);
    apis_put(ops->get_info, bwidth_likwid_get_info);
    apis_put(ops->read, bwidth_likwid_read);
}

BWIDTH_F_UNLOAD(likwid)
{
    likwid_dispose();
}

BWIDTH_F_GET_INFO(likwid)
{
    info->api         = API_LIKWID;
...
BWIDTH_F_READ(likwid)
{
    ...
    for (i = j = 0; i < tp_own.cpu_count; ++i) {
        for (k = 0; k < evs_count; ++k, ++j) {
            ctrs_accum[j] += (ullong) ctrs[(cpus_count * k) + tp_own.cpus[i].id];
