...
#define CP_SET_CONFIG(o, e, p)                                                                                         \
    0x0D0, 0x02, (0x01 << 0x08),                                                                                       \
        ((o & BC(2) & BC(1)) | (p << 2) | (e << 1))         // CONFIG_CLOS,CLOS_PM_QOS_CONFIG,MBOX_CMD_WRITE_BIT
...
static state_t commands_mmsr_send_single(uint cpu, ullong msr, ullong data, int write, ullong *answer)
{
    ...
    if (ioctl(driver_fd, ISST_IF_MSR_CMD, &msr_cmds) == -1) {
        debug("ioctl failed for MSR command 0x%llX (%d, %s)", msr, errno, strerror(errno));
    } else {
        if (read) {
...
static state_t commands_mmio_send(cmds_mmio_t *cmds, uint *answers)
{
    ...
    if (ioctl(driver_fd, ISST_IF_MMIO_CMD, cmds) == -1) {
        debug("ioctl failed to send %d MMIO commands 0x%X: %d, %s", cmds->req_count, cmds->io_reg[0].reg, errno,
              strerror(errno));
    ...
    debug("Answer for the %d MMIO commands 0x%02X: 0x%02X", cmds->req_count, cmds->io_reg[0].reg,
          cmds->io_reg[0].value);
...
static state_t commands_mbox_send(cmds_mbox_t *cmds, uint *answers)
{
    ...
    do {
        if (ioctl(driver_fd, ISST_IF_MBOX_CMD, cmds) == -1) {
            debug("ioctl failed to send %d MBOX commands 0x%X_0x%X: %u retrys remaining (%d, %s)", cmds->cmd_count,
                  cmds->mbox_cmd[0].command, cmds->mbox_cmd[0].sub_command, retrys, errno, strerror(errno));
            --retrys;
        } else {
            break;
        }
    ...
    debug("Answer for MBOX command 0x%X_0x%X: 0x%X", cmds->mbox_cmd[0].command, cmds->mbox_cmd[0].sub_command,
          cmds->mbox_cmd[0].resp_data);
    return EAR_SUCCESS;
}

static int commands_mbox_fill(cmds_mbox_t *cmds, uint cpu, uint cmd1, uint cmd2, uint param, uint req_data,
                              uint *answers, int send)
{
...
__attribute__((unused)) static void clos_print(clos_t *clos, int fd)
{
    ...
    for (c = 0; c < CLOS_COUNT; ++c) {
        if (clos[c].max_khz == HHZ_TO_KHZ(255)) {
            dprintf(fd, "CLOS%d: MAX GHZ - %.1lf GHz (%s)\n", c, ((double) clos[c].min_khz) / 1000000.0,
                    strprio[clos[c].low]);
        } else {
            dprintf(fd, "CLOS%d: %.1lf GHz - %.1lf GHz (%s)\n", c, KHZ_TO_GHZ((double) clos[c].max_khz),
                    KHZ_TO_GHZ((double) clos[c].min_khz), strprio[clos[c].low]);
        }
    }
}
...
#else
        debug("PM register: 0x%X", en->pm_reg) debug("CP register: 0x%X", en->cp_reg)
            debug("TF register: 0x%X", en->tf_reg) debug("TR register: 0x%llX", en->tr_reg)
#endif
#endif
...
#if SHOW_DEBUGS
    debug("Keeper values:");
    debug("PM register: 0x%X", en->pm_reg) debug("CP register: 0x%X", en->cp_reg) debug("TF register: 0x%X", en->tf_reg)
