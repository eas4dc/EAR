...
static const char *likwid_names[] = {
    "get_cpuInfo",           "get_cpuTopology",      "topology_init",           "topology_finalize",
    "perfmon_init",          "perfmon_addEventSet",  "perfmon_setupCounters",   "perfmon_startCounters",
    "perfmon_getLastResult", "perfmon_readCounters", "perfmon_readCountersCpu", "perfmon_finalize",
...
static state_t static_load()
{
    ...
    if (load_test(ear_getenv("HACK_FILE_LIKWID")))
        return s;
    if (load_test(LIKWID_PATH "/lib64/liblikwid.so"))
        return s;
    if (load_test(LIKWID_PATH "/lib/liblikwid.so"))
        return s;
