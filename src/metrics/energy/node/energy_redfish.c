...
state_t energy_init(void **c)
{
    ...
    if (state_fail(redfish_read((char *) redfish_query, (void *) &redfish_pwr_out, NULL, NULL, REDTYPE_LLONG))) {
        pthread_mutex_unlock(&ompi_lock);
        return EAR_ERROR;
    }
...
static state_t compute_energy(ulong *energy, ulong *time_ms)
{
    ...
    if (redfish_pwr_out <= 0) {
        verbose(VPOWER_ERROR, "%sWARNING%s Current power read: %lld W. Using last power read: %lld W", COL_YLW, COL_CLR,
    ...
    debug("inst. power %lu W / elapsed %lu s / energy %lu J / acc. energy %lu MJ", redfish_pwr_out, current_elapsed,
          current_energy, accumulated_energy);
