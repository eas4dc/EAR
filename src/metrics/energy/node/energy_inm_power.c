...
state_t inm_power_reading(struct ipmi_intf *intf, struct ipmi_data *out, inm_power_data_t *cpower)
{
    ...
    Byte 5 – Domain ID [0:3] – Domain ID (Identifies the domain that this Intel® Node Manager policy applies to) =00h –
    Entire platform [4:7] – Reserved. Write as 0000b. Byte 6 – Policy ID (ignored)
    * Answer
    ...
    * Byte 5:6 – Current Value (unsigned integer) see Section 3.1.1
    ...
    Byte 13:16 – Timestamp as defined by the IPMI v2.0 specification indicating when the response message was sent. If
    Intel® NM cannot obtain valid time, the timestamp is set to FFFFFFFFh as defined in the IPMI v2.0 specification.
    Byte 17:20 – Statistics Reporting Period (the timeframe in seconds, over which the firmware collects statistics).
    This is unsigned integer value. For all global statistics this field contains the time after the last statistics
    reset.
     *
     */
...
Byte 13:16 – Timestamp as defined by the IPMI v2.0 specification indicating when the response message was sent. If
Intel® NM cannot obtain valid time, the timestamp is set to FFFFFFFFh as defined in the IPMI v2.0 specification. 4b Byte
17:20 – Statistics Reporting Period (the timeframe in seconds, over which the firmware collects statistics). This is
unsigned integer value. For all global statistics this field contains the time after the last statistics reset. 4b
 */
...
#if SHOW_DEBUGS
    // debug("current power 0x%04x min power 0x%04x max power 0x%04x avg power
    // 0x%04x\n",current_power,min_power,max_power,avg_power);
    debug("current power %u min power %u max power %u avg power %u\n", (uint) current_power, (uint) min_power,
          (uint) max_power, (uint) avg_power);
    ...
    if (cpower->current_power > cpower->max_power) {
        verbose(VPOWER_ERROR, "enery_inm_power: New max power reading current:%u max %u", cpower->current_power,
                cpower->max_power);
    }
    if ((cpower->current_power < (cpower->max_power * MAX_TIMES_POWER_SUPPORTED)) && (cpower->current_power > 0)) {
        ...
        verbose(VPOWER_ERROR,
                "enery_inm_power:warning, invalid power detected. current %u, corrected to last valid power %u",
...
state_t inm_power_thread_main(void *p)
{
    ...
    if (inm_current_power_reading.current_power > 0) {
        ...
        debug("AVG power in last %lu sec is %u Total energy %lu\n", current_elapsed,
              inm_current_power_reading.avg_power, inm_power_accumulated_energy);
    } else {
