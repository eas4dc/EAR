...
static state_t inm_power_reading(ipmi_ctx_t ipmi_ctx, uint *maxp, uint *minp, uint *avgp, uint *currentp,
                                 uint *periodp);
...
static int intelnm_sdr_callback(ipmi_sdr_ctx_t sdr_ctx, uint8_t record_type, const void *sdr_record,
                                unsigned int sdr_record_len, void *arg)
{
    int ret;
    ...
    if ((ret = ipmi_sdr_oem_parse_intel_node_manager(sdr_ctx, sdr_record, sdr_record_len, &slave, &lun, &channel, NULL,
                                                     NULL, NULL, NULL)) < 0) {
        return 0;
    }
...
static int sdr_cache_get_cache_directory(const char *cache_dir, char *buf, unsigned int buflen)
{
    ...
    memset(tbuf, '\0', MAXPATHLEN + 1);
    if (sdr_get_config_directory(cache_dir, tbuf, MAXPATHLEN) < 0)
        return (-1);
...
state_t energy_init(void **c)
{
    ...
    while ((lret = pthread_mutex_trylock(&node_energy_lock_nm)) && (etries < MAX_LOCK_TRIES))
        etries++;
    ...
    if (!sdr_done) {
        ...
        if (ipmi_sdr_cache_open(sdr_ctx, ipmi_ctx, cachefilenamebuf) < 0) {
            ...
            char buff[4096];
            snprintf(buff, sizeof(buff), "%s/sbin/ipmi-oem intelnm get-node-manager-statistics mode=globalpower",
                     FREEIPMI_BASE);
        ...
        if (ipmi_sdr_cache_iterate(sdr_ctx, intelnm_sdr_callback, NULL) < 0) {
            if (!found) {
...
#define destroy_and_return(st)                                                                                         \
    ...
    *currentp = 0;
    ...
    if (ipmi_cmd_oem_intel_node_manager_get_node_manager_statistics(ipmi_ctx, channel, slave, lun, mode, domainid,
                                                                    policyid, obj_cmd_rs) < 0) {
    ...
    if (FIID_OBJ_GET(obj_cmd_rs, "average", &val) < 0) {
    ...
    *avgp = val;
    ...
    if (FIID_OBJ_GET(obj_cmd_rs, "maximum", &val) < 0) {
        debug("FIID_OBJ_GET: 'maximum': %s\n", fiid_obj_errormsg(obj_cmd_rs));
        destroy_and_return(EAR_ERROR);
    }
    *maxp = val;
    // debug("Maximum power %d\n", val);
    if (FIID_OBJ_GET(obj_cmd_rs, "minimum", &val) < 0) {
    ...
    *minp = val;
    ...
    if (FIID_OBJ_GET(obj_cmd_rs, "current", &val) < 0) {
        debug("FIID_OBJ_GET: 'current power': %s\n", fiid_obj_errormsg(obj_cmd_rs));
        destroy_and_return(EAR_ERROR);
    }
    *currentp = val;
    if (FIID_OBJ_GET(obj_cmd_rs, "statistics_reporting_period", &val) < 0) {
    ...
    *periodp = val;
    ...
    if ((*currentp < (*maxp * MAX_TIMES_POWER_SUPPORTED)) && (*currentp > 0)) {
        ...
        verbose(VPOWER_ERROR,
                "enery_nm_freeipmi:warning, invalid power detected. current %u, corrected to last valid power %u",
