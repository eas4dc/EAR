...
char *metrics_info_tostr(metrics_info_t *m, char *buffer)
{
    ...
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->cpu.layer, m->cpu.api_str, m->cpu.devs_count, m->cpu.scope_str,
             m->cpu.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->imc.layer, m->imc.api_str, m->imc.devs_count, m->imc.scope_str,
             m->imc.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->ram.layer, m->ram.api_str, m->ram.devs_count, m->ram.scope_str,
             m->ram.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->cch.layer, m->cch.api_str, m->cch.devs_count, m->cch.scope_str,
             m->cch.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->flp.layer, m->flp.api_str, m->flp.devs_count, m->flp.scope_str,
             m->flp.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->tmp.layer, m->tmp.api_str, m->tmp.devs_count, m->tmp.scope_str,
             m->tmp.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->pow.layer, m->pow.api_str, m->pow.devs_count, m->pow.scope_str,
             m->pow.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->cpi.layer, m->cpi.api_str, m->cpi.devs_count, m->cpi.scope_str,
             m->cpi.granularity_str);
    tsprintf(buffer, &sti, 1, "%s||%s||%u||%s||%s", m->gpu.layer, m->gpu.api_str, m->gpu.devs_count, m->gpu.scope_str,
             m->gpu.granularity_str);
    return buffer;
}
