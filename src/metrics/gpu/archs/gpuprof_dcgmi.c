...
static int get_user_devices()
{
    ...
    for (d = 0; d < devs_count; ++d) {
        ...
        while (popen_read(&p_tmp, "sssssa", &args[0], &args[1], &args[2], &args[3], &args[4])) {
            if (strcmp(args[1], "Serial") == 0) {
                serials[d] = (ullong) atoll(args[4]);
                debug("D%d detected serial: %llu", d, serials[d]);
            }
        }
        popen_close(&p_tmp);
        // Editing flag -i
...
GPUPROF_F_LOAD(gpuprof_dcgmi_load)
{
    ...
    if (strstr(strtolow(count), "error") != NULL) {
        debug("Failed while reading dcgmi discovery: DCGMI returned error");
        popen_close(&p);
        return;
    }
        ...
        if (strstr(buffer, "GPU") != NULL) {
            ...
            for (int pool_idx = 0; pool_idx < pool[dev].values_count; ++pool_idx) {
                ...
                if (event_pool_mask & (1 << pool_idx)) {
                    debug("D%d_M%d: %04.2lf += %04.2lf, %0.lf samples, %p hash", dev, pool_idx,
                          pool[dev].values[pool_idx], pool_aux_buffer[pool_aux_idx], pool[dev].samples_count,
                          pool[dev].hash);
...
GPUPROF_F_DATA_COPY(gpuprof_dcgmi_data_copy)
{
    ...
    memcpy(dataD[0].values, dataS[0].values, devs_count * events_count * sizeof(double));
...
static void pool_cache_update(char *hash, char *evs_supported, uint evs_count, short int mask)
{
    ...
    debug("Pool cache updated: pos: %hd mask: %hx hash: %s hash events count: %u supported events list: %s",
          pool_cache.dirty_pos, pool_cache.masks[pool_cache.dirty_pos], pool_cache.hashes[pool_cache.dirty_pos],
          pool_cache.hash_evs_count[pool_cache.dirty_pos], pool_cache.evs_supported[pool_cache.dirty_pos]);
