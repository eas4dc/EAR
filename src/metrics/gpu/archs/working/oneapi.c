...
static struct ze_s {
	...
	ze_result_t (*sFrequencyGetProperties) (zes_device_handle_t h, uint32_t *n, zes_freq_handle_t *ph);
...
static zes_device_handle_t sysmans[32]; // 32 GPUs are fairly enough
...
state_t oneapi_init(ctx_t *c)
{
	...
	for (dr = 0; dr < count_drivers; ++dr) {
		...
		for (dv = 0; dv < count_devices; ++dv) {
			...
			// Ok, its a GPU
			sysman[sysmans_count++] = devices[dv];
		}
		free(devices);
	}
	free(drivers);
	...
	// Monitor suscription
	pool = calloc(dev_count, sizeof(gpu_t));
...
static void oneapi_read_power(gpu_t *data)
{
	...
	for (sm = 0; sm < sysmans_count; ++sm) {
		...
		ret_fail(ze.sDeviceEnumPowerDomains(sysmans[i], &domains_count, domains));
		for (ma = 0; ma < domains_count; ++ma) {
			ret_fail(ze.sPowerGetEnergyCounter(domains[sm], &info));
			// It has to be tested.
			data->energy_j = ((double) info.energy) / 1000.0;
		}
	}
...
static void oneapi_read_frequency(gpu_t *data)
{
	...
	for (sm = 0; sm < sysmans_count; ++sm) {
		// Retrieving the domains
		ret_fail(ze.sDeviceEnumFrequencyDomains(sysmans[i], &domains_count, domains));
		for (ma = 0; ma < domains_count; ++ma) {
			// Getting domain characteristics
			ret_fail(ze.sFrequencyGetProperties(domains[sm], &props));
			//
			ret_fail(ze.sFrequencyGetState(domains[sm], &info));
...
static void oneapi_read_utilization(gpu_t *data)
{
	...
	for (sm = 0; sm < sysmans_count; ++sm) {
		// Retrieving the domains
		ret_fail(ze.sDeviceEnumEngineGroups(sysmans[i], &domains_count, domains));
		for (ma = 0; ma < domains_count; ++ma) {
			// Getting domain characteristics
			ret_fail(ze.sEngineGetProperties(domains[sm], &props));
			// Maybe ZES_ENGINE_GROUP_COMPUTE_ALL?
			if (props.type == ZES_ENGINE_GROUP_ALL) {
				ret_fail(ze.sEngineGetActivity(domains[sm], &info));
				// Utilization here, it has to be tested
				util.gpu = (s2.activeTime - s1.activeTime) / (s2.timestamp - s1.timestamp);
			}
		}
	}
...
state_t oneapi_read(ctx_t *c, gpu_t *data)
{
	...
	memcpy(data, pool, sysmans_count * sizeof(gpu_t));
