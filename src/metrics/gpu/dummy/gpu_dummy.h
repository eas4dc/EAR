...
state_t gpu_dummy_count(ctx_t *c, uint *gpu_count);

state_t gpu_dummy_pool(void *c);

state_t gpu_dummy_read(ctx_t *c, gpu_t *data);

state_t gpu_dummy_read_copy(ctx_t *c, gpu_t *data2, gpu_t *data1, gpu_t *data_diff);

/* Sets the required variables to work with gpu_dummy_data_* functions. */
state_t gpu_dummy_data_init(uint dev_count);
