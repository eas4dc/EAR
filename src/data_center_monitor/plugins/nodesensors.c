...
declr_up_action_init(_conf)
{
    ...
    for (uint edc = 0; edc < num_edcmons; edc++)
        print_edcmon_tags_conf(&edcmons[edc], edc);
    ...
    if (!query)
        return rsprintf("[D] Not enough memory, disabled");
    num_pdus = (uint *) calloc(total_enabled, sizeof(uint));
    if (!num_pdus)
        return rsprintf("[D] Not enough memory, disabled");

    for (uint i = 0; i < total_enabled; i++)
        query[i] = (char *) calloc(QUERY_SIZE, sizeof(char));
    // Error control
...
static state_t static_read_power(uint i, uint *num_rows, uint expected_items)
{
    ...
    while (it < expected_items) {
        while (popen_read2(&nodesensor_p, sep, "ss", &node, &power)) {
            ...
            strncpy(edcmon_info.edcmon_data[it + idx].nodename, node,
                    sizeof(edcmon_info.edcmon_data[it + idx].nodename));
            ...
            debug("nodesensors[%u] time %u node %s power %lf W ", it + idx,
                  (uint) edcmon_info.edcmon_data[it + idx].timestamp, edcmon_info.edcmon_data[it + idx].nodename,
                  edcmon_info.edcmon_data[it + idx].power);
            it++;
        }
    }
    *num_rows = it;
