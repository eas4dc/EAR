...
int main(int argc, char *argv[])
{
    ...
    printf("Max sockets supported: %d\n", MAX_SOCKETS_SUPPORTED);
#if USE_GPUS
    ...
    if (my_cluster.eard.force_frequencies) {
        printf("EARD controls node CPU frequencies.");
    }
    printf("Default cluster policy is %s\n", my_cluster.power_policies[my_cluster.default_policy].name);
    printf("EARL optimization by default set to %d\n", DEFAULT_EARL);

    if (!is_privileged_command(&my_cluster))
        return 0;
    ...
    if (hack == NULL)
        strcpy(install, installp);
    else
        strcpy(install, hack);

    if (my_node_conf) {
        xsnprintf(path_files, sizeof(path_files), "%s/lib/plugins/policies/%s.so", install,
                  my_node_conf->policies[my_cluster.default_policy].name);
    }
    print_value_str("Energy optimization policy", HACK_POWER_POLICY, path_files);
#if USE_GPUS
#ifdef GPU_OPT
    if (my_node_conf) {
        xsnprintf(path_files, sizeof(path_files), "%s/lib/plugins/policies/gpu_%s.so", install,
                  my_node_conf->policies[my_cluster.default_policy].name);
    }
#else
