...
#define CLEAN_GPU_APPS                                                                                                 \
    "DELETE Applications,Jobs,Signatures,Power_signatures,GPU_signatures FROM Applications inner join Jobs on "        \
    "Applications.job_id = Jobs.job_id AND "                                                                           \
    "Applications.step_id = Jobs.step_id inner join Signatures on Signatures.id = signature_id inner join "            \
    "Power_signatures on "                                                                                             \
...
#define CLEAN_GPU_LOOPS                                                                                                \
    "DELETE Loops,Signatures,GPU_signatures FROM Loops inner join Jobs on Jobs.job_id = Loops.job_id  AND "            \
...
#define CLEAN_MPI_APPS                                                                                                 \
    "DELETE Applications,Jobs,Signatures,Power_signatures from Applications inner join Jobs on Applications.job_id = " \
    "Jobs.job_id and Applications.step_id = Jobs.step_id "                                                             \
    "inner join Signatures on Signatures.id = signature_id inner join Power_signatures on Power_signatures.id = "      \
    "power_signature_id "

#define CLEAN_NMPI_APPS                                                                                                \
    "DELETE Applications,Jobs,Power_signatures from Applications inner join Jobs on Applications.job_id = "            \
    "Jobs.job_id and Applications.step_id = Jobs.step_id "                                                             \
    "inner join Power_signatures on Power_signatures.id = power_signature_id "

#define CLEAN_LOOPS_JOBS                                                                                               \
    "DELETE Signatures,Loops FROM Loops INNER JOIN Jobs ON Jobs.job_id = Loops.job_id AND Jobs.step_id = "             \
    "Loops.step_id INNER JOIN Signatures ON Loops.signature_id = Signatures.id "

#define CLEANUP_LOOPS_JOBS                                                                                             \
    "DELETE Loops FROM Loops INNER JOIN Jobs ON Jobs.job_id = Loops.job_id AND Jobs.step_id = Loops.step_id"
...
#define CLEANUP_APPS                                                                                                   \
    "DELETE Applications,Jobs from Applications inner join Jobs on Applications.job_id = Jobs.job_id and "             \
    "Applications.step_id = Jobs.step_id "
...
#endif
    printf("\t-j jobid.stepid\tJob id and step id to delete. If no step_id is introduced, every step within the job "
           "will be deleted\n");
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < queries_id; i++) {
        query_filters = 0;
        if (rm_loops && num_days < 1 &&
            i < NUM_LOOP_Q) { // this is more precise as it doesn't need Jobs to exist to delete them
            if (job_id > -1) {
