...
void print_header(int fd, char ***values, __attribute__((unused))int columns, int rows)
{
	int i = 0;
	dprintf(fd, "%s", values[i][0]);
	for (i = 1; i < rows; i++) {
...
bool print_power_signature_format(application_t *app, char f)
{
    ...
    switch(fmt) {
        ...
        case TIME: //time
            ...
            break;
        case ENERGY: //energy
            print_format(ENERGY_WIDTH, app->power_sig.DC_power*app->power_sig.time, 2);
            break;
		case CARBON_FOOTPRINT:
            print_format(CARBON_WIDTH, (((app->power_sig.DC_power*app->power_sig.time)*PUE)/(3600*1000))*CARBON_INTENSITY, 2);
			break;
        case DC_POWER: //DC power
            print_format(POWER_WIDTH, app->power_sig.DC_power, 2);
            break;
        case DRAM_POWER: //DRAM power
            print_format(DRAM_POWER_WIDTH, app->power_sig.DRAM_power, 2);
            break;
        case PCK_POWER: //PCK power
            print_format(PCK_POWER_WIDTH, app->power_sig.PCK_power, 2);
            break;
...
bool print_signature_format(signature_t *sig, char f, int32_t time_length)
{
    ...
    switch(fmt) {
        case DC_POWER: //DC power
            print_format(POWER_WIDTH, sig->DC_power, 2);
            break;
        case DRAM_POWER: //DRAM power
            print_format(DRAM_POWER_WIDTH, sig->DRAM_power, 2);
            break;
        case PCK_POWER: //PCK power
            print_format(PCK_POWER_WIDTH, sig->PCK_power, 2);
            break;
        case GBS: //GBS
            print_format(GBS_WIDTH, sig->GBS, 2);
            break;
        case TPI: //TPI
            print_format(TPI_WIDTH, sig->TPI, 2);
            break;
        case CPI: //CPI
            print_format(CPI_WIDTH, sig->CPI, 2);
            break;
        case GFLOPS: //gflops
            print_format(GFLOPS_WIDTH, sig->Gflops/sig->DC_power, 2);
            break;
        case TIME: //time
        ...
        case VPI: //vpi
            ...
            break;
        case ENERGY: //energy
            print_format(ENERGY_WIDTH, sig->DC_power*sig->time, 2);
            break;
		case CARBON_FOOTPRINT:
            ...
            { // declare a custom scope to avoid compiler complains
                ...
                if (ptr->num_gpus > 0) {
                    ...
                    if (num_gpus > 0) {
                        ...
                        print_format_custom("%-7.2lf/%-7.2lf %-7.3lf %6lu%%/%-7s %-10.2lf", gpu_total_power,
                                gpu_power, (double)gpu_freq/1000000, gpu_util, mem_util_buf, gpu_gflops);
                    }
                    else {
...
void print_app_format(application_t *app, char *fmt, int num_nodes)
{
	...
	if (app->is_mpi){ select_color_by_phase(app->signature.CPI, app->signature.GBS, app->signature.IO_MBS, app->signature.Gflops);}
	...
	for (i = 0; i < strlen(fmt); i++) { //this is preferable to a while loop because of the use of continues
		...
		if (app->is_mpi && !all_pow_sig) is_consumed = print_signature_format(&app->signature, fmt[i], 0);
		else is_consumed = print_power_signature_format(app, fmt[i]);
		...
		switch(tmp) {
			...
			case USER_ID: //user id
				print_format(OTHER_WIDTH, app->job.user_id);
				break;
...
void print_header_format(char *fmt)
{
	while (*fmt) {
		switch(*fmt) {
			case APP_ID: //application id
			...
			case DATE_ITER:
#if REPORT_TIMESTAMP
				print_format(OTHER_WIDTH, "DATE");
#else
				print_format(OTHER_WIDTH, "ITER.");
#endif
				break;
			case NODE_ID: //node id
				print_format(OTHER_WIDTH, "NODE ID");
				break;
			case NUM_NODES: //num nodes
			...
			case ALL_FREQS: //all three freqs in format
				print_format_custom("%-3s/%-3s/%-8s ", "AVG", "DEF", "IMC(GHz)");
				break;
			case FREQUENCY: //frequency
				print_format(FREQ_WIDTH, "AVG(GHz)");
				break;
			case IMC_FREQ: //imc freq
				print_format(FREQ_WIDTH, "IMC(GHz)");
				break;
			case DEF_FREQ: //def_f
			...
			case VPI: //vpi
				...
				break;
			case ENERGY: //energy
				print_format(ENERGY_WIDTH, "ENERGY(J)");
				break;
			case CARBON_FOOTPRINT:
...
void print_loop_format(loop_t *loop, char *fmt)
{
	...
	for (uint32_t i = 0; i < strlen(fmt); i++) { //this is preferable to a while loop because of the use of continues
		...
		//is_consumed is 1 only if that format char corresponds to signature/power signature
		if (print_signature_format(&loop->signature, fmt[i], 3)) continue;
...
void read_applications_from_files(char *path, char *format)
{
	...
	if (full_length) print_full_apps(apps, num_apps, false, format);
	else print_short_apps(apps, num_apps, false, format);
}
...
int main(int argc, char *argv[])
{
	...
	if (strlen(format) < 1) {
		if (is_loops)
			...
			strcpy(format, "jxuapnrtpgCseG");
		else
			strcpy(format, "jxuapNrtpgCseG");
	}

	if (file_name != NULL) read_applications_from_files(file_name, format);
	else if (is_events) read_events(user, &query_adds);
	else if (is_loops) read_loops(user, &query_adds, format);
	else read_applications_from_database(user, &query_adds, format);
