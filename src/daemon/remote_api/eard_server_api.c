...
char can_propagate(request_t *command)
{
    ...
    if (command->num_nodes < 1) {
        ...
        if (command->node_dist > total_ips)
            return 0;
        if (self_ip == -1 || ips == NULL || total_ips < 1)
            return 0;
    }
...
int get_self_ip()
{
    ...
    if (s != 0) {
        error("get_self_ip:getaddrinfo fails for port %s (%s)", buff, strerror(errno));
        return EAR_ERROR;
    }
...
request_header_t propagate_data(request_t *command, uint port, void **data)
{
    ...
    if (command->num_nodes < 1) {
        ...
        for (i = 0; i < NUM_PROPS; i++) {
            ...
            // we calculate how many ips will be sent to this node by getting the max possible ip index and subtracting
            // the starting one
    ...
    *data     = final_data;
    ...
    if (default_type == EAR_ERROR && final_size > 0) {
        free(final_data);
        head.size = 0;
    } else if (final_size < 1 && (default_type != EAR_TYPE_APP_STATUS || default_type != EAR_ERROR))
        head.type = EAR_ERROR;
...
int propagate_powercap_status(request_t *command, uint port, powercap_status_t **status)
{
    ...
    request_header_t head = propagate_data(
        command, port,
        (void **) &temp_status); // we don't check for number of powercap_status since we will have 1 at most
...
int propagate_get_power(request_t *command, uint port, power_check_t **power)
{
    ...
    request_header_t head = propagate_data(
        command, port, (void **) &temp_power); // we don't check for number of uints since we will have 1 at most
...
int propagate_policy_status(request_t *command, uint port, policy_status_t **status)
{
    ...
    num_status =
        propagate_and_cat_data(command, port, (void **) &temp_status, sizeof(status_t), EAR_TYPE_POLICY_STATUS, 1);
    *status = temp_status;
    return num_status;
}
...
int propagate_app_status(request_t *command, uint port, app_status_t **status, int num_apps)
{
    ...
    num_status = propagate_and_cat_data(command, port, (void **) &temp_status, sizeof(app_status_t),
                                        EAR_TYPE_APP_STATUS, num_apps);
    if (num_status == 0)
        return 0;
    *status = temp_status;
    return num_status;
