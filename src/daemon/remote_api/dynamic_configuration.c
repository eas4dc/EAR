...
typedef struct eard_policy_symbols {
    state_t (*set_risk)(policy_conf_t *ref_pol, policy_conf_t *node_pol, ulong risk, ulong opt_target, ulong cfreq,
                        ulong *nfreq, ulong *f_list, uint nump);
} eard_polsym_t;
...
ulong lower_valid_freq(ulong f, uint p_states, ulong *f_list)
{
    ...
    if (found)
        ...
        return f_list[default_policy_context.p_state];
}
...
int dynconf_set_def_pstate(uint p_states, uint p_id)
{
    ...
    if (p_states > frequency_get_num_pstates())
        return EAR_ERROR;
...
void dyncon_get_policy_status(int fd, request_t *command)
{
    policy_status_t *status;
    int num_status              = propagate_policy_status(command, my_cluster_conf.eard.port, &status);
    unsigned long return_status = num_status;
    debug("return_status %lu status=%p", return_status, status);
    if (num_status < 1) {
        error("Panic propagate_policy_status returns less than 1 status");
        return_status = 0;
        _write(fd, &return_status, sizeof(return_status));
        return;
    }
    powermon_get_policy_status(&status[num_status - 1]);
    send_data(fd, sizeof(status_t) * num_status, (char *) status, EAR_TYPE_POLICY_STATUS);
...
void dyncon_release_idle_power(int fd, request_t *command)
{
    ...
    if (return_status < 1)
        error("dyncon_release_idle power and return status < 1");
...
void dyncon_get_powerstatus(int fd, request_t *command)
{
    ...
    send_data(fd, sizeof(powercap_status_t) * return_status + ((sizeof(uint) * 2 + sizeof(int)) * (status->num_greedy)),
...
void dyncon_set_risk(int fd, request_t *command)
{
    ...
    for (i = 0; i < my_node_conf->num_policies; i++) {
        if (polsyms_fun[i].set_risk != NULL) {
            verbose(VRAPI, "Setting risk level for %s to %lu", my_node_conf->policies[i].name,
                    (unsigned long) command->my_req.risk.level);
            polsyms_fun[i].set_risk(&my_original_node_conf.policies[i], &my_node_conf->policies[i],
...
{
    if (comm->req == EAR_RC_NEW_JOB || comm->req == EAR_RC_END_JOB || comm->req == EAR_RC_NEW_TASK ||
        comm->req == EAR_RC_STATUS || comm->req == EAR_RC_NODE_PURGE)
        return 1;
    // if ((comm->req == last_command) && (comm->time_code == last_command_time)) return 0;
    for (uint i = 0; i < MAX_HISTORIC_COMM; i++) {
        //  if ((list_last_commands[i].req == comm->req ) && (list_last_commands[i].time_code == comm->time_code))
        //  return 0;
        if (!memcmp(&list_last_commands[i], comm, sizeof(request_t)))
            return 0;
    }
    return 1;
}
...
#define check_and_set_verbose(domain, val)                                                                             \
    if (!strcasecmp(level, #domain)) {                                                                                 \
        domain = value;                                                                                                \
        return EAR_SUCCESS;                                                                                            \
    }
#define check_and_reset_verbose(domain)                                                                                \
    if (!strcasecmp(level, #domain)) {                                                                                 \
        domain = domain##_DEF;                                                                                         \
        return EAR_SUCCESS;                                                                                            \
    }

state_t _dyncon_reset_verbose(char *level)
{
    check_and_reset_verbose(VCONNECT);
    check_and_reset_verbose(VCONF);
    check_and_reset_verbose(VEARD);
    check_and_reset_verbose(VEARD_PC);
    check_and_reset_verbose(VEARD_INIT);
    check_and_reset_verbose(VEARD_NMGR);
    check_and_reset_verbose(VEARD_NMGR_DEBUG);
    check_and_reset_verbose(VEARD_LAPI);
    check_and_reset_verbose(VNODEPMON);
    check_and_reset_verbose(VNODEPMON_BASIC);
    check_and_reset_verbose(VJOBPMON);
    check_and_reset_verbose(VJOBPMON_BASIC);
    check_and_reset_verbose(VPMON_DEBUG);
    check_and_reset_verbose(VTASKMON);
    check_and_reset_verbose(VRAPI);
    check_and_reset_verbose(VAPI);
    check_and_reset_verbose(VCHCK);
    check_and_reset_verbose(VGM);
    check_and_reset_verbose(VGM_PC);
    check_and_reset_verbose(VCCONF);
    check_and_reset_verbose(VPRIV);
    check_and_reset_verbose(VMYSQL);
    check_and_reset_verbose(VDBH);
    check_and_reset_verbose(VTYPE);
    check_and_reset_verbose(VCOMM);
    return EAR_ERROR;
}

state_t _dyncon_set_verbose(char *level, int32_t value)
{
    check_and_set_verbose(VCONNECT, value);
    check_and_set_verbose(VCONF, value);
    check_and_set_verbose(VEARD, value);
    check_and_set_verbose(VEARD_PC, value);
    check_and_set_verbose(VEARD_INIT, value);
    check_and_set_verbose(VEARD_NMGR, value);
    check_and_set_verbose(VEARD_NMGR_DEBUG, value);
    check_and_set_verbose(VEARD_LAPI, value);
    check_and_set_verbose(VNODEPMON, value);
    check_and_set_verbose(VNODEPMON_BASIC, value);
    check_and_set_verbose(VJOBPMON, value);
    check_and_set_verbose(VJOBPMON_BASIC, value);
    check_and_set_verbose(VPMON_DEBUG, value);
    check_and_set_verbose(VTASKMON, value);
    check_and_set_verbose(VRAPI, value);
    check_and_set_verbose(VAPI, value);
    check_and_set_verbose(VCHCK, value);
    check_and_set_verbose(VGM, value);
    check_and_set_verbose(VGM_PC, value);
    check_and_set_verbose(VCCONF, value);
    check_and_set_verbose(VPRIV, value);
    check_and_set_verbose(VMYSQL, value);
    check_and_set_verbose(VDBH, value);
    check_and_set_verbose(VTYPE, value);
    check_and_set_verbose(VCOMM, value);
    return EAR_ERROR;
}

#define debug_if_not_null(value)                                                                                       \
    if (value == NULL) {                                                                                               \
        debug(#value " not set");                                                                                      \
    } else {                                                                                                           \
...
void dyncon_process_message(char *in_message)
{
    ...
    while (key != NULL) {
        ...
        if (!strcasecmp(key, "type"))
            ...
            level = val;
        else if (!strcasecmp(key, "values"))
            values = val;
        else if (!strcasecmp(key, "value"))
            values = val;
    ...
    } else if (type != NULL && !strcasecmp(type, "verbose")) {
        if (action != NULL && !strcasecmp(action, "set")) {
            if (level != NULL && num_values > 0) {
                if (state_fail(_dyncon_set_verbose(level, int_values[0]))) {
                    warning("Verbose definition for %s does not exist", level);
                }
            }
        } else if (action != NULL && !strcasecmp(action, "reset")) {
            if (level != NULL) {
                if (state_fail(_dyncon_reset_verbose(level))) {
                    warning("Verbose definition for %s does not exist", level);
                }
            }
        }
...
state_t process_remote_requests(int clientfd)
{
    ...
    switch (req) {
        ...
        case EAR_RC_STATUS:
            ...
            return EAR_SUCCESS;
            break;
        case EAR_RC_POLICY_STATUS:
            verbose(VRAPI + 1, "Policy status received");
            dyncon_get_policy_status(clientfd, &command);
            return EAR_SUCCESS;
            break;
...
void policy_load()
{
    ...
    for (i = 0; i < my_node_conf->num_policies; i++) {
        sprintf(basic_path, "%s/eard_policies/%s_eard.so", my_cluster_conf.install.dir_plug,
                my_node_conf->policies[i].name);
...
void *eard_dynamic_configuration(void *tmp)
{
    ...
    if (node_found == EAR_ERROR)
        verbose(VRAPI, "Node not found in configuration file");
