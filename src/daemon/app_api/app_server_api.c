...
{
    if (write(fd_out, data, sizeof(app_recv_t)) != sizeof(app_recv_t))
        return EAR_ERROR;
    return EAR_SUCCESS;
}
...
int find_first_free()
{
    int i;
    i = 0;
    while ((apps_eard[i].recv == -1) && (i < MAX_FDS))
        i++;
    if (i == MAX_FDS)
        return -1;
    return i;
...
void accept_new_connection(char *root, int pid)
{
        ...
        if (i < 0) {
            ...
            for (i = 0; i < MAX_FDS; i++) {
                if (apps_eard[i].recv >= 0) {
                    check_connection(root, i);
                }
            }
            if ((i = find_first_free()) < 0)
                return;
        }
    }
    ...
    if (apps_eard[i].send < 0) {
        if (errno == ENXIO) {
            while ((tries < MAX_TRIES) && ((apps_eard[i].send = open(eard_to_app, O_WRONLY | O_NONBLOCK)) < 0) &&
                   (errno == ENXIO))
                tries++;
        }
    }
...
void remove_connection(char *root, int i)
{
    ...
    for (i = 0; i < MAX_FDS; i++) {
        if (apps_eard[i].recv >= 0) {
            if (apps_eard[i].recv > max)
                max = apps_eard[i].recv;
            new_con = i;
        }
    }
    connections = new_con;
...
static void ear_set_gpufreq(int fd_out, uint gpuid, ulong gpu_freq)
{
    ...
    if ((ret = mgt_gpu_freq_data_alloc(&array)) != EAR_SUCCESS) {
        data.ret = ret;
        send_app_answer(fd_out, &data);
    }
...
void app_api_process_connection(char *root)
{
    ...
    if (read(fd_app_to_eard, &req, sizeof(app_send_t)) != sizeof(app_send_t))
        return;
    switch (req.req) {
        case CONNECT:
...
void *eard_non_earl_api_service(void *noinfo)
{
    ...
    if (state_fail(energy_load(energy_plugin_obj))) {
        verbose(VAPP_API + 1, "Energy plug-in load method failed: %s", state_msg);
    }
    if (state_fail(energy_init(&my_eh_app_api))) {
        error("Error initializing energy_init in No-EARL_API thread: %s", state_msg);
        pthread_exit(0);
    }
