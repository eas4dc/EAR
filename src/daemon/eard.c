...
static void set_sigaction(int signum, const struct sigaction *act);
...
void check_policy(policy_conf_t *p)
{
    ...
    if (p->def_freq == (float) 0) {
        if (p->p_state >= frequency_get_num_pstates())
            p->p_state = frequency_get_nominal_pstate();
    } else {
...
void eard_restart()
{
    ...
    if (ear_install == NULL) {
        ...
        sprintf(my_bin, "%s/sbin/eard", ear_install);
...
void signal_handler(int sig)
{
    ...
    if (sig == SIGPIPE) {
        verbose(VCONF, "SIGPIPE received! Closing pipes...\nApplication terminated abnormally.");
    ...
    if (sig == SIGTERM || sig == SIGINT) {
        if (sig == SIGTERM) {
            verbosen(VCONF, "SIGTERM received!");
        } else {
            verbosen(VCONF, "SIGINT received!");
        }
        verbose(VCONF, " Finishing...");
        eard_must_exit = 1;
        eard_exit(0);
    }

    if (sig == SIGSEGV) {
        verbose(VCONF, "SIGSEGV received in main EARD thread! Stack...");
        print_stack(verb_channel);
    }

    if (sig == SIGUSR2) {
        verbose(VCONF, "SIGUSR2 received! Reopening log...");
        reopen_log();
        return;
    }

    if (sig == SIGHUP) {
        verbose(VCONF, "SIGHUP received! Reloading EAR configuration file (%s)...", my_ear_conf_path);
        signal_sighup = 1;
        ...
        if (read_cluster_conf(my_ear_conf_path, &my_cluster_conf) != EAR_SUCCESS) {
            ...
            verbose(VCONF, "Loading EAR configuration... There are %d Nodes in the cluster", my_cluster_conf.num_nodes);
            ...
            verbose(VCONF, "Getting my node configuration...");
            free(my_node_conf);
...
void signal_catcher()
{
    // Create the mask of blocked signals
    sigset_t eard_mask;
    sigfillset(&eard_mask);
    // Remove the following signals from the block mask
    ...
    // Set the blocked signals list from eard_mask
    ...
    set_sigaction(SIGPIPE, (const struct sigaction *) &action);
    set_sigaction(SIGTERM, (const struct sigaction *) &action);
    set_sigaction(SIGINT, (const struct sigaction *) &action);
    set_sigaction(SIGHUP, (const struct sigaction *) &action);
    set_sigaction(SIGUSR2, (const struct sigaction *) &action);
    set_sigaction(SIGSEGV, (const struct sigaction *) &action);
}
...
int main(int argc, char *argv[])
{
    ...
    if ((coeffs_size == 0) && (coeffs_default_size == 0))
        verbose(VEARD_INIT, "No coefficients found, power policies will not be applied");
    ...
    coeffs_default_conf =
        create_coeffs_default_shared_area(coeffs_default_path, my_coefficients_default, coeffs_default_size);
    ...
    my_ser_cluster_conf =
        create_ser_cluster_conf_shared_area(ser_cluster_conf_path, ser_cluster_conf, ser_cluster_conf_size);
    ...
    if (eard_min_pstate < 0) {
        ...
        if (eard_min_pstate >= api_pstate_count) {
            eard_error("The minimum P-State (%d) must be less than the number of available P-States (%u)",
                       eard_min_pstate, api_pstate_count);
        }
    }
    ...
    verbose(VEARD_INIT, "Loading the energy plug-in from the main thread...");

    char energy_plugin_obj[MAX_PATH_SIZE];
    build_energy_plugin_path(energy_plugin_obj, sizeof(energy_plugin_obj), &my_cluster_conf, my_node_conf);

    if (state_fail(energy_load(energy_plugin_obj))) {
        verbose(VEARD_INIT + 1, "Energy plug-in load method failed: %s", state_msg);
    }
    if (state_fail(energy_init(&eard_handler_energy))) {
        error("While initiating the energy plug-in: %s", state_msg);
        error_energy = 1;
    }
...
static void set_sigaction(int signum, const struct sigaction *act)
{
    if (sigaction(signum, act, NULL) < 0) {
        error("Setting action for signal %d (%s)", signum, strerror(errno));
    }
}
