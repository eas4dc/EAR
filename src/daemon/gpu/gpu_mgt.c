...
state_t gpu_mgr_init()
{
    ...
    if (my_node_conf->gpu_def_freq > 0) {
        ...
        verbose(VCONF, "Configuring GPU default frequency at job init/end to %lu", my_node_conf->gpu_def_freq);
    } else {
...
state_t gpu_mgr_set_freq(uint num_dev_req, ulong *freqs)
{
    ...
    if (num_dev_req > num_dev) {
        error("Num GPUS requestedi(%u)  to change frequency greather than num GPUS detected (%u)", num_dev_req,
              num_dev);
        num_dev_req = num_dev;
    }
