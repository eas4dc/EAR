...
/** This functions creates the name of the file mapping the shared memory for the dynamic power settings, it is placed
 * at TMP
 */
...
{
    if (mem != NULL)
        munmap(mem, sizeof(settings_conf_t));
...
void print_settings_conf(settings_conf_t *setting)
{
    ...
    verbose(VCONF,
            "\tMax. CPU freq.: %lu / Def. CPU freq.: %lu / Def. P-State: %u"
            " / CPU policy th.: %.2lf / CPU max. P-State: %d\n",
            setting->max_freq, setting->def_freq, setting->def_p_state, setting->settings[0], setting->cpu_max_pstate);
...
resched_t *create_resched_shared_area(char *path, int *fd)
{
    ...
    resched_t *mem =
        (resched_t *) create_shared_area(path, perms, (char *) &my_settings, sizeof(my_settings), &fd_resched, 1);
...
app_mgt_t *create_app_mgt_shared_area(char *path, int *fd)
{
    ...
    app_mgt_t *mem;
    mem = (app_mgt_t *) create_shared_area(path, perms, (char *) &my_app_data, sizeof(my_app_data), &fd_app_mgt, 1);
...
pc_app_info_t *create_pc_app_info_shared_area(char *path, int *fd)
{
    ...
    pc_app_info_t *mem;
    mem = (pc_app_info_t *) create_shared_area(path, perms, (char *) &my_data, sizeof(my_data), &fd_pc_app_info, 1);
