...
/* Below macros define the indices to store/recover fds to/from the saved array used in eards_save_connection and
 * eards_recover_connection. */
...
static int eards_open(char *path, int flags, int *ear_fd_req, uint max_tries)
{
    ...
    int connected = 0;
    do {
        debug("Connecting with EARD using file %s", path);
        // ENXIO is for pipes
        if (((fd = open(path, flags)) < 0) && (errno == ENXIO))
            tries++;
        else if (fd >= 0)
            connected = 1;
	else
...
int eards_connect(application_t *my_app, ulong lid)
{
    ...
    debug("Connecting with daemon job_id=%lu step_id%lu and PID %d", my_app->job.id, my_app->job.step_id, getpid());
    i = 0;
    ...
    connected = eards_open(ear_commreq_global, O_WRONLY | O_NONBLOCK, &ear_fd_req_global, 1);
    debug("Global Com pipe connected %s", (connected ? "yes" : "no"));

    if (!connected) {
        // Not possible to connect with ear_daemon
        debug("while opening the communicator for requests %s (%d attempts) (%s)", ear_commreq_global, tries,
              strerror(errno));
        return_print(EAR_ERROR, "while opening the communicator for requests %s (%d attempts) (%s)", ear_commreq_global,
    ...
    connecting = 1;
    /* We now connect with the EARD, the tag CONNECT_EARD_NODE_SERVICES is used as the first service requested (for
    ...
    if (ear_fd_req_global >= 0) {
        ...
        ear_fd_ack = open(ear_commack, O_RDONLY | O_NONBLOCK);

        debug("ACK pipe created with fd %d", ear_fd_ack);

        /* According to the fifo(7) man page, opening a fifo in non-blocking
         * mode succeeds even if no one has opened on the write side yet. */
        if (ear_fd_ack < 0) {
        ...
        int connected_req = eards_open(ear_commreq, O_WRONLY | O_NONBLOCK, &ear_fd_req, 10);

        debug("%s pipe connected %s", ear_commreq, (connected_req ? "yes" : "no"));
#if 0
        if ((ear_fd_req = open(ear_commreq, O_WRONLY | O_NONBLOCK)) < 0) {
#endif
        if (!connected_req) {
            error("Error opening local req communicator with EARD (%s), closing the EARD connection.", strerror(errno));
            CLOSE_LOCAL_COMM();
            connecting = 0;
            return EAR_ERROR;
        }
...
static state_t eards_save_connection(char *tmp, ulong job_id, ulong step_id, ulong local_id)
{
    ...
    if (state_fail(create_base_path(base_path, sizeof(base_path), tmp, (uint) my_id, (uint) local_id))) {
    ...
    snprintf(fds_file_pathname, sizeof(fds_file_pathname), "%s/%s", base_path, FDS_FILENAME);
    ...
    if (fds_file < 0) {
        verbose(2, "%sWarning%s FDs file couldn't be opened: %d (%s)", COL_YLW, COL_CLR, errno, strerror(errno));
        return_msg(EAR_ERROR, "FDs file could not be opened.");
    }
    // Write the communication info
    if (write(fds_file, fds, sizeof(fds)) < 0) {
        verbose(2, "%sWarning%s FDs file couldn't be written: %d (%s)", COL_YLW, COL_CLR, errno, strerror(errno));
        close(fds_file);
...
state_t eards_recover_connection(char *tmp, ulong job_id, ulong step_id, ulong local_id, int rank_id)
{
    ...
    if (fds_file < 0) {
        verbose(2, "%sWarning%s FDs file %s couldn't be opened: %d (%s)", COL_YLW, COL_CLR, fds_file_pathname, errno,
                strerror(errno));
        return_msg(EAR_ERROR, "FDs file could not be opened.");
    }
    ...
    if (read(fds_file, fds, sizeof(fds)) < 0) {
        verbose(2, "%sWarning%s FDs file couldn't be read: %d (%s)", COL_YLW, COL_CLR, errno, strerror(errno));
        close(fds_file);
