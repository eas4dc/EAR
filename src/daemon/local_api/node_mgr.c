...
/** Initialices the lock file and attaches the shared memory region. nodelist is internally allocated. Must be used by
 * earl */
...
state_t nodemgr_lock_init(char *tmp)
{
    ...
    if (ear_file_is_regular(node_mgr_lock)) {
        ...
        fd_node_mgr_lck = ear_file_lock_create(node_mgr_lock);
        if (fd_node_mgr_lck >= 0)
            chmod(node_mgr_lock, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
        umask(oldm);
        if (fd_node_mgr_lck < 0)
            return EAR_ERROR;
    }
