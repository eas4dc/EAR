...
void pmgt_process_message(char *level, int32_t num_values, int32_t values[num_values])
{
    ...
    if (!strcasecmp(level, "domain")) {
        ...
        if (num_values == 1) {
            ...
            freturn(pcsyms_fun[DOMAIN_CPU].set_powercap_value, 0, LEVEL_DEVICE, (uint32_t *) values,
                    current_util[DOMAIN_CPU]);
            return;
        }
        ...
        if (num_values == 1) {
            ...
            freturn(pcsyms_fun[DOMAIN_GPU].set_powercap_value, 0, LEVEL_DEVICE, (uint32_t *) values,
                    current_util[DOMAIN_GPU]);
            return;
        }
        ...
        if (num_values < gpu_pc_num_gpus + pc_topology_info.socket_count * 2) {
            warning("powercap for device must include all DRAMs + CPUs + GPUs (received %d, required %d)", num_values,
                    gpu_pc_num_gpus + pc_topology_info.socket_count * 2);
            return;
        } else {
            for (int32_t i = 0; i < pc_topology_info.socket_count; i++)
                debug("Setting CPU[%d] to %u Watts", i, values[i]);
            ...
            if (domains_loaded[DOMAIN_CPU])
                freturn(pcsyms_fun[DOMAIN_CPU].set_powercap_value, 0, LEVEL_DEVICE, (uint32_t *) values,
                        current_util[DOMAIN_CPU]);
            if (domains_loaded[DOMAIN_DRAM])
                freturn(pcsyms_fun[DOMAIN_DRAM].set_powercap_value, 0, LEVEL_DEVICE,
                        (uint32_t *) &values[pc_topology_info.socket_count], current_util[DOMAIN_DRAM]);
            if (domains_loaded[DOMAIN_GPU])
                freturn(pcsyms_fun[DOMAIN_GPU].set_powercap_value, 0, LEVEL_DEVICE,
                        (uint32_t *) &values[pc_topology_info.socket_count * 2], current_util[DOMAIN_GPU]);
            return;
        }
    }
    ...
    for (int32_t i = 0; i < NUM_DOMAINS; i++) {
        if (domains_loaded[i])
            freturn(pcsyms_fun[i].set_powercap_value, 0, LEVEL_DOMAIN, (uint32_t *) &manual_domain_power[i],
                    current_util[i]);
    }
}
...
void compute_power_distribution()
{
...
#if USE_GPUS
    if ((domains_loaded[DOMAIN_GPU]) && (gpu_pc_model != API_DUMMY))
        bpd_gpu = gpu_tdps[0];
#endif
    ...
    debug("W[NODE]=%.2f W[CPU]=%.2f W[GPU]=%.2f", pdomains_def[DOMAIN_NODE], pdomains_def[DOMAIN_CPU],
          pdomains_def[DOMAIN_GPUS]);
}
    ...
    for (i = 0; i < NUM_DOMAINS; i++) {
        ...
        if (i == DOMAIN_CPU || i == DOMAIN_NODE) {
            ...
            for (g = 0; g < gpu_pc_num_gpus; g++) {
                // debug("Comparing utilization for GPU %d : %lu vs
                // %lu",g,current_util[DOMAIN_GPU][g],prev_util[DOMAIN_GPU][g]);
...
state_t pmgt_init()
{
    ...
    if (current_util[DOMAIN_NODE] != NULL)
        current_util[DOMAIN_NODE][0] = 100;
    prev_util[DOMAIN_NODE] = calloc(1, sizeof(ulong));
    if (prev_util[DOMAIN_NODE] != NULL)
        prev_util[DOMAIN_NODE][0] = 100;
    ...
    for (i = 0; i < pc_topology_info.socket_count; i++)
        current_util[DOMAIN_DRAM][i] = 100;
    prev_util[DOMAIN_DRAM] = calloc(pc_topology_info.socket_count, sizeof(ulong));
    for (i = 0; i < pc_topology_info.socket_count; i++)
        prev_util[DOMAIN_DRAM][i] = 100;
    ...
    if (gpu_pc_model != API_DUMMY) {
        ...
        if (obj_path != NULL) {
            ...
            if (ret == EAR_SUCCESS)
                domains_loaded[DOMAIN_GPU] = 1;
            else
                verbose(VEARD_PC, "Error loading GPU powercap plugin");
        } else {
    ...
    if (domains_loaded[DOMAIN_GPU]) {
        ...
        for (i = 0; i < gpu_pc_num_gpus; i++)
            prev_util[DOMAIN_GPUS][i] = 0;
        debug("Static GPU utilization set to 100 for %d GPUS", gpu_pc_num_gpus);
    }
    if (domains_loaded[DOMAIN_NODE] || domains_loaded[DOMAIN_CPU] || domains_loaded[DOMAIN_DRAM] ||
        domains_loaded[DOMAIN_GPU])
        ret = EAR_SUCCESS;
    else
        ret = EAR_ERROR;
#else
    ...
    for (i = 0; i < NUM_DOMAINS; i++) {
        if (domains_loaded[i]) {
            ...
            security_range -=
                cdomain_settings[i].security_range; // security_range starts at 100 and every domain takes a bit from it
        }
    }
    compute_power_distribution(); // power distribution can only be computed AFTER we have all the domain's settings
                                  // (for now only CPU matters)
...
state_t pmgt_enable(pwr_mgt_t *phandler)
{
    ...
    int i;
    for (i = 0; i < NUM_DOMAINS; i++) {
        if (pcsyms_fun[i].set_verb_channel != NULL)
            pcsyms_fun[i].set_verb_channel(verb_channel);
    }
    for (i = 0; i < NUM_DOMAINS; i++) {
...
static void reallocate_power_between_domains()
{
    ...
    for (i = 0; i < NUM_DOMAINS; i++) {
        ...
        if (ret == EAR_ERROR)
            debug("error when setting powercap value");
    }
    debug("%s", buffer2);
}
...
void pmgt_set_power_per_domain(pwr_mgt_t *phandler, dom_power_t *pdomain, uint st)
{
    ...
    debug("[NODE-GPU %f CPU %f DRAM %f GPU %f][NODE-GPU %f CPU %f DRAM %f GPU %f]", pnode, pcpu, pdram, pgpu,
          pdomains[DOMAIN_NODE], pdomains[DOMAIN_CPU], pdomains[DOMAIN_DRAM], pdomains[DOMAIN_GPU]);
#endif
    if (my_pc_opt.last_t1_allocated < my_pc_opt.def_powercap)
        return;
    if (pdomain->platform > pmgt_limit) {
        verbose(VEARD_PC, "%s Warning! we are over the limit: Limit %u Power %u%s", COL_RED, pmgt_limit,
                pdomain->platform, COL_CLR);
        float excess = ((float) (pdomain->platform - pmgt_limit)) / (float) pmgt_limit;
        for (i = 0; i < NUM_DOMAINS; i++) {
            if (pdomains[i] > 0)
                pdomains[i] = pdomains[i] - excess * pdomains[i];
        }
        debug("reduced power domains by %.2f, new CPU %.2f new GPU %.2f", excess, pdomains[DOMAIN_CPU],
              pdomains[DOMAIN_GPU]);
        pmgt_reset_pdomain();
    ...
    for (cc = 1; cc <= max_context_created; cc++) {
        if (current_ear_app[cc] != NULL) {
            ...
            if (domain == DOMAIN_GPU) {
                ...
                for (i = 0; i < MAX_GPUS_SUPPORTED; i++) {
                    if (pmapp->pc_app_info->req_gpu_f[i] != 0)
                        f[i] = pmapp->pc_app_info->req_gpu_f[i];
                }
            }
#endif
#endif
    ...
    if (domain == DOMAIN_CPU) {
        for (i = 0; i < MAX_CPUS_SUPPORTED; i++) {
            if (use_min_cpufreq_in_idle) {
                if (!f[i])
                    f[i] = frequency_pstate_to_freq(frequency_get_num_pstates() - 1);
            } else {
                if (!f[i])
                    f[i] = pmgt_idle_def_freq;
            }
        }
    }
}
...
void pmgt_get_status(pmgt_status_t *status)
{
    ...
    for (i = 0; i < NUM_DOMAINS; i++) {
        if (pcsyms_fun[i].get_powercap_status != NULL) {
            ...
            if (ret == EAR_ERROR)
                debug("error getting domain powercap");
        }
    }

    debug("pmgt_get_status: DOMAIN_CPU: %d DOMAIN_GPU: %d", cdomain_status[DOMAIN_CPU].ok,
          cdomain_status[DOMAIN_GPU].ok);
    ...
    if (cdomain_status[DOMAIN_CPU].ok == PC_STATUS_OK && cdomain_status[DOMAIN_GPU].ok == PC_STATUS_OK) { // OK + OK
        ...
        if (cdomain_status[DOMAIN_CPU].ok == PC_STATUS_RELEASE ||
            cdomain_status[DOMAIN_GPU].ok ==
                PC_STATUS_RELEASE) { // if we are GREDDY + RELEASE, we can still rebalance so we are ok
            status->status = PC_STATUS_OK;
            ...
            status->requested =
                ear_min(status->requested, 255); // a total maximum step of 256 since it is the max value we can pass up
...
static uint pmgt_powercap_status_per_domain(uint action)
{
    ...
    debug("GPU has stress %u and CPU has %u. Status GPU %u CPU %u. ask %u ok %u rel %u",
          cdomain_status[DOMAIN_GPU].stress, cdomain_status[DOMAIN_CPU].stress, cdomain_status[DOMAIN_GPU].ok,
          cdomain_status[DOMAIN_CPU].ok, totalask, totalok, totalrel);
    ...
    if ((cdomain_status[DOMAIN_CPU].ok == PC_STATUS_GREEDY) && (cdomain_status[DOMAIN_GPU].ok == PC_STATUS_RELEASE)) {
        ...
        to         = DOMAIN_CPU;
        is_release = 1;
    } else if ((cdomain_status[DOMAIN_GPU].ok == PC_STATUS_GREEDY) &&
               (cdomain_status[DOMAIN_CPU].ok == PC_STATUS_RELEASE)) {
        from       = DOMAIN_CPU;
        to         = DOMAIN_GPU;
        is_release = 1;
    } else if ((cdomain_status[DOMAIN_GPU].ok == PC_STATUS_GREEDY) &&
               (cdomain_status[DOMAIN_CPU].ok == PC_STATUS_GREEDY)) {
        if (abs((int) cdomain_status[DOMAIN_GPU].stress - (int) cdomain_status[DOMAIN_CPU].stress) > 20) {
            ...
            cdomain_status[from].exceed =
                ear_min(cdomain_status[to].requested,
                        ear_min(cdomain_status[from].current_pc * DOMAINS_REDIST, DOMAINS_REDIST_MAX));
        } else {
            is_release = 0;
            debug("could not redistribute power, GPU has stress %u and CPU has %u", cdomain_status[DOMAIN_GPU].stress,
                  cdomain_status[DOMAIN_CPU].stress);
        }
    ...
    switch (is_release) {
        ...
        case 1:
            ...
            ret = freturn(pcsyms_fun[from].set_powercap_value, 0, LEVEL_DOMAIN, &new_power_send,
                          current_util[from]); /* Receives less power than before */
            if (ret == EAR_ERROR) {
                debug("error setting powercap value");
                break;
            }
            ret = freturn(pcsyms_fun[to].set_powercap_value, 0, LEVEL_DOMAIN, &new_power_recv,
                          current_util[to]); /* Receives more power that before */
            if (ret == EAR_ERROR)
                debug("error setting powercap value");
            break;
        case 2:
