...
unsigned long long get_nm_temp(nm_t *id, nm_data_t *nm)
{
    ...
    if ((id == NULL) || (nm == NULL) || (id->con != NM_CONNECTED) || (nm->temp == NULL))
        return 0;
    temp_total = nm->temp[0];
    for (i = 1; i < id->nsockets; i++) {
        if (temp_total < nm->temp[i])
...
int verbose_node_metrics(nm_t *id, nm_data_t *nm)
{
    ...
    sprintf(msg, " avg_cpu_freq=%.2lf, avg_imc_freq=%.2lf, temp=%lld", (double) nm->avg_cpu_freq / (double) 1000000,
            (double) nm->avg_imc_freq / (double) 1000000, nm->avg_temp);
