...
int services_mgt_imcfreq(eard_head_t *head, int req_fd, int ack_fd)
{
    ...
    switch (call) {
        ...
        case RPC_MGT_IMCFREQ_GET_CURRENT_MIN:
            serial_clean(&wb);
            //
            if (state_fail(s = mgt_imcfreq_get_current_ranged_list(no_ctx, (pstate_t *) big_chunk1,
                                                                   (pstate_t *) big_chunk2))) {
...
int is_new_job(local_connection_t *my_con, ulong jid, ulong sid)
{
    int i;
    i = 0;
    while ((i < num_local_con) && !((my_con[i].jid == jid) && (my_con[i].sid == sid)))
        i++;
    if (i < num_local_con)
        return 1;
    return 0;
...
{
    int i = 0;
    while ((i < NUM_PRIVILEGED_SERVICES) && (privileged_services[i] != service))
        i++;
    if (i < NUM_PRIVILEGED_SERVICES)
        return 1;
    else
        return 0;
}
...
void connect_service(struct daemon_req *new_req)
{
    ...
    if (AFD_SETT(ear_req_fd, &rfds_basic, "req%lu", new_req->con_id.lid) == 0) {
        error("ear_req_fd for local communication is too large (%d, max is %d), rejecting connection", ear_req_fd,
              AFD_MAX);
    }
    /* This function notifies the local connection */
    if (!eard_local_conn[newc].anonymous)
...
state_t service_close(int req_fd, int ack_fd)
{
    ...
    debug("Closing FDS (req %d, ack %d) communication (%lu, %lu)", req_fd, ack_fd, eard_local_conn[id].jid,
          eard_local_conn[id].sid);
    was_anonymous = eard_local_conn[id].anonymous;
...
state_t service_close_by_id(ulong jid, ulong sid)
{
    ...
    do {
        ...
        if (i >= 0) {
            ...
            /* Closing the fd's */
            AFD_CLR(eard_local_conn[i].fds[REQ_LOCAL_FD], &rfds_basic);
...
state_t service_listen(int req_fd, int ack_fd, struct daemon_req *req)
{
    ...
    if ((size = eards_read(req_fd, (char *) req, sizeof(eard_head_t), NO_WAIT_DATA)) != sizeof(eard_head_t)) {
        error("when reading the RPC header %ld: expected %lu bytes and received %lu FD=%d", req->req_service,
              sizeof(eard_head_t), size, req_fd);
        return EAR_ERROR;
    }
...
state_t eard_local_api()
{
    ...
    if (values_rapl == NULL)
        error("values_rapl returns NULL in eard initialization");
     ...
     *
     */
    while (((numfds_ready = aselectv(&rfds_basic, NULL)) >= 0) || ((numfds_ready < 0) && (errno == EINTR))) {
