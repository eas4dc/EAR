...
#define earl_early_verb(lvl, msg, ...)                                                                                 \
    do {                                                                                                               \
...
/* Holds the configuration of the eard */
static eard_state_t eard_conf;
...
static node_sharing_strategy_t node_sharing_strategy = node;
...
static state_t check_eard_earl_compatibility(state_t eard_connect_state, eard_state_t *eard_conf)
{
    ...
    if (state_ok(st = eards_get_state(eard_conf))) {
        if (eard_conf->version.major != VERSION_MAJOR) {
            verbose_warning_master("Major versions differ (EARD %u | EARL %u). Future versions shall consider this "
                                   "difference as an incompatibility.",
                                   eard_conf->version.major, VERSION_MAJOR);
        }
...
#endif
        verbose(1, "%30s: '%s'/'%s'", "App/user id", application.job.app_id, application.job.user_id);
        verbose(1, "%30s: '%s'/'%lu'/'%lu'/'%u'", "Node/job id/step_id/app_id", application.node_id, application.job.id,
                application.job.step_id, AID);
        // verbose(2, "App/loop summary file: '%s'/'%s'", app_summary_path, loop_summary_path);
        verbose(1, "%30s: %u/%lu (%d)", "P_STATE/frequency (turbo)", EAR_default_pstate, application.job.def_f,
                ear_use_turbo);
        verbose(1, "%30s: %u/%d/%d/%.3f", "Tasks/nodes/ppn/node_ratio", ear_my_size, num_nodes, ppnode, ratio_PPN);
        verbose(1, "%30s: %s (%d)", "Policy (learning)", application.job.policy, ear_whole_app);
        verbose(1, "%30s: %0.2lf/%0.2lf", "Policy threshold/Perf accuracy", application.job.th,
                get_ear_performance_accuracy());
        verbose(1, "%30s: %d/%d/%d", "DynAIS levels/window/AVX512", get_ear_dynais_levels(),
                get_ear_dynais_window_size(), dynais_build_type());
        verbose(1, "%30s: %s", "VAR path", get_ear_tmp());
        verbose(1, "%30s: %u", "EAR privileged user", system_conf->user_type == AUTHORIZED);
        verbose(1, "%30s: %d ", "EARL on", lib_shared_region->earl_on);
        verbose(1, "%30s: %lu", "Max CPU freq", system_conf->max_freq);
        verbose(1, "%30s: %s", "TAG", system_conf->tag);
        verbose(1, "-------------------------------------------------------");
    }
}

/* notifies mpi process has succesfully connected with EARD */
/*********************** This function synchronizes all the node masters to guarantee all the nodes have connected with
 * EARD, otherwise, EARL is set to OFF **************/
...
#if MPI
        verbose_master(2, "[%s] Synchronizing with other masters. Status: %d. Waiting for %d masters", node_name,
                       status, masters_info.my_master_size);
        masters_connected = 0;
        ...
        if (masters_connected != masters_info.my_master_size) {
            ...
            if (masters_info.my_master_rank >= 0) {
                verbose_master(1, "%s:some EARD is not running (connected nodes %d expected %d)\n", node_name,
                               masters_connected, masters_info.my_master_size);
            }
            eard_ok = 0;
            return;
        } else {
...
void cleanup_job_folder()
{
    ...
    if (clean_job_folder) {
        ...
        debug("Folder doesn't need to be removed from the library\n");
    }
}
...
state_t create_eid_folder(char *tmp, int jid, int sid, uint AID)
{
    ...
    if ((ret < 0) && (errno != EEXIST)) {
        verbose(WF_SUPPORT_VERB, "EAR error AID folder cannot be created (%s) (%s)", EID_application_path_finish,
                strerror(errno));
        node_mgr_info_unlock();
        return EAR_ERROR;
    }
...
/****************************************************************************************************************************************************************/
/****************** This function creates the shared memory region to coordinate processes in same node and with other
 * nodes  ***********************************/
...
void create_shared_regions()
{
    ...
    verbose(VPROC_INIT, "[%s] Master (PID: %d) creating shared regions for node synchronisation...", node_name,
            master_pid);
...
#endif
    verbose(VPROC_INIT, "EARL[%d][%d] My node ID is %d/%d", ear_my_rank, getpid(), my_node_id,
    ...
    verbose_master(2, "[%s] Master (PID: %d) Shared region for shared signatures created for %d processes", node_name,
                   master_pid, lib_shared_region->num_processes);
...
#if MPI
    if (share_global_info(masters_info.masters_comm, (char *) &lib_shared_region->num_processes, sizeof(int),
                          (char *) masters_info.ppn, sizeof(int)) != EAR_SUCCESS) {
    ...
    for (i = 1; i < masters_info.my_master_size; i++) {
        if (masters_info.ppn[i] > masters_info.max_ppn)
            masters_info.max_ppn = masters_info.ppn[i];
        if (masters_info.my_master_rank == 0)
            verbose_master(2, "[%s] Master (PID: %d) Processes in node %d = %d", node_name, master_pid, i,
                           masters_info.ppn[i]);
    }

    verbose_master(2, "[%s] Master (PID: %d) max number of ppn is %d", node_name, master_pid, masters_info.max_ppn);
    /* For scalability concerns, we can compile the system sharing all the processes information
     * (SHARE_INFO_PER_PROCESS) or only 1 per node (SHARE_INFO_PER_NODE)*/
    ...
    verbose_master(2, "ratio_PPN %f = procs node %f max %f", ratio_PPN, (float) lib_shared_region->num_processes,
                   (float) masters_info.max_ppn);
    ...
    if (masters_info.nodes_info == NULL) {
        ...
        debug("%d Bytes (%d x %lu)  allocated for masters_info node_info", total_size, total_elements,
              sizeof(shsignature_t));
    }
...
void attach_shared_regions()
{
...
#endif
        do {
            verbose(VPROC_INIT, "EARL[%d][%d] attaching to libsh path %s", ear_my_rank, getpid(),
                    lib_shared_region_path);
    ...
    if (get_shared_signatures_path(tmp, ID, AID, shsignature_region_path) != EAR_SUCCESS) {
        ...
        verbose(VPROC_INIT, "EARL[%d ][%d] attaching to shared regions %s", ear_my_rank, getpid(),
                shsignature_region_path);
        sig_shared_region =
            attach_shared_signatures_area(shsignature_region_path, lib_shared_region->num_processes, &fd_sigsh);
        if (sig_shared_region == NULL) {
    ...
    // if (node_mgr_index != NULL_NODE_MGR_INDEX) node_mgr_job_info =
...
/****************************************************************************************************************************************************************/
/******************  This data is shared with EARD, it's per-application
 * info************************************************************************************/
/****************************************************************************************************************************************************************/
...
/****************************************************************************************************************************************************************/
/* Connects the mpi process to a new communicator composed by masters, only processes with master=1 belongs to the new
 * communicator */
/****************************************************************************************************************************************************************/
...
void attach_to_master_set(int master, uint can_synch)
{
    ...
    if (can_synch) {
        ...
        if (PMPI_Comm_split(masters_info.new_world_comm, color, masters_info.my_master_rank,
                            &masters_info.masters_comm) != MPI_SUCCESS) {
    ...
    if ((masters_comm_created) && (!color)) {
        ...
        debug("New master communicator created with %d masters. My master rank %d\n", masters_info.my_master_size,
              masters_info.my_master_rank);
    }
    verbose(VPROC_INIT, "EARL[%d][%d] Attach to master set end, rank %d size %d", ear_my_rank, getpid(),
            masters_info.my_master_rank, masters_info.my_master_size);
}
...
static int get_local_id(char *node_name)
{
    ...
    if (app_node_multiprocess) {
        ...
        if ((fd_master_lock = ear_file_lock_master(master_lock_filename)) < 0) {
            if (errno != EBUSY)
                debug("Error creating lock file %s", strerror(errno));
            lid = 1;
        } else {
            lid = 0;
        }
...
static void get_job_identification()
{
    ...
    if (account_id == NULL)
        strcpy(my_account, NULL_ACCOUNT);
    else
        strcpy(my_account, account_id);
...
void check_large_job_use_case()
{
    ...
    if (app_mgt_info->nodes > LIMIT_LARGE_JOBS) {
        debug("We ar a large job and we will start at a lower freq: Size %u limit %u", app_mgt_info->nodes,
              LIMIT_LARGE_JOBS);
    }
}
...
void ear_lib_sig_handler(int s)
{
    ...
    if (s == SIGCHLD) {
        ...
        while ((pid = waitpid(-1, &ret, WNOHANG)) > 0) {
            if (WIFEXITED(ret)) {
                verbose(VPROC_MGT, "Child %d finish with exitcode %d", pid, WEXITSTATUS(ret));
            } else {
                verbose(VPROC_MGT, "Child %d finish with signal %d", pid, WTERMSIG(ret));
            }
        }
        return;
    }
    ...
    if ((s == SIGBUS) || (s == SIGSEGV)) {
        ...
        if (ear_getenv(FLAG_EAR_DEBUG) != NULL)
    ...
    if (cancelled || sig_shared_region[my_node_id].exited)
        return;
    if (syscall(SYS_gettid) != main_pid)
        return;
...
static int get_ear_application_id_multiprocess(int app_multi)
{
    ...
    if (app_multi) {
        ...
        if (ear_my_rank == 0) {
            ...
            earl_early_verb(VJOB_INIT, "Rank %d generating AID file %s. Writing PID %d", ear_my_rank, AID_number_file,
                            master_pid);
            ...
            state_t st;
            do {
                st = ear_file_read(AID_number_file, (char *) &master_pid, sizeof(master_pid), 1);
                if (st != EAR_SUCCESS) {
                    usleep(100);
                    tries++;
                }
...
static int get_ear_application_id(uint can_synch)
{
    int master_pid;
#if MPI
    verbose(2, "EARL[%d] can synchronize %d", getpid(), can_synch);
    if (can_synch) {
        master_pid = getpid();
        if (PMPI_Bcast(&master_pid, 1, MPI_INT, 0, MPI_COMM_WORLD) != MPI_SUCCESS) {
            verbose(2, "EARl[%d] MPI_Bcast failed", getpid());
        }
        verbose(2, "EARL[%d][rank %d/%d] APP ID from MPI %d", getpid(), ear_my_rank, ear_my_size, master_pid);
    } else {
        ...
        verbose(2, "APP ID from MPI multiprocess %d", master_pid);
    }
#else
    master_pid = get_ear_application_id_multiprocess(app_node_multiprocess);
#endif
...
static int get_loop_strategy()
{
    char *mode = ear_getenv(FLAG_LOOP_STRATEGY);
    if (!mode)
        return ear_guided;
    strtoup(mode);
    verbose_master(2, "EARL: Loop detection mode %s", mode);
    return (strcmp(mode, "TIME") == 0 ? TIME_GUIDED : DYNAIS_GUIDED);
}
...
void ear_init()
{
    ...
    if (cfake_eard_error != NULL)
        fake_eard_error = atoi(cfake_eard_error);
    if (cfake_force_shared_node != NULL)
        fake_force_shared_node = atoi(cfake_force_shared_node);
    ...
    if (ear_getenv(FLAG_NTASK_WORK_SHARING) != NULL) {
        ...
        if (ear_my_size > 1)
            app_node_multiprocess = 1;
        if (app_node_multiprocess) {
            earl_early_verb(1, "Node multiprocess enabled. Rank: %d Tasks: %d", ear_my_rank, ear_my_size);
        }
    } else {
        ear_my_rank = 0;
        ear_my_size = 1;
        if (get_my_sched_node_rank() > 0)
            first_application = 0;
    }
#endif
    ...
    if (folder_open(&tmp_test, tmp) != EAR_SUCCESS) {
        ...
        earl_early_verb(1,
                        "%sWarning%s EAR_TMP directory (%s) does not exist in the node. Setting /tmp as the EAR_TMP "
    ...
    if (state_fail(tmp_exists)) {
        earl_early_verb(1, "%sWarning%s Job-Step directory doesn't exist. Node multiprocess disabled.", COL_YLW,
                        COL_CLR);
    ...
    if (ext_def_freq_str) {
        ...
        earl_early_verb(VEAR_INIT, "%sWarning%s Externally defined default freq (kHz): %lu", COL_YLW, COL_CLR,
                        ext_def_freq);
    }
    ...
    if (state_ok(tmp_exists)) {
        ...
        if (is_already_connected(get_ear_tmp(), my_job_id, my_step_id, AID, my_pid)) {
            earl_early_verb(1,
                            "%sWarning%s (Job/step/application) (%d/%d/%d) seems to be already conncted. Did the "
                            "process mutate? Recovering EARD connection info...",
    ...
    earl_early_verb(verb_pid_lvl, "--- EARL INIT [PID: %d AID %d] [MASTER: %d] ---", my_pid, AID, i_am_master);
    ...
    earl_early_verb(VPROC_INIT, "Rank %d attached to master set. Synch %d MP %d.", ear_my_rank, synchronize_masters,
                    app_node_multiprocess);
    ...
    if ((synchronize_masters == 0) && (!app_node_multiprocess)) {
        verbose(1, "Rank %d: Masters can't synchronize and not a multiprocess app. Initialization aborted.",
                ear_my_rank);
        return;
    }
    ...
    if (fake_eard_error)
        verbose_master(0, "Warning, executing fake eard errors: %u", fake_eard_error);

    earl_early_verb(VPROC_INIT, "Rank %d testing valid ID %u (settings %p resched %p)", ear_my_rank,
                    create_ID(my_job_id, my_step_id), system_conf, resched_conf);
    ...
    if (ear_getenv("LOGNAME") != NULL)
        strcpy(application.job.user_id, ear_getenv("LOGNAME"));
    else
        strcpy(application.job.user_id, "No userid");
    ...
    verbose(VPROC_INIT - 1, "EARL[%d][%d] EARD connection section EARD=%d MASTER=%u", ear_my_rank, getpid(), eard_ok,
            my_id == 0);
    if (eard_ok) { // all processes will connect with eard
        ...
        verbose(VPROC_INIT - 1, "%sProcess %d PID %d Connecting with EAR Daemon (EARD) %d node %s status=%s %s", col,
                my_node_id, my_pid, ear_my_rank, node_name, ((ret == EAR_SUCCESS) ? "Connected" : "Not Connected"),
                COL_CLR);

        state_t eard_comp_state = check_eard_earl_compatibility(ret, &eard_conf);
        ...
        if (lib_supports_gpu != eard_supports_gpu) {
            verbose(1,
                    "%sERROR%s EARL (%u) and EARD (%u) differ on GPU support. Please, inform to your system admin "
                    "about this issue.",
        ...
        if ((ret == EAR_SUCCESS) && (eard_comp_state == EAR_SUCCESS)) {
            ...
            verbose(VPROC_INIT - 1, "%sRank %d NOT connected with EARD%s %s", COL_BLU, ear_my_rank, COL_CLR, state_msg);
            eard_ok = 0;
            if (!my_id)
                notify_eard_connection(0);
        }
    ...
    verbose(VPROC_INIT, "EARL[%d][rank = %d]  Created/Attached shared regions Total procs %d", getpid(), ear_my_rank,
            lib_shared_region->num_processes);
    verbose_shared_data();
    ...
    if (!eard_ok) {
        ...
        if (!my_id && (fd_dummy >= 0)) {
            ...
            if (err)
                verbose_master(2, "Error cleaning dummy area %s (%s)", dummy_areas_path, strerror(err));
        }
    ...
    if (system_conf->user_type != AUTHORIZED)
        VERB_SET_LV(ear_min(verb_level, 1));
...
#endif
    {
        ...
        if (proc_mask_cpu_count > 0) {
            ...
            verbose(VPROC_INIT, "%sWARNING%s CPU mask for thread ID %d has no CPUs.", COL_YLW, COL_CLR, my_pid);
        }

        sig_shared_region[my_node_id].affinity = ear_affinity_is_set;
    } else {
        verbose(1, "%sERROR%s Getting affinity mask for thread ID %d: %s", COL_RED, COL_CLR, my_pid, strerror(errno));
    }
    ...
    if (masters_info.my_master_rank >= 0) {
        ...
        while (!are_signatures_ready(lib_shared_region, sig_shared_region, NULL))
            usleep(100);
    ...
    if (state_ok(init_power_policy(system_conf, resched_conf))) {
        ...
        verbose_master(VEAR_INIT, "%sWARNING%s An error ocurred initializing the power policy.", COL_YLW, COL_CLR);
    }

    char *cnode_share_strategy = ear_getenv(FLAG_NODE_SHARING_STRATEGY);
    if (cnode_share_strategy) {
        verbosen_info_master("Node sharing strategy define %s", cnode_share_strategy);
        if (strcmp(cnode_share_strategy, "job") == 0) {
            node_sharing_strategy = job;
        } else if (strcmp(cnode_share_strategy, "node") == 0) {
            node_sharing_strategy = node;
        }
    }
    ...
    if (cpu_power_model_init(node_sharing_strategy) == EAR_SUCCESS) {
    ...
    utils_create_plugin_path(my_plug_path, system_conf->installation.dir_plug, ear_getenv(HACK_EARL_INSTALL_PATH),
                             system_conf->user_type);
    ...
    if (summary_pathname == NULL) {
        ...
        xsnprintf(plugins_ext, sizeof(plugins_ext), "%s:csv_ts.so", system_conf->lib_info.plugins);
    ...
    if (creport_earl_events != NULL)
        report_earl_events = atoi(creport_earl_events);
    if (report_earl_events)
        verbose_master(1, " EARL events reported to DB %s", (report_earl_events ? "enabled" : "disabled"));
    ...
    // Tracing init
    traces_init(system_conf, application.job.app_id, ear_my_rank, my_node_id, num_nodes, ear_my_size, ppnode);
...
#if !EAR_OFF
    /* Monitor is used for applications with no mpi of few mpi calss per second. Gets control of signature
     * computation if neded*/

    if (!module_mpi_is_enabled()) {
        ear_guided = TIME_GUIDED;
    }
    verbose_master(2, "EARL[%d] LOOP strategy %s", getpid(), (ear_guided == TIME_GUIDED) ? "Time" : "Dynais");
#if 0
        if (ear_guided == TIME_GUIDED){
#endif
    if (monitor_init() != EAR_SUCCESS)
        verbose(0, "Monitor init fails! %s", state_msg);
    ...
    if (monitor_register(earl_monitor) != EAR_SUCCESS)
        verbose(0, "Monitor register fails! %s", state_msg);
    ...
    verbose_master(EARL_GUIDED_LVL, "Initialization cost: %llu usec dynais_guided %u ", elap_eard_init,
                   ear_guided != TIME_GUIDED);
#endif
...
static void print_resource_usage()
{
  ...
  verbose(0,"%s: User cpu time %llu system cpu time %llu page faults %ld block input %ld block output %ld voluntary ctx
%ld involuntary ctx %ld", node_name, my_usage.ru_utime.tv_sec*1000+my_usage.ru_utime.tv_usec/1000,
my_usage.ru_stime.tv_sec*1000+my_usage.ru_stime.tv_usec/1000, my_usage.ru_majflt, my_usage.ru_inblock,
my_usage.ru_oublock, my_usage.ru_nvcsw, my_usage.ru_nivcsw);
...
void ear_finalize(int exit_status)
{
    ...
    if (state_fail(st)) {
        verbose(2, "%sEARL[%d][%d] at %s[WARNING]%s ear_finalize without lock, continuing to avoid app block", COL_YLW,
                ear_my_rank, getpid(), node_name, COL_CLR);
    }
    exiting = 1;
    if (state_ok(st)) {
    ...
    if (fake_error)
        verbose_master(0, "Warning, executing fake errors in ear_finalize : %u", fake_error);
    ...
    metrics_dispose(&application.signature, get_total_resources(), (node_sharing_strategy == node) ? 0 : 1);
    sig_shared_region[my_node_id].exited = 1;
    ...
    if (!my_id) {
        if (application.is_learning)
            verbose_master(1, "Application in learning phase mode");
        ...
        verbose(2, "EARL[%d][%d] Application report (type %s)", ear_my_rank, getpid(),
                (first_application ? "Regular App" : "WF App"));
    ...
    timestamp_getfast(&end_eard_init);
    elap_eard_init = timestamp_diff(&end_eard_init, &start_eard_init, TIME_USECS);
    verbose_master(EARL_GUIDED_LVL,
                   "Finalization cost (%s): %llu usec dynais calls %lu dynais_enabled %s  periodic mode %s guided by "
                   "%s mpi_with_stats %llu total mpi %llu",
                   node_name, elap_eard_init, dynais_calls, (dynais_enabled == DYNAIS_ENABLED) ? "ON" : "OFF",
                   (ear_periodic_mode == PERIODIC_MODE_ON) ? "ON" : "OFF",
                   (ear_guided == TIME_GUIDED) ? "TIME" : "Dynais", total_mpi_call_statistics(), total_mpi_calls);
#endif
    if (exit_status != EAR_SUCCESS)
        exit(exit_status);
}
...
static void _go_to_time_guided(int new_ear_guided)
{
...
void ear_mpi_call(mpi_call call_type, p2i buf, p2i dest)
{
    ...
    if (!ear_whole_app) {
        ...
        if (masters_info.my_master_rank >= 0) {
            traces_mpi_call(ear_my_rank, my_id, (ulong) ear_event_l, (ulong) buf, (ulong) dest, (ulong) call_type);
        }
        ...
        switch (ear_periodic_mode) {
            case PERIODIC_MODE_OFF: {
                switch (dynais_enabled) {
                    case DYNAIS_ENABLED: {
// DYNAIS_ENABLED is only enabled for MPI applications
#if MPI
                        ...
                        if (check_periodic_mode == 0) {
                            ...
                            /* Check here if we must move to periodic mode, do it every N mpicalls to reduce the
                             * overhead */
                            if ((total_mpi_calls % check_every) == 0) {
                                /* We add a minimum number of mpi calls to allow the application to start a "normal"
                                 * computation */
                                ...
                                /* In that case, the maximum time without signature has been passed, go to set
                                 * ear_periodic_mode ON */
                                if (limit_exceeded || time_from_mpi_init >= dynais_timeout) {
                                    if (limit_exceeded) {
                                        ...
                                        verbose_master(
                                            EARL_GUIDED_LVL,
                                            "Going to periodic mode after %lf secs: mpi calls in period %u.\n",
                                        ...
                                        _go_to_time_guided(TIME_GUIDED);
            ...
            case PERIODIC_MODE_ON:
                ...
                if ((mpi_calls_per_second > 0) && (total_mpi_calls % mpi_calls_per_second) == 0) {
                    ...
                    if (!limit_exceeded && (avg_mpi_calls_per_second() >= MAX_MPI_CALLS_SECOND))
                        limit_exceeded = 1;
#endif
                }
                break;
    ...
    if (!ear_whole_app) {
        ...
        switch (ear_status) {
            ...
            case NEW_ITERATION:
                ...
                states_new_iteration(my_id, ear_loop_size, ear_iterations, ear_loop_level, ear_event_l,
                                     mpi_calls_per_loop, 1);
                mpi_calls_per_loop = 1;
                break;
            case END_LOOP:
...
void ear_mpi_call_dynais_off(mpi_call call_type, p2i buf, p2i dest)
{
    ...
    if (!ear_whole_app) {
        ...
        if (masters_info.my_master_rank >= 0)
            traces_mpi_call(ear_my_rank, my_id, (unsigned long) buf, (unsigned long) dest, (unsigned long) call_type,
                            (unsigned long) ear_event_l);
        ...
        switch (ear_status) {
            ...
            case NEW_ITERATION:
                ...
                traces_new_n_iter(ear_my_rank, my_node_id, ear_event_l, ear_loop_size, ear_iterations);
                states_new_iteration(my_id, ear_loop_size, ear_iterations, (uint) ear_level, ear_event_l,
                                     mpi_calls_per_loop, 1);
                mpi_calls_per_loop = 1;
                break;
            default:
        ...
        switch (ear_status) {
            case IN_LOOP:
                if (loop_id == manual_loopid) {
                    ...
                    debug("Error END_LOOP and current loop id %lu different from active one %lu", manual_loopid,
                          loop_id);
                    ear_status = NO_LOOP;
                }
                break;
            case NO_LOOP:
                debug("Error, application was not in a loop");
                break;
        }
    }
}
...
state_t earl_periodic_actions_init(void *no_arg)
{
    ...
    if (!module_mpi_is_enabled() || (ear_guided == TIME_GUIDED)) {
        ...
        mpi_calls_in_period = lib_period;
        if (!ear_whole_app)
            states_new_iteration(my_id, 1, ear_iterations, 1, 1, mpi_calls_in_period, 0);
    }
...
state_t earl_periodic_actions(void *no_arg)
{
    ...
    if (!module_mpi_is_enabled() || (ear_guided == TIME_GUIDED)) {
        // if (ear_guided == TIME_GUIDED) verbose_master(1, "(Time guided) elapsed %lu iteration iters %d period %u",
        // period_elapsed, ear_iterations, lib_period);
    ...
    if ((seconds >= 5) && (ear_periodic_mode != PERIODIC_MODE_ON)) {
        ...
        /* total_mpi_calls is computed per process in this file, not in metrics. It is used to automatically switch to
         * time_guided */
        ...
        verbose_master(EARL_GUIDED_LVL, "%d: Phase time_guided %d dynais_guided %d period %u", my_node_id,
                       new_ear_guided == TIME_GUIDED, new_ear_guided == DYNAIS_GUIDED, lib_period);
#if MPI
        if (new_ear_guided != DYNAIS_GUIDED) {
            _go_to_time_guided(new_ear_guided);
        }
#endif
        ear_guided = new_ear_guided;
    }
...
static state_t earl_log_backup(char *pathname, size_t pathname_size)
{
    ...
    st = ear_file_read(pathname, file_buff, (size_t) file_size, 0);
    if (state_fail(st)) {
    ...
    if (verb_path) {
        ...
        if (masters_info.my_master_rank >= 0 || per_proc_verb_file) {
            if ((mkdir(verb_path, S_IRWXU | S_IRGRP | S_IWGRP) < 0) && (errno != EEXIST)) {
                ...
                error("MR[%d] Verbose files folder cannot be created (%s)", masters_info.my_master_rank,
                      strerror(errno));
            } else {
                using_verb_files = 1;
                ...
                snprintf(file_name, sizeof(file_name) - 1, "%s/earl-%d-%d-%u-%s-%d.log", verb_path, my_job_id,
                         my_step_id, AID, node_name, main_pid);
