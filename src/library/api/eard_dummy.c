...
static void create_dummy_node_conf(cluster_conf_t *dummycc, char *nodename)
{
    ...
    snprintf(island->ranges[island->num_ranges].r_def.prefix, sizeof(island->ranges[island->num_ranges].r_def.prefix),
             "%.*s", (int) sizeof(island->ranges[island->num_ranges].r_def.prefix) - 1, nodename);
    island->num_ranges++;
}
...
state_t eard_dummy_cluster_conf(char *ear_tmp, uint ID)
{
    ...
    if (dummy_node_conf->energy_plugin != dummy_cc.install.obj_ener) {
        strcpy(dummy_cc.install.obj_ener, dummy_node_conf->energy_plugin);
    }
...
state_t eard_dummy_frequency_list(char *ear_tmp, uint ID)
{
    ...
    if (stat(path, &buffer)) {
        ...
        dummy_frequencies = attach_shared_area(path, size, O_RDONLY, &freqs_fd, NULL);
    }
...
state_t create_eard_dummy_shared_regions(char *ear_tmp, uint ID)
{
    ...
    verbose_master(2, "All the dummy regions ready create in tmp=%s", ear_tmp);
    return EAR_SUCCESS;
}
