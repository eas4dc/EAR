...
void create_dummy_path_lock(char *ear_tmp, uint ID, char *dummy_areas_path,
			    uint size)
{
	snprintf(dummy_areas_path, size, "%s/.dummy_areas_lock.%u", ear_tmp,
		 ID);
...
static void create_dummy_node_conf(cluster_conf_t * dummycc, char *nodename)
{
	...
	island->ranges =
	    realloc(island->ranges,
		    sizeof(node_range_t) * (island->num_ranges + 1));
	...
	island->ranges[island->num_ranges].start =
	    island->ranges[island->num_ranges].end = -1;
	island->ranges[island->num_ranges].db_ip =
	    island->ranges[island->num_ranges].sec_ip = 0;
	island->num_ranges++;
}
...
static void create_dummy_conf_paths(cluster_conf_t * dummycc, char *ear_tmp)
{
	...
	if (install != NULL)
		strcpy(dummycc->install.dir_inst, install);
	else
		strcpy(dummycc->install.dir_inst, DEFAULT_EAR_INSTALL_PATH);
	if (etc != NULL)
		strcpy(dummycc->install.dir_conf, etc);
	else
		strcpy(dummycc->install.dir_conf, DEFAULT_EAR_ETC_PATH);
	strcpy(dummycc->install.dir_plug, dummycc->install.dir_inst);
	strcat(dummycc->install.dir_plug, "/lib/plugins");
	strcpy(dummycc->install.obj_power_model, DEFAULT_ENERGY_MODEL);
...
state_t eard_dummy_cluster_conf(char *ear_tmp, uint ID)
{
	...
	if (gethostname(nodename, sizeof(nodename)) < 0) {
		verbose_master(0, "Error getting node name (%s)",
			       strerror(errno));
	}
	strtok(nodename, ".");
	...
	if (check_cc) {
		...
		verbose_master(0,
			       " FAKE_EAR_NOT_INSTALLED when creating cluster conf");
		cc_read_result = EAR_ERROR;
	}

	if (cc_read_result != EAR_ERROR) {
		verbose_master(2, "Reading cluster_conf");
		uint vlevel = VERB_GET_LV();
		VERB_SET_LV(0);
		// Change th value or this var to enable/disable some verbosity
		VCCONF = 3;
	...
	check_null(dummy_node_conf, "Invalid configuration file for node %s",
		   nodename);
	if (dummy_node_conf != NULL) {
		if (VERB_GET_LV() >= 2)
			print_my_node_conf(dummy_node_conf);
	}
	// Updating the energy plugin with the one in the nod_conf
	if (dummy_node_conf->energy_plugin != dummy_cc.install.obj_ener) {
		strcpy(dummy_cc.install.obj_ener,
		       dummy_node_conf->energy_plugin);
	}
	...
	local_dummy_ser_cc =
	    create_ser_cluster_conf_shared_area(path, dummy_ser_cc,
						dummy_ser_cc_size);

	check_null(local_dummy_ser_cc,
		   "Error serializing cluster_conf in path %s", " ");
	//verbose_master(2, "Dummy cluster_conf done %s", path);
...
state_t eard_dummy_earl_settings(char *ear_tmp, uint ID)
{
	...
	cpufreq_base_init(NULL, &bf);
	freq_base = (ulong) bf.frequency;

	//verbose_master(1,"Using freq_base %lu", freq_base);

	//verbose_master(2, "looking for policy %d", dummy_cc.default_policy);  
	my_policy =
	    get_my_policy_conf(dummy_node_conf, dummy_cc.default_policy);
	check_null(my_policy, "My policy returns NULL for policy %d",
		   dummy_cc.default_policy);
	...
	memcpy(dummy_sett->settings, my_policy->settings,
	...
	memcpy(&dummy_sett->installation, &dummy_cc.install,
	       sizeof(conf_install_t));
	...
	//verbose_master(2, "Dummy resched are done %s", path);
...
state_t eard_dummy_coefficients(char *ear_tmp, uint ID)
{
	...
	check_null(dummy_coeff, "Error creating coefficients for path %s",
		   path);

	//verbose_master(2, "Dummy coefficients area done %s", path);
	return EAR_SUCCESS;
}
...
state_t eard_dummy_frequency_list(char *ear_tmp, uint ID)
{
	...
	if (stat(path, &buffer)) {
		dummy_frequencies =
		    create_frequencies_shared_area(path, frequencies, size);
	} else {
		dummy_frequencies =
		    attach_shared_area(path, size, O_RDONLY, &freqs_fd, NULL);
	}
	check_null(dummy_frequencies,
		   "Error creating frequencies area at path %s", path);
...
void eard_dummy_replace_base_freq(ulong base)
{
	...
	if ((dummy_frequencies == NULL) || (dummy_sett == NULL)) {
		verbose_master(2, "Error, dummy_frequencies %p dummy_sett %p",
			       dummy_frequencies, dummy_sett);
		return;
	}
...
{
	verbose(2, "Creating dummy areas in %s and ID %u . SYSTEM_TYPE %u",
		ear_tmp, ID, SYSTEM_TYPE);
	earl_config_dummy = 1;
	...
	verbose_master(2, "All the dummy regions ready create in tmp=%s",
		       ear_tmp);
	return EAR_SUCCESS;
}
