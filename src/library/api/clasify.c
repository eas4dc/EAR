...
#define flops_per_sig(s)                                                                                               \
    (s->FLOPS[0] + s->FLOPS[1] + s->FLOPS[2] + s->FLOPS[3] + s->FLOPS[4] + s->FLOPS[5] + s->FLOPS[6] + s->FLOPS[7])
...
char *phase_to_str(uint phase)
{
    switch (phase) {
        ...
        case APP_IO_BOUND:
            return "IO_BOUND";
        case APP_BUSY_WAITING:
            return "CPU_BUSY_WAITING";
        case APP_CPU_GPU:
            return "CPU_GPU_COMP";
        case APP_COMP_CPU:
            return "CPU_BOUND";
...
{
    verbose_master(vl, "CPU bound (CPI  < %.2lf GBS < %.2lf) MEM bound (CPI > %2.lf GBS > %.2lf)", CPI_CPU_BOUND,
                   GBS_CPU_BOUND, CPI_MEM_BOUND, GBS_MEM_BOUND);
}
...
state_t classify_init(topology_t *tp_in, settings_conf_t *libconf)
{
    ...
    if (load_medoids(path, libconf->tag, medoids) == EAR_SUCCESS &&
        load_extremes(path, libconf->tag, extremes) == EAR_SUCCESS) {
        ...
        verbose_info_master(
            "K-medoids & roofline strategies not available. Loading default model for the classification");
        switch (tp_in->vendor) {
            case VENDOR_INTEL:
                /* Before Sylake */
...
state_t must_switch_to_time_guide(ulong last_sig_elapsed, uint *ear_guided)
{
    ...
    if (is_low_mpi_activity()) {
        ...
        if (first_warning_mpi_th) {
            verbose_master(WARN_CLASSIFY,
                           "Warning, going to time guided because low number of MPI calls per second current %f ",
                           avg_mpi_calls_per_second());
            first_warning_mpi_th = 0;
        }
        *ear_guided = TIME_GUIDED;
    }
    if (signature_reported && (last_sig_elapsed > ((5 * perf_accuracy_min_time) / 1000000))) {
        verbose_master(WARN_CLASSIFY, "Warning, going to time guided because elapsed time since last signature is %lu ",
                       last_sig_elapsed);
        *ear_guided = TIME_GUIDED;
    }
    return EAR_SUCCESS;
    ...
    *busy = USEFUL_COMP;
    ...
    if (VERB_ON(INFO_CLASSIFY)) {
        ...
        if (c1 || c2 || c3) {
            ...
            char gflops_msg[32];
            snprintf(gflops_msg, sizeof gflops_msg, "GFLOP/s class=%u %.3lf/%.3lf)", c3, sig->Gflops,
                     GFLOPS_BUSY_WAITING);

            char busy_waiting_msg[64];
            snprintf(busy_waiting_msg, sizeof busy_waiting_msg, "App node workload declared as busy waiting: %u",
                     *busy);
            ...
            verbose_master(INFO_CLASSIFY, "%*s%s\n%*s%s\n%*s%s\n%*s%s\n%s\n", (int) (max_strlen - strlen(hdr_msg)) / 2,
                           " ", hdr_msg, (int) (max_strlen - strlen(cpi_msg)) / 2, " ", cpi_msg,
                           (int) (max_strlen - strlen(gbs_msg)) / 2, " ", gbs_msg,
                           (int) (max_strlen - strlen(gflops_msg)) / 2, " ", gflops_msg, busy_waiting_msg);
        }
    }
...
state_t default_is_cpu_bound(signature_t *sig, uint num_cpus, uint *cbound)
{
    *cbound = 0;
    if ((sig->CPI < CPI_CPU_BOUND) && (sig->GBS < GBS_CPU_BOUND))
        *cbound = 1;
    // verbose_master(INFO_CLASSIFY, "CPI %lf thresh %lf GB/s %lf thresh %lf", sig->CPI, CPI_CPU_BOUND, sig->GBS,
    // GBS_CPU_BOUND);
    return EAR_SUCCESS;
}
