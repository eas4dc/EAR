		       ...
		       MPI_Datatype recvtype, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Allgather(sendbuf, sendcount, sendtype, recvbuf,
			...
			MPI_Datatype recvtype, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Allgatherv(sendbuf, sendcount, sendtype, recvbuf,
		       ...
		       MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Allreduce(sendbuf, recvbuf, count, datatype, op, comm);
	debug("<< C Allreduce...............");
	after_mpi(Allreduce);
	return res;
}
		      ...
		      MPI_Datatype recvtype, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount,
			       recvtype, comm);
	debug("<< C Alltoall...............");
	after_mpi(Alltoall);
	return res;
		       ...
		       MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Alltoallv(sendbuf, sendcounts, sdispls, sendtype, recvbuf,
			...
			int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Bsend_init(buf, count, datatype, dest, tag, comm,
				 request);
	debug("<< C Bsend_init...............");
			 ...
			 MPI_Comm * comm_cart)
{
	...
	int res =
	    next_mpic.Cart_create(comm_old, ndims, dims, periods, reorder,
				  comm_cart);
	debug("<< C Cart_create...............");
			...
			MPI_Comm * intercomm, int array_of_errcodes[])
{
	...
	int res =
	    next_mpic.Comm_spawn(command, argv, maxprocs, info, root, comm,
				 ...
				 int array_of_errcodes[])
{
	...
	before_mpi(Comm_spawn_multiple, (p2i) array_of_commands,
		   (p2i) array_of_info);
	int res =
	    next_mpic.Comm_spawn_multiple(count, array_of_commands,
			  ...
			  MPI_Datatype datatype, MPI_Status * status)
{
	...
	int res =
	    next_mpic.File_read_at(fh, offset, buf, count, datatype, status);
	debug("<< C File_read_at...............");
			      ...
			      MPI_Status * status)
{
	...
	int res =
	    next_mpic.File_read_at_all(fh, offset, buf, count, datatype,
				       status);
	debug("<< C File_read_at_all...............");
			   ...
			   MPI_Status * status)
{
	...
	int res =
	    next_mpic.File_write_at(fh, offset, buf, count, datatype, status);
			       ...
			       MPI_Datatype datatype, MPI_Status * status)
{
	...
	int res =
	    next_mpic.File_write_at_all(fh, offset, buf, count, datatype,
					status);
	debug("<< C File_write_at_all...............");
		    ...
		    MPI_Datatype recvtype, int root, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount,
			     recvtype, root, comm);
	debug("<< C Gather...............");
	after_mpi(Gather);
	return res;
		     ...
		     MPI_Datatype recvtype, int root, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Gatherv(sendbuf, sendcount, sendtype, recvbuf, recvcounts,
		 ...
		 MPI_Datatype target_datatype, MPI_Win win)
{
	...
	int res =
	    next_mpic.Get(origin_addr, origin_count, origin_datatype,
		    ...
		    int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ibsend(buf, count, datatype, dest, tag, comm, request);
	debug("<< C Ibsend...............");
	after_mpi(Ibsend);
	return res;
}
			      ...
			      MPI_Comm * newintercomm)
{
	...
	int res =
	    next_mpic.Intercomm_create(local_comm, local_leader, peer_comm,
		   ...
		   int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Irecv(buf, count, datatype, source, tag, comm, request);
	debug("<< C Irecv...............");
	after_mpi(Irecv);
	return res;
}
		    ...
		    int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Irsend(buf, count, datatype, dest, tag, comm, request);
	debug("<< C Irsend...............");
	after_mpi(Irsend);
	return res;
}
		   ...
		   int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Isend(buf, count, datatype, dest, tag, comm, request);
	debug("<< C Isend...............");
	after_mpi(Isend);
	return res;
}
		    ...
		    int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Issend(buf, count, datatype, dest, tag, comm, request);
	debug("<< C Issend...............");
	after_mpi(Issend);
	return res;
}
		 ...
		 MPI_Datatype target_datatype, MPI_Win win)
{
	...
	int res =
	    next_mpic.Put(origin_addr, origin_count, origin_datatype,
			  target_rank, target_disp, target_count,
			  target_datatype, win);
	debug("<< C Put...............");
	after_mpi(Put);
		  ...
		  int tag, MPI_Comm comm, MPI_Status * status)
{
	...
	int res =
	    next_mpic.Recv(buf, count, datatype, source, tag, comm, status);
	debug("<< C Recv...............");
	after_mpi(Recv);
	return res;
}
		       ...
		       int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Recv_init(buf, count, datatype, source, tag, comm,
				request);
	debug("<< C Recv_init...............");
	after_mpi(Recv_init);
	return res;
		    ...
		    MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Reduce(sendbuf, recvbuf, count, datatype, op, root, comm);
	debug("<< C Reduce...............");
	after_mpi(Reduce);
	return res;
}
			    ...
			    MPI_Op op, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Reduce_scatter(sendbuf, recvbuf, recvcounts, datatype, op,
				     comm);
	debug("<< C Reduce_scatter...............");
			...
			int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Rsend_init(buf, count, datatype, dest, tag, comm,
				 request);
	debug("<< C Rsend_init...............");
		     ...
		     MPI_Datatype recvtype, int root, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Scatter(sendbuf, sendcount, sendtype, recvbuf, recvcount,
			      recvtype, root, comm);
	debug("<< C Scatter...............");
	after_mpi(Scatter);
	return res;
		      ...
		      int root, MPI_Comm comm)
{
	...
	int res =
	    next_mpic.Scatterv(sendbuf, sendcounts, displs, sendtype, recvbuf,
		       ...
		       int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Send_init(buf, count, datatype, dest, tag, comm, request);
	debug("<< C Send_init...............");
	after_mpi(Send_init);
	return res;
}
		      ...
		      MPI_Status * status)
{
	...
	int res =
	    next_mpic.Sendrecv(sendbuf, sendcount, sendtype, dest, sendtag,
			      ...
			      MPI_Comm comm, MPI_Status * status)
{
	...
	int res =
	    next_mpic.Sendrecv_replace(buf, count, datatype, dest, sendtag,
			...
			int dest, int tag, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ssend_init(buf, count, datatype, dest, tag, comm,
				 request);
	debug("<< C Ssend_init...............");
		     ...
		     MPI_Status array_of_statuses[])
{
	...
	int res =
	    next_mpic.Testall(count, array_of_requests, flag,
			      array_of_statuses);
	debug("<< C Testall...............");
	after_mpi(Testall);
	return res;
		     ...
		     int *flag, MPI_Status * status)
{
	...
	int res =
	    next_mpic.Testany(count, array_of_requests, indx, flag, status);
	debug("<< C Testany...............");
	after_mpi(Testany);
	return res;
}
		      ...
		      MPI_Status array_of_statuses[])
{
	...
	int res =
	    next_mpic.Testsome(incount, array_of_requests, outcount,
		     ...
		     MPI_Status * array_of_statuses)
{
	...
	int res =
	    next_mpic.Waitall(count, array_of_requests, array_of_statuses);
	debug("<< C Waitall...............");
	after_mpi(Waitall);
	return res;
}
		      ...
		      MPI_Status * array_of_statuses)
{
	...
	int res =
	    next_mpic.Waitsome(incount, array_of_requests, outcount,
			       array_of_indices, array_of_statuses);
	debug("<< C Waitsome...............");
	after_mpi(Waitsome);
	return res;
			...
			MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iallgather(sendbuf, sendcount, sendtype, recvbuf,
			 ...
			 MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iallgatherv(sendbuf, sendcount, sendtype, recvbuf,
			...
			MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iallreduce(sendbuf, recvbuf, count, datatype, op, comm,
				 request);
	debug("<< C Iallreduce...............");
		       ...
		       MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ialltoall(sendbuf, sendcount, sendtype, recvbuf,
				recvcount, recvtype, comm, request);
	debug("<< C Ialltoall...............");
	after_mpi(Ialltoall);
	return res;
			...
			MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ialltoallv(sendbuf, sendcounts, sdispls, sendtype,
		    ...
		    MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ibcast(buffer, count, datatype, root, comm, request);
	debug("<< C Ibcast...............");
	after_mpi(Ibcast);
	return res;
}
		     ...
		     MPI_Request * request)
{
	...
	int res =
	    next_mpic.Igather(sendbuf, sendcount, sendtype, recvbuf, recvcount,
		      ...
		      MPI_Request * request)
{
	...
	int res =
	    next_mpic.Igatherv(sendbuf, sendcount, sendtype, recvbuf,
		     ...
		     MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ireduce(sendbuf, recvbuf, count, datatype, op, root, comm,
			      request);
	debug("<< C Ireduce...............");
	after_mpi(Ireduce);
	return res;
			     ...
			     MPI_Op op, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Ireduce_scatter(sendbuf, recvbuf, recvcounts, datatype,
				      op, comm, request);
	debug("<< C Ireduce_scatter...............");
		   ...
		   MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iscan(sendbuf, recvbuf, count, datatype, op, comm,
			    request);
	debug("<< C Iscan...............");
	after_mpi(Iscan);
	return res;
		      ...
		      MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iscatter(sendbuf, sendcount, sendtype, recvbuf, recvcount,
			       recvtype, root, comm, request);
	debug("<< C Iscatter...............");
	after_mpi(Iscatter);
	return res;
		       ...
		       int root, MPI_Comm comm, MPI_Request * request)
{
	...
	int res =
	    next_mpic.Iscatterv(sendbuf, sendcounts, displs, sendtype, recvbuf,
