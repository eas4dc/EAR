...
{
    debug("install path: %s | custom path: %s | Authorised ? %d", install_path, custom_path,
                                      ...
                                      const char *plug_name, settings_conf_t *sconf)
{
    ...
    if (hack_earl_install) {
        snprintf(result_path, result_path_s - 1, "%s/plugins/%s/%s", hack_earl_install, plug_endpt, plug_name);
    ...
    snprintf(result_path, result_path_s - 1, "%s/%s/%s", sconf->installation.dir_plug, plug_endpt, plug_name);
    debug("Looking for %s", result_path);
    if (ear_file_exists(result_path))
        return EAR_SUCCESS;

    /* Test whether the plug-in is where plug_endpt literally says */
    debug("Looking for %s", plug_name) if (ear_file_exists(plug_name))
    {
