...
void print_lib_shared_data(lib_shared_data_t *sh_data)
{
    fprintf(stderr, "sh_data num_processes %d signatures %d cas_counters %llu\n", sh_data->num_processes,
            sh_data->num_signatures, sh_data->cas_counters);
}
...
void free_node_signatures(lib_shared_data_t *data, shsignature_t *sig)
{
    ...
    if (ear_msync(sig, sizeof(shsignature_t) * data->num_processes, MS_SYNC | MS_INVALIDATE) < 0)
        verbose_master(2, "Memory sync fails %s", strerror(errno));
}
...
int select_global_cp(int size, int max, int *ppn, shsignature_t *my_sh_sig, int *node_cp, int *rank_cp)
{
    ...
    *rank_cp = rank;
    debug("(MIN PERC MPI %.1lf, MAX PERC MPI %.1lf) (MPI_CALLS %llu MPI_TIME %.3fsec USER_TIME=%.3fsec)\n",
          minp * 100.0, maxp * 100.0, total_mpi, (float) total_mpi_time / 1000000.0,
...
{
    snprintf(msg, max, "RANK[%d] total_mpi_calls %llu exec_time %llu mpi_time %llu perc_mpi %.3lf ", info->rank,
             info->total_mpi_calls, info->exec_time, info->mpi_time, info->perc_mpi * 100.0);
}
...
{
    snprintf(msg, max, "%d;%llu;%llu;%llu;%.3lf", info->rank, info->total_mpi_calls, info->exec_time, info->mpi_time,
             info->perc_mpi * 100.0);
}
...
void print_sh_signature(int localid, shsignature_t *sig)
{
    ...
    fprintf(stderr, "RANK[%d]= %d mpi_data={total_mpi_calls %llu mpi_time %llu exec_time %.3f PercTime %lf }\n",
            localid, sig->mpi_info.rank, sig->mpi_info.total_mpi_calls, sig->mpi_info.mpi_time, t,
            sig->mpi_info.perc_mpi);
    fprintf(stderr,
            "RANK[%d]= %d signature={cpi %.3lf tpi %.3lf time %.3lf Gflops %f dc_power %.3lf avgf %.1f deff %.1f} "
            "state %d new_freq %.1f\n",
            localid, sig->mpi_info.rank, sig->sig.CPI, sig->sig.TPI, sig->sig.time, sig->sig.Gflops, sig->sig.DC_power,
            avgf, deff, sig->app_state, newf);
}
...
{
    if (sig == NULL || t_fetch_decode == NULL) {
        return_msg(EAR_ERROR, Generr.input_null);
    } else if (n_procs == 0) {
        return_msg(EAR_WARNING, "Number of processes is zero.");
    }
    ...
    *t_fetch_decode = 0;
    for (i = 0; i < n_procs; i++) {
        *t_fetch_decode += sig[i].sig.stalls.fetch_decode;
    }
...
state_t compute_job_node_stalls_resources(const shsignature_t *sig, int n_procs, ull *t_resources)
{
    ...
    *t_resources = 0;
    for (i = 0; i < n_procs; i++) {
        *t_resources += sig[i].sig.stalls.resources;
    }
...
state_t compute_job_node_stalls_memory(const shsignature_t *sig, int n_procs, ull *t_memory)
{
    ...
    *t_memory = 0;
    for (i = 0; i < n_procs; i++) {
        *t_memory += sig[i].sig.stalls.memory;
    }
...
void init_earl_node_mgr_info()
{
    ...
    for (uint j = 0; j < MAX_CPUS_SUPPORTED; j++) {
        ...
        while (!abort_init && (appid_folder_full = folder_getnext_type(&SID_folder, NULL, ".app", DT_DIR))) {
            ...
            if (curr_app == total_num_earl_apps) {
                verbose_warning_master("Initialzing node_mgr_earl info. Current total apps: %d Count based on folders "
                ...
                total_num_earl_apps++;
                verbose_master(WF_SUPPORT_VERB, "New Area space %p, total apps %d", node_mgr_job_info,
                               total_num_earl_apps);
            }
            ...
            if (apps) {
                ...
                if (get_lib_shared_data_path(tmp, ID, lid, lib_shared_region_path_jobs) == EAR_SUCCESS) {
                    node_mgr_job_info[curr_app].libsh =
                        attach_lib_shared_data_area(lib_shared_region_path_jobs, &node_mgr_job_info[curr_app].fd_lib);
                ...
                if (get_shared_signatures_path(tmp, ID, lid, sig_shared_region_path_jobs) == EAR_SUCCESS) {
                    node_mgr_job_info[curr_app].shsig = attach_shared_signatures_area(
    ...
    node_mgr_info_unlock();
    verbose_master(WF_SUPPORT_VERB, "init_earl_node_mgr_info done: Total apps computed %d , based on folders %d",
            ...
            if ((id != -1) && (sid != -1) && (j != node_mgr_index)) {
                ...
                if (state_ok(ret_st)) {
                    node_mgr_job_info[j].libsh =
                        attach_lib_shared_data_area(lib_shared_region_path_jobs, &node_mgr_job_info[j].fd_lib);
                ...
                if (state_ok(ret_st)) {
                    node_mgr_job_info[j].shsig = attach_shared_signatures_area(
                        sig_shared_region_path_jobs, node_mgr_job_info[j].libsh->num_processes,
...
void release_earl_node_mgr_info()
{
    ...
    for (uint curr_app = 0; curr_app < total_num_earl_apps; curr_app++) {
        ...
        if (node_mgr_job_info[curr_app].fd_sig != -1)
            dettach_shared_signatures_area(node_mgr_job_info[curr_app].fd_sig);
        node_mgr_job_info[curr_app].libsh = NULL;
        node_mgr_job_info[curr_app].shsig = NULL;
    }
...
uint node_manager_job_changed(job_id jid, job_id sid, time_t mt)
{
    while (node_mgr_info_lock() != EAR_SUCCESS)
        ;
    for (uint j = 0; j < MAX_CPUS_SUPPORTED; j++) {
        if ((node_mgr_data[j].jid == jid) && (node_mgr_data[j].sid == sid) &&
...
void update_earl_node_mgr_info()
{
    ...
    if (!release) {
        ...
        if (expected_apps != earl_apps_in_sh_data) {
            release = 1;
            verbose_master(WF_SUPPORT_VERB, "EARL[%d] Num EARL apps have changed expected %u current %u", getpid(),
                           expected_apps, earl_apps_in_sh_data);
        }
    }
...
#if WF_SUPPORT
                verbose_master(vl, "Job[%u] %lu.%lu.%lu %s\nNode [%.2lf GB/s %.2lf W]", i, nmgr_earl[i].jid,
                               nmgr_earl[i].sid, nmgr_earl[i].lid, sig_buff,
#else
                verbose_master(vl, "Job[%u] %lu.%lu %s\nNode [%.2lf GB/s %.2lf W]", i, nmgr_earl[i].jid,
                               nmgr_earl[i].sid, sig_buff,
#endif
                               nmgr_earl[i].libsh->node_signature.GBS, nmgr_earl[i].libsh->node_signature.DC_power);
...
void accum_estimations(lib_shared_data_t *data, shsignature_t *sig)
{
    ...
    for (p = 0; p < data->num_processes; p++) {
        ...
        debug("Accumulated energy for process %d %lu (%lf x %lf) mem %llu avg %lu (%lu x %lf) dram %lu (%lf) pack %lu "
              "(%lf) ",
              p, sig_shared_region[p].sig.accum_energy, sig_shared_region[p].sig.DC_power, time_s,
              sig_shared_region[p].sig.accum_mem_access, sig_shared_region[p].sig.accum_avg_f,
              sig_shared_region[p].sig.avg_f, time_s, sig_shared_region[p].sig.accum_dram_energy,
...
#if SHOW_DEBUGS
    for (uint p = 0; p < data->num_processes; p++) {
        debug("LP[%d] Power %.2lf GB/s %.2lf TPI %.2lf", p, sig_shared_region[p].sig.DC_power,
              sig_shared_region[p].sig.GBS, sig_shared_region[p].sig.TPI);
    }
#endif
    return;
}
...
state_t update_job_affinity_mask(lib_shared_data_t *data, shsignature_t *sig)
{
    ...
    for (int i = 0; i < data->num_processes; i++) {
        ...
        if (!ret_val) {
            ...
            char ret_msg[64];
            ret_val = snprintf(ret_msg, sizeof(ret_msg), "Getting the affinity mask of local rank (%d)", errno);
    ...
    if (data->num_cpus != node_mask_num_cpus_backup) {
        verbose_warning("The number of job's CPUs changed from %u to %u.", node_mask_num_cpus_backup, data->num_cpus);
    }
...
state_t compute_job_node_cache_metrics(const shsignature_t *sig, int n_procs, cache_signature_t *cache)
{
    ...
    /* Init the destination memory location */
    memset(cache, 0, sizeof(cache_signature_t));

    /* Accumulate absoulte values */
    for (int i = 0; i < n_procs; i++) {
        cache->l1d_misses += sig[i].sig.cache.l1d_misses;
        cache->l2_misses += sig[i].sig.cache.l2_misses;
        cache->l3_misses += sig[i].sig.cache.l3_misses;
        cache->ll_misses += sig[i].sig.cache.ll_misses;

        cache->l1d_hits += sig[i].sig.cache.l1d_hits;
        cache->l2_hits += sig[i].sig.cache.l2_hits;
        cache->l3_hits += sig[i].sig.cache.l3_hits;
        cache->ll_hits += sig[i].sig.cache.ll_hits;

        cache->l1d_accesses += sig[i].sig.cache.l1d_accesses;
        cache->l2_accesses += sig[i].sig.cache.l2_accesses;
        cache->l3_accesses += sig[i].sig.cache.l3_accesses;
        cache->ll_accesses += sig[i].sig.cache.ll_accesses;
    }

    /* Compute rates */
    cache->l1d_miss_rate = (cache->l1d_accesses) ? cache->l1d_misses / (double) cache->l1d_accesses : 0;
    cache->l1d_hit_rate  = (cache->l1d_accesses) ? cache->l1d_hits / (double) cache->l1d_accesses : 0;

    cache->l2_miss_rate = (cache->l2_accesses) ? cache->l2_misses / (double) cache->l2_accesses : 0;
    cache->l2_hit_rate  = (cache->l2_accesses) ? cache->l2_hits / (double) cache->l2_accesses : 0;

    cache->l3_miss_rate = (cache->l3_accesses) ? cache->l3_misses / (double) cache->l3_accesses : 0;
    cache->l3_hit_rate  = (cache->l3_accesses) ? cache->l3_hits / (double) cache->l3_accesses : 0;

    cache->ll_miss_rate = (cache->ll_accesses) ? cache->ll_misses / (double) cache->ll_accesses : 0;
    cache->ll_hit_rate  = (cache->ll_accesses) ? cache->ll_hits / (double) cache->ll_accesses : 0;
