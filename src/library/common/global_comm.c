...
/**************************************************************************************************/
state_t ishare_global_info(MPI_Comm comm, char *data_send, int size_send, char *data_recv, int size_recv,
                           MPI_Request *req)
{
    if ((data_send == NULL) || (data_recv == NULL))
        return EAR_ERROR;
    if ((size_send == 0) || (size_recv == 0))
        return EAR_ERROR;
    PMPI_Iallgather(data_send, size_send, MPI_SIGNED_CHAR, data_recv, size_recv, MPI_SIGNED_CHAR, comm, req);
    return EAR_SUCCESS;
}
...
{
    PMPI_Allgather(data_send, size_send, MPI_SIGNED_CHAR, data_recv, size_recv, MPI_SIGNED_CHAR, comm);
    return EAR_SUCCESS;
}
...
state_t send_node_signatures(masters_info_t *mi, lib_shared_data_t *data, shsignature_t *sig, shsignature_t *all_sig,
                             int show_sig)
{
    state_t ret;
    int max_ppn;
    ...
    if (!mi->node_info_pending) {
        ...
        if (ishare_global_info(mi->masters_comm, (char *) mi->my_mpi_info, sizeof(shsignature_t) * max_ppn,
            ...
            if (show_sig)
                print_shared_signatures(data, all_sig);
        }
    }
    ret = EAR_SUCCESS;
    return ret;
...
state_t check_mpi_info(masters_info_t *mi, int *node_cp, int *rank_cp, int show_sig)
{
    ...
    *rank_cp    = -1;
    ...
    if ((mi->my_master_rank >= 0) && mi->node_info_pending && (is_info_ready(&mi->req) == EAR_SUCCESS)) {
        ...
        if (show_sig && mi->my_master_rank == 0)
            print_global_signatures(mi);
...
{
    if (ishare_global_info(mi->masters_comm, (char *) my_data, sizeof(mpi_summary_t), (char *) mi->nodes_mpi_summary,
                           sizeof(mpi_summary_t), &mpi_summary_req) != EAR_SUCCESS) {
...
void verbose_node_mpi_summary(int vl, masters_info_t *mi)
{
    ...
    for (i = 0; i < mi->my_master_size; i++) {
        ...
        verbose_master(vl, "MR[%d] maxp %.2f minp %.2f sd %.2f mean %.2f mag %.2f", i, l->max, l->min, l->sd, l->mean,
                       l->mag);
    }
}
#endif
