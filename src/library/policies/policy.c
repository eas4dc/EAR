...
#define verbose_policy_info(msg, ...) verbose_info2_master("Policy: " msg, ##__VA_ARGS__);
...
#define POLICY_MPI_CALL_ENABLED_DEF                                                                                    \
    1 // Enables doing stuff at MPI call level.
      // Useful for fast overhead testing.

static int POLICY_MPI_CALL_ENABLED = POLICY_MPI_CALL_ENABLED_DEF;
...
unsigned long long int total_mpi = 0; // This variable only counts the number of MPI calls and it's only used
                                      // at state_comm::verbose_signature. It only is useful when only the master
                                      // verboses, so it's value only shows master info while that signature verbose
                                      // message tries to show per-node info.
 ...
 * If mgt_cpufreq, met_cpi and met_bwidth API are correct, \p default_policy_name is used.
 * Otherwise 'monitoring' is used.
 * This function also honors FLAG_APP_MGR_POLICIES and prepends 'app_' to the policy object name.
 * The final policy object name always have '.so' suffixed.
 */
...
static state_t from_proc_to_core(ulong *process_cpu_freqs_khz, int process_cnt, ulong **core_cpu_freqs_khz, int cpu_cnt,
                                 ulong *cpu_freq_max, ulong *cpu_freq_min, int process_id);
...
static state_t policy_freq_selection(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs, node_freq_domain_t *dom,
...
static state_t policy_cpu_freq_selection(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs, node_freq_domain_t *dom,
                                         node_freqs_t *avg_freq, int process_id);
...
static int policy_mpi_features_strategy()
{
    char *mode = ear_getenv(FLAG_MPI_MONITORING);
    if (!mode)
        return POLICY_MPI_CALL_ENABLED;
    strtoup(mode);
    verbose_master(2, "EARL: Policy MPI features mode %s", mode);
    return (strcmp(mode, "ENABLED") == 0 ? 1 : 0);
}
...
state_t init_power_policy(settings_conf_t *app_settings, resched_t *res)
{
    ...
    POLICY_MPI_CALL_ENABLED = policy_mpi_features_strategy();
    verbose_policy_info("MPI policy actions: %s", (POLICY_MPI_CALL_ENABLED ? "Enabled" : "Disabled"));

    return policy_init();
}

/* This is the main function for frequency selection at application level */
/* The signatures are used first the last signatur computed is a global variable, second signatures are in the shared
 * memory */
...
state_t policy_app_apply(ulong *freq_set, int *ready)
{
    ...
    *ready = EAR_POLICY_READY;
    ...
    if (*ready == EAR_POLICY_READY) {
        ...
        st = policy_master_freq_selection(&my_pol_ctx, &policy_last_global_signature, freqs, &freqs_domain, &avg_nf,
                                          ALL_PROCESSES);
...
state_t policy_busy_waiting_configuration(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs,
                                          node_freq_domain_t *dom, node_freqs_t *avg_freq)
{
...
static state_t policy_init()
{
    ...
    char *cexclusive_mode =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_EXCLUSIVE_MODE) : NULL; // Job has the node exclusive
    char *cuse_earl_phases =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_EARL_PHASES) : NULL; // Detect application phases
    char *cenable_load_balance =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_LOAD_BALANCE) : NULL; // Load balance efficiency
    char *cuse_turbo_for_cp =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_TURBO_CP) : NULL; // Boost critical path processes
    char *ctry_turbo_enabled = (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_TRY_TURBO)
                                                                      : NULL; // Boost turbo if policy sets max cpufreq
    char *cuse_energy_models =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_USE_ENERGY_MODELS) : NULL; // Use energy models
    char *cmin_cpufreq =
        (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_MIN_CPUFREQ) : NULL; // Minimum CPU freq.
    ...
    if (cfake_io_phase != NULL)
        fake_io_phase = atoi(cfake_io_phase);
    if (cfake_bw_phase != NULL)
        fake_bw_phase = atoi(cfake_bw_phase);

    if (fake_io_phase || fake_bw_phase) {
        verbose_master(0, "%sWARNING%s Using FAKE I/O phase: %u FAKE BW phase: %u", COL_RED, COL_CLR, fake_io_phase,
                       fake_bw_phase);
    }
    ...
    verbose_policy_info("Min. CPU freq. limited: %llu kHz / Min. IMC freq. limited: %llu kHz", min_cpufreq,
                        min_imcfreq);
    ...
    if (cexclusive_mode != NULL)
        exclusive_mode = atoi(cexclusive_mode);
    ...
    if (cuse_earl_phases != NULL)
        use_earl_phases = atoi(cuse_earl_phases);
    ...
    if (cenable_load_balance != NULL)
        enable_load_balance = atoi(cenable_load_balance);
    ...
    if (cuse_turbo_for_cp != NULL)
        use_turbo_for_cp = atoi(cuse_turbo_for_cp);
    ...
    if (ctry_turbo_enabled != NULL)
        try_turbo_enabled = atoi(ctry_turbo_enabled);
...
{
    if (my_pol_ctx.num_gpus == 0)
        gpu_ready = EAR_POLICY_READY;
    if ((cpu_ready == EAR_POLICY_READY) && (gpu_ready == EAR_POLICY_READY))
        return 1;
    return 0;
}
...
{
    if ((cpu_ready == EAR_POLICY_TRY_AGAIN) || (gpu_ready == EAR_POLICY_TRY_AGAIN))
        return 1;
    return 0;
}
...
state_t policy_node_apply(signature_t *my_sig, ulong *freq_set, int *ready)
{
    ...
    if (!lib_shared_region->estimate_node_metrics) {
        verbose_master(POLICY_INFO,
                       "%sPOLICY%s Policy not applied in EARL[%d] because is master in master/worker use case", COL_BLU,
                       COL_CLR, getpid());
        *ready = EAR_POLICY_READY;
        return EAR_SUCCESS;
    }
    ...
    if (use_earl_phases) {
        ...
        if ((earl_phase_classification == APP_COMP_BOUND) || (earl_phase_classification == APP_MPI_BOUND)) {
            ...
            if (busy == BUSY_WAITING) {
                verbose_master(POLICY_PHASES, "%sPOLICY%s CPU Busy waiting configuration (%s)", COL_BLU, COL_CLR,
                               node_name);
            ...
            if ((last_earl_phase_classification == APP_BUSY_WAITING) ||
                (last_earl_phase_classification == APP_IO_BOUND) || !(last_gpu_state & curr_gpu_state)) {
    ...
    if (polsyms_fun.node_policy_apply) {
        ...
        if ((cpu_ready != EAR_POLICY_READY) && (freqs_domain.cpu == POL_GRAIN_CORE)) {
            memset(freqs->cpu_freq, 0, sizeof(ulong) * MAX_CPUS_SUPPORTED);
        }
    ...
    if (VERB_ON(POLICY_INFO)) {
        ...
        verbose_master(POLICY_INFO + 1,
                       "%sPOLICY%s CPU freq. policy ready: %d\n"
                       "GPU policy ready: %d\nGlobal node readiness: %d",
                       COL_BLU, COL_CLR, cpu_ready, gpu_ready, *ready);
    }
...
#define verbose_policy_ok(msg, ...) verbose_policy_info("%sPOLICY OK%s " msg, COL_BLU, COL_CLR, ##__VA_ARGS__)
...
state_t policy_end()
{
...
#if DEBUG_CPUFREQ_COST
    verbose_master(1, "Overhead of cpufreq setting %lu us: Total calls %lu: Cost per call %f", total_cpufreq_cost_us,
                   calls_cpufreq, (float) total_cpufreq_cost_us / (float) calls_cpufreq);
#endif
    ...
    // Let other processes acquire the GPU optimize lock.
    policy_gpu_unlock_opt();
...
state_t policy_new_iteration(signature_t *sig)
{
    ...
    if ((last_phase_io_bnd || last_phase_bw) && metrics_CPU_phase_ok()) {
        verbose_policy_info("Validating CPU phase: (GFlop/s = %.2lf, IO = %.2lf CPI %.2lf)", sig->Gflops, sig->IO_MBS,
                            sig->CPI);

        uint restore = 0;

        if (last_phase_io_bnd && (sig->IO_MBS < phases_limits.io_th))
            restore = 1;
        if (last_phase_bw && (sig->CPI > phases_limits.cpi_busy_waiting))
            restore = 1;
        /* WARNING: Below condition won't be accomplished never. */
        if ((sig->Gflops / lib_shared_region->num_cpus) > phases_limits.gflops_busy_waiting)
            restore = 1;
    ...
    if (last_gpu_state & _GPU_Idle) {
        ...
        verbose_master(POLICY_GPU, "%sPOLICY%s Validating GPU phase: (#GPUs) %u", COL_BLU, COL_CLR,
                       sig->gpu_sig.num_gpus);
...
static node_freq_domain_t per_process_dom = {
    .cpu = POL_GRAIN_CORE, .mem = POL_NOT_SUPPORTED, .gpu = POL_NOT_SUPPORTED, .gpumem = POL_NOT_SUPPORTED};
        ...
        if (polsyms_fun.mpi_init != NULL) {
            ...
            if (state_ok(polsyms_fun.mpi_init(c, call_type, &per_process_node_freq, &process_id))) {
                ...
                if (process_id != NO_PROCESS) {
                    ...
                    if (state_fail(policy_freq_selection(c, &aux_per_process, &per_process_node_freq, &per_process_dom,
                                                         NULL, process_id))) {
...
state_t policy_mpi_end(mpi_call call_type)
{
    if (POLICY_MPI_CALL_ENABLED) {
        ...
        if (polsyms_fun.mpi_end != NULL) {
            ...
            if (process_id != NO_PROCESS) {
                st = policy_freq_selection(c, &aux_per_process, &per_process_node_freq, &per_process_dom, NULL,
                                           process_id);
                if (state_fail(st)) {
...
static void select_policy_name(char *ret_pol_name, size_t pol_name_size, char *default_policy_name)
{
    ...
    if (!app_mgr) {
        snprintf(ret_pol_name, pol_name_size - 1, "%s.so", policy_name);
    } else {
        snprintf(ret_pol_name, pol_name_size - 1, "app_%s.so", policy_name);
    }
}
...
{
    if (freq_set != NULL)
        node_freqs_copy(freq_set, &nf);
    return EAR_SUCCESS;
}

static state_t fill_mgt_api_pstate_data(uint api_id, pstate_t **pstate_lst, uint *pstate_cnt)
{
    debug("Requesting MGT_CPUFREQ (%u) vs. MGT_IMCFREQ (%u): %u", MGT_CPUFREQ, MGT_IMCFREQ, api_id);
...
static state_t from_proc_to_core(ulong *process_cpu_freqs_khz, int process_cnt, ulong **core_cpu_freqs_khz, int cpu_cnt,
                                 ulong *cpu_freq_max, ulong *cpu_freq_min, int proc_id)
{
    ...
    if (proc_id >= 0) {
        verbose(3, "from_proc_to_core  [%d] Freq %.2f", proc_id,
                (float) process_cpu_freqs_khz[proc_id] / (float) 1000000.0);
    }
    ...
    if (process_cpu_freqs_khz == NULL || process_cnt < 0 || core_cpu_freqs_khz == NULL || *core_cpu_freqs_khz == NULL ||
        cpu_cnt < 0) {
    ...
    if (ignore_affinity_mask) {
        ...
        if (cpu_freq_max != NULL)
            *cpu_freq_max = cpu_freq_khz;
        if (cpu_freq_min != NULL)
            *cpu_freq_min = cpu_freq_khz;
        return EAR_SUCCESS;
    }
    ...
    for (int proc_lrank = init_rank; proc_lrank < end_rank; proc_lrank++) {
        ...
        if (process_cpu_freqs_khz[proc_lrank] < (ulong) min_cpufreq) {
            debug("The selected frequency (%lu) for process %d is lower than the minimum"
                  " permitted (%lu). Filtering...",
                  process_cpu_freqs_khz[proc_lrank], proc_lrank, (ulong) min_cpufreq);
        }
#endif // SHOW_DEBUGS
...
static uint must_set_policy_cpufreq(pc_app_info_t *pc_data, settings_conf_t *settings_conf)
{
    ...
    if (pc_data == NULL)
        return 1;
    if ((pc_data->cpu_mode == PC_DVFS) && (settings_conf->pc_opt.current_pc <= POWER_CAP_UNLIMITED))
        return 1;
                                         ...
                                         node_freqs_t *avg_freq, int process_id)
{
    ...
    if (dom->cpu == POL_GRAIN_CORE)
        sprintf(pol_grain_str, "GRAIN_CORE");
    else
        sprintf(pol_grain_str, "GRAIN_NODE");

    verbose_master(3, "%-12s: %s\n%-12s: %d", "Policy level", pol_grain_str, "Affinity", ear_affinity_is_set);
    ...
    if ((dom->cpu == POL_GRAIN_CORE) && (ear_affinity_is_set)) {
        from_proc_to_core(freq_set, lib_shared_region->num_processes, &freq_per_core, arch_desc.top.cpu_count,
                          &max_cpufreq_sel_khz, &min_cpufreq_sel_khz, process_id);
    } else {
    ...
    *(c->ear_frequency) = max_cpufreq_sel_khz;
    ...
    if (pc_app_info_data != NULL) {
        debug("pc_app_info_data->cpu_mode (%d) == PC_DVFS ? %d", pc_app_info_data->cpu_mode,
              pc_app_info_data->cpu_mode == PC_DVFS);
    }
    debug("system_conf->pc_opt.current_pc (%d) <= POWER_CAP_UNLIMITED (%d) ? %d", system_conf->pc_opt.current_pc,
          POWER_CAP_UNLIMITED, system_conf->pc_opt.current_pc <= POWER_CAP_UNLIMITED);
#endif
    ...
    if (must_set_policy_cpufreq(pc_app_info_data, system_conf)) {
        if ((masters_info.my_master_rank >= 0) && (process_id == ALL_PROCESSES)) {
            verbose_policy_info("Setting a CPU frequency (kHz) range of [%lu, %lu] GHz...", min_cpufreq_sel_khz,
                                max_cpufreq_sel_khz);
        }
        uint do_change = 1;
#if 0
...
static state_t policy_mem_freq_selection(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs, node_freq_domain_t *dom,
                                         node_freqs_t *avg_freq)
{
    ...
    verbose_policy_info("(Lower/Upper) requested IMC pstate (%lu/%lu)", freqs->imc_freq[IMC_MAX],
                        freqs->imc_freq[IMC_MIN]);
    ...
    if (dyn_unc && metrics_get(MGT_IMCFREQ)->ok) {
        ...
        for (int sid = 0; sid < imc_devices; sid++) {
            ...
            if (imc_up[sid] != ps_nothing) {
                verbose_policy_info("Setting IMC freq (Upper/Lower) [SID=%d] to (%llu/%llu)", sid,
                                    imc_pstates[imc_low[sid]].khz, imc_pstates[imc_up[sid]].khz);
            } else {
                verbose_policy_info("Setting IMC freq (Upper) [SID=%d] (%llu) and min", sid,
                                    imc_pstates[imc_low[sid]].khz);
            }
        }
        ...
        if (state_fail(mgt_imcfreq_set_current_ranged_list(NULL, imc_low, imc_up))) {
            uint sid = 0;
            if (imc_up[sid] != ps_nothing) {
                verbose_error_master("Error setting IMC freq to %llu and %llu", imc_pstates[imc_low[0]].khz,
                                     imc_pstates[imc_up[0]].khz);
            } else {
                verbose_error_master("Error setting IMC freq to %llu and min", imc_pstates[imc_low[0]].khz);
            }
        }
...
static state_t policy_gpu_mem_freq_selection(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs,
                                             node_freq_domain_t *dom, node_freqs_t *avg_freq)
{
    return EAR_SUCCESS;
}

static state_t policy_gpu_freq_selection(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs, node_freq_domain_t *dom,
                                         node_freqs_t *avg_freq)
{
    ...
    if (verb_level >= POLICY_GPU) {
        for (i = 0; i < c->num_gpus; i++) {
            verbose_policy_info("Setting a GPU freq[%d] = %.3f (GHz)", i, (float) gpu_f[i] / GHZ_TO_KHZ);
        }
                                     ...
                                     node_freqs_t *avg_freq, int process_id)
{
    ...
    if (process_id == ALL_PROCESSES) {
        ...
        if (dom->gpu != POL_NOT_SUPPORTED && policy_gpu_opt_enabled()) {
...
#define check(st, msg)                                                                                                 \
    ...
    if (mgt_cpufreq_governor_is_available(no_ctx, Governor.userspace)) {
        ...
        check(mgt_cpufreq_governor_get_list(no_ctx, governor_list_before_init), "Retrieving the governor list.");
        ...
        uint governor_int;
        check(governor_toint(governor_str, &governor_int), "Converting governor from str to int.");

        // Setting the governor
#if VERSION_MAJOR > 4 || (VERSION_MAJOR == 4 && VERSION_MINOR >= 3)
        // #warning mgt_cpufreq_governor_set_mask compiled
        check(mgt_cpufreq_governor_set_mask(no_ctx, governor_int, cpus_to_be_set), "Setting governor.");
#endif
    }
...
void compute_avg_sel_freq(ulong *avg_sel_mem_f, ulong *imcfreqs)
{
    ...
    for (uint sock_id = 0; sock_id < arch_desc.top.socket_count; sock_id++) {
        ...
        // Validate pstate index before access
        if (low >= imc_pstate_cnt) {
            verbose_master(2, "Invalid pstate index %lu", low);
            continue;
        }
...
static void policy_new_phase(uint phase, signature_t *sig)
{
    verbose_master(3, "policy_new_phase summary computation %s (%u/%u)", phase_to_str(phase), policy_cpu_bound,
                   policy_mem_bound);

    sig_ext_t *se = (sig_ext_t *) sig->sig_ext;
    if (se != NULL) {
        for (uint ph = 0; ph < EARL_MAX_PHASES; ph++)
            se->earl_phase[ph].elapsed = 0;
        ...
        if (phase == APP_COMP_BOUND) {
            ...
            verbose_master(3, "COMP_PHASE: cpu %u mem %u mix %u", policy_cpu_bound, policy_mem_bound,
                           !policy_cpu_bound && !policy_mem_bound);

            if (policy_cpu_bound)
                se->earl_phase[APP_COMP_CPU].elapsed = elapsed_comp;
            else if (policy_mem_bound)
                se->earl_phase[APP_COMP_MEM].elapsed = elapsed_comp;
            else
                se->earl_phase[APP_COMP_MIX].elapsed = elapsed_comp;
        }
...
static void build_plugin_path(char *policy_plugin_path, size_t plug_path_size, settings_conf_t *app_settings)
{
    ...
    /* Try to load a hacked CPU policy plug-in */
    if (hack_so) {
        if (state_ok(
                utils_build_valid_plugin_path(policy_plugin_path, plug_path_size, "policies", hack_so, app_settings)))
            return;
    }

    /* If the above didn't apply, try to load the configured CPU policy plug-in */
    char my_policy_name[SZ_FILENAME];
    select_policy_name(my_policy_name, sizeof(my_policy_name), app_settings->policy_name);

    if (state_fail(utils_build_valid_plugin_path(policy_plugin_path, plug_path_size, "policies", my_policy_name,
                                                 app_settings))) {
