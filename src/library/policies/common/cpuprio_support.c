...
static state_t policy_set_prio_with_mask(uint *dst_prio_list, uint dst_prio_list_count, uint priority,
                                         const cpu_set_t *mask);

/** Sets \p priority on those positions of \p not enabled by \p mask. */
static state_t policy_set_prio_with_mask_inverted(uint *dst_prio_list, uint dst_prio_list_count, uint priority,
                                                  const cpu_set_t *mask);
...
void policy_setup_priority()
{
    ...
    if ((user_cpuprio_list = (uint *) envtoat(FLAG_PRIO_CPUS, NULL, &user_cpuprio_list_count, ID_UINT)) != NULL) {
    ...
    } else if ((user_cpuprio_list = (uint *) envtoat(FLAG_PRIO_TASKS, NULL, &user_cpuprio_list_count, ID_UINT)) !=
               NULL) {
        ...
        if (state_fail(policy_setup_priority_per_task(user_cpuprio_list, user_cpuprio_list_count))) {
...
void policy_reset_priority()
{
    ...
    if (state_fail(reset_priority_list_with_mask((uint *) cpuprio_api->current_list, cpuprio_api->devs_count,
                                                 &lib_shared_region->node_mask))) {
    ...
    if (VERB_ON(VEARL_INFO)) {
        ...
        mgt_cpuprio_data_tostr((cpuprio_t *) cpuprio_api->avail_list, (uint *) cpuprio_api->current_list, buffer,
                               SZ_BUFFER_EXTRA);
...
static state_t policy_setup_priority_per_cpu(uint *setup_prio_list, uint setup_prio_list_count)
{
    if (setup_prio_list && setup_prio_list_count) {
        ...
        state_t ret = set_priority_list((uint *) cpuprio_api->current_list, cpuprio_api->devs_count,
                                        (const uint *) setup_prio_list, setup_prio_list_count,
        ...
        if (VERB_ON(VEARL_INFO)) {
            ...
            mgt_cpufreq_prio_data_print((cpuprio_t *) cpuprio_api->avail_list, (uint *) cpuprio_api->current_list,
                                        verb_channel);
        }
    ...
    if (setup_prio_list && setup_prio_list_count) {
        ...
        for (int i = 0; i < lib_shared_region->num_processes; i++) {
            ...
            if (prio_list_idx < setup_prio_list_count) {
                ...
                if (state_fail(policy_set_prio_with_mask((uint *) cpuprio_api->current_list, cpuprio_api->devs_count,
                                                         prio, mask))) {
                    char err_msg[64];
                    snprintf(err_msg, sizeof(err_msg), "Setting priority list for task %d (%s)", prio_list_idx,
                             state_msg);
        ...
        policy_set_prio_with_mask_inverted((uint *) cpuprio_api->current_list, cpuprio_api->devs_count, PRIO_SAME,
                                           (const cpu_set_t *) &lib_shared_region->node_mask);
