...
static const char *polsyms_nam[] = {"plugin_api_version",
                                    "policy_init",
                                    "policy_apply",
                                    "policy_app_apply",
...
p_plugin_t policy_plugin_load(char *plugin_path_name)
{
    ...
    if (policy_plugin->plugin_api_version) {
        if (*(policy_plugin->plugin_api_version) > api_version) {
            verbose_warning_master("Plugin %s version (%c) greater than API version (%c)", plugin_path_name,
                                   *(policy_plugin->plugin_api_version), api_version);
        }
    } else {
