...
state_t policy_set_risk(policy_conf_t *ref, policy_conf_t *current, risk_t risk_level, ulong opt_target, ulong mfreq,
                        ulong *nfreq, ulong *f_list, uint nump)
{
    ...
    if (panic) {
        ...
        current->p_state =
            ref->p_state + WARNING1_DEF_PSTATES_LESS + WARNING2_DEF_PSTATES_LESS + PANIC_DEF_PSTATES_LESS;
    ...
    *nfreq            = f;
    ...
    debug("new conf %lf def_pstate %u def_freq %f max_freq %lu", current->settings[0], current->p_state,
          current->def_freq, *nfreq);
    return EAR_SUCCESS;
}
