...
static node_freq_domain_t mt_freq_dom;
...
static double last_mpi_stats_perc_mpi_sd, last_mpi_stats_perc_mpi_mean, last_mpi_stats_perc_mpi_mag,
    last_mpi_stats_perc_mpi_median;
...
state_t policy_init(polctx_t *c)
{
    if (c) {
        ...
        create_policy_domain(c, &mt_freq_dom);
        ...
        verbose_master(2, "%sMIN TIME%s Using energy models: %u", COL_BLU, COL_CLR, use_energy_models);

        int i, sid = 0;

        topology_init(&mt_top);
        ...
        if (module_mpi_is_enabled()) {
            ...
            verbose_master(2, "%sMIN TIME%s Busy waiting MPI: %u", COL_BLU, COL_CLR, block_type == BUSY_WAITING_BLOCK);
        }
        ...
        // Read-friendly verbose of eUFS configuration
        verbose_master(2,
                       "      --- eUFS configuration ---\n"
                       "Dynamic Uncore freq. management: %d\n"
                       "      Using HW selection as ref: %d\n"
                       "              Extra IMC penalty: %.2lf\n"
                       "               Network uses IMC: %u\n"
                       "      --------------------------",
                       dyn_unc, imc_set_by_hw, imc_extra_th, network_use_imc);

        nominal_node = frequency_get_nominal_freq();
        verbose_master(2, "%sMIN TIME%s Nominal frequency of the node: %lu kHz", COL_BLU, COL_CLR, nominal_node);
        ...
        last_mpi_stats_perc_mpi_sd = last_mpi_stats_perc_mpi_mean = last_mpi_stats_perc_mpi_mag =
            last_mpi_stats_perc_mpi_median                        = 0;
        ...
        if (c->num_gpus) {
            if (state_fail(policy_gpu_load(c->app, &gpu_plugin))) {
                verbose_error_master("Loading GPU policy.");
            }

            if (gpu_plugin.init != NULL) {
                verbose_info2_master("[MIN TIME] Initializing GPU policy...");
                gpu_plugin.init(c);
            }
        ...
        verbose_node_freqs(3, &min_time_def_freqs);
#endif
...
/** This is the main function in this file, it implements the power policy. */
state_t policy_apply(polctx_t *c, signature_t *sig, node_freqs_t *freqs, int *ready)
{
    ...
    double min_eff_gain, local_eff_gain, base_eff_gain;
    ...
    uint cbound, turbo_set = 0; // , mbound

    ulong curr_imc_freq;
    uint eUFS            = dyn_unc;
    uint curr_imc_pstate = 0;
    ...
    if (c->use_turbo) {
        ...
        min_pstate = frequency_closest_pstate(c->app->max_freq);
    }
    ...
    if (c->pc_limit > 0 && c->pc_limit != POWER_CAP_UNLIMITED) {
    ...
    if (eUFS) {
        ...
        if (ref_imc_pstate == -1 || curr_imc_pstate != ref_imc_pstate) {
            ...
            verbose_master(2, "CPU pstate %lu IMC freq %lu IMC pstate %u", curr_pstate, my_app->avg_imc_f,
                           curr_imc_pstate);
    ...
    int policy_stable;
    int_policy_ok(c, my_app, &cpufreq_signature, &policy_stable);
    ...
    if ((min_time_state == SELECT_CPUFREQ) || (eUFS == 0)) {
        ...
        for (uint i = 0; i < nump; i++) {
            ...
            if (freq_per_core) {
                ...
                verbose_master(3, "CPI %.2f TPI %.2lf/%.2lf GBS %.2lf/%.2lf Power %.2lf/%.2lf", my_app->CPI, ltpi,
                               my_app->TPI, lgbs, my_app->GBS, lpower, my_app->DC_power);
...
#endif
                double factor =
                    ear_math_exp(-5 * ear_math_scale(percs_mpi[min_mpi], percs_mpi[max_mpi], percs_mpi[i])) *
                    critical_path[i];
                if (factor < 0.01) {
                    factor = 0.0;
                }
            ...
            if (use_energy_models) {
                ...
                compute_cpu_freq_min_time(my_app, cpu_energy_model, min_pstate, time_ref, local_eff_gain, curr_pstate,
                                          best_pstate, best_freq, def_freq, &new_freq[i]);
                debug("time ref %f | curr pstate %lu | best pstate %lu | best freq %lu | def freq %lu | new freq %lu",
                      time_ref, curr_pstate, best_pstate, best_freq, def_freq, new_freq[i]);
                if (critical_path[i]) {
                    new_freq[i] = frequency_pstate_to_freq(min_pstate);
                }

                if (freq_per_core) {
                    verbose_master(2, "PROC[%u][%u] CPI %.2f TPI %.2f GFlops %.2f Power %.2f PMPI %.2f penalty %.2lf",
                                   MASTER_ID, i, sig_shared_region[i].sig.CPI, sig_shared_region[i].sig.TPI,
                                   sig_shared_region[i].sig.Gflops, sig_shared_region[i].sig.DC_power,
                                   my_node_mpi_calls[i].perc_mpi, local_eff_gain);
                }
                if (use_turbo_for_cp) {
                    debug("Try to boost: freq per core ? %u already using turbo ? %u min_mpi ? %u", freq_per_core,
                          c->use_turbo, min_mpi);
                    ...
                    if (sig_ready[def_pstate] == 0) {
                        ...
                        *ready    = EAR_POLICY_TRY_AGAIN;
                        ...
                        if (curr_freq != def_freq) {
                            ...
                            if (policy_no_models_is_better_min_time(sig, prev_sig, time_ref, cpi_ref, gbs_ref, freq_ref,
                                                                    min_eff_gain)) {
        ...
        for (uint lp = 0; lp < lib_shared_region->num_processes; lp++)
            sig_shared_region[lp].new_freq = new_freq[lp];
        ...
        if ((use_energy_models || *ready == EAR_POLICY_READY) && eUFS && !(app_mpi_bound && network_use_imc)) {
            ...
            if ((min_cpufreq_sel == nominal_node) && cbound) {
                ...
                debug("min CPU freq selected %lu - nominal CPU freq %lu - cbound %u", min_cpufreq_sel, nominal_node,
                      cbound);
                if (!imc_set_by_hw) {
            ...
            verbose_master(3,
                           "%sNot IMC freq selection%s: using energy models %u eUFS %u MPI_BOUND = %u Network_IMC %u",
        ...
        if (turbo_set) {
            // TODO: what happen then if user does not activate IMC controller? The policy will also go to
            // SELECT_IMCFREQ state
        ...
        *ready         = EAR_POLICY_TRY_AGAIN;
        ...
        /*  SELECT IMCFREQ */
        verbose_master(2, "%sSelecting IMC freq%s: nominal %d - last CPU %u,ref IMC %u -- current CPU %lu, IMC %u",
                       COL_BLU, COL_CLR, min_pstate, last_cpu_pstate, ref_imc_pstate, curr_pstate, curr_imc_pstate);
        if (!imc_set_by_hw) {
            uint decrease_imc;
            if (ref_imc_pstate != imc_min_pstate[sid]) {
                ...
                decrease_imc = must_decrease_imc(imc_data, curr_pstate, curr_imc_pstate, curr_pstate, ref_imc_pstate,
                                                 my_app, imc_extra_th);

                /*  If we are over the limit after previous IMC update, we decrease the IMC */
                if (decrease_imc && !first_imc_try) {
                    uint must_start_again =
                        must_start(imc_data, curr_pstate, curr_imc_pstate, curr_pstate, ref_imc_pstate, my_app);
                    ...
                    for (sid = 0; sid < imc_devices; sid++) {
                        freqs->imc_freq[sid * IMC_VAL + IMC_MAX] =
                            ear_max(freqs->imc_freq[sid * IMC_VAL + IMC_MAX] - 1, imc_min_pstate[sid]);
                        freqs->imc_freq[sid * IMC_VAL + IMC_MIN] = imc_max_pstate[sid];
                    }
                    sid = 0;
                    ...
                    if (freqs->imc_freq[sid * IMC_VAL + IMC_MAX] > imc_min_pstate[sid])
                        *ready = EAR_POLICY_TRY_AGAIN;
                    else {
                        *ready         = EAR_POLICY_READY;
                        min_time_state = SELECT_CPUFREQ;
            ...
            uint increase_imc = must_increase_imc(imc_data, curr_pstate, curr_imc_pstate, curr_pstate, ref_imc_pstate,
                                                  my_app, imc_extra_th);
            ...
            if (increase_imc) {
                ...
                for (sid = 0; sid < imc_devices; sid++) {
                    freqs->imc_freq[sid * IMC_VAL + IMC_MAX] =
                        ((freqs->imc_freq[sid * IMC_VAL + IMC_MAX] > 0) ? (freqs->imc_freq[sid * IMC_VAL + IMC_MAX] - 1)
                                                                        : 0);
                    freqs->imc_freq[sid * IMC_VAL + IMC_MIN] = imc_max_pstate[sid];
                }
                sid = 0;
                debug("Selecting IMC range %llu - %llu%s", imc_pstates[freqs->imc_freq[sid * IMC_VAL + IMC_MAX]].khz,
                      imc_pstates[freqs->imc_freq[sid * IMC_VAL + IMC_MIN]].khz, COL_CLR);
                ...
                for (sid = 0; sid < imc_devices; sid++) {
                    freqs->imc_freq[sid * IMC_VAL + IMC_MAX] =
                        ((freqs->imc_freq[sid * IMC_VAL + IMC_MAX] < imc_max_pstate[sid])
                             ? freqs->imc_freq[sid * IMC_VAL + IMC_MAX] + 1
                             : imc_max_pstate[sid]);
                    freqs->imc_freq[sid * IMC_VAL + IMC_MIN] = imc_max_pstate[sid];
                }
                sid = 0;
                ...
                if (freqs->imc_freq[sid * IMC_VAL + IMC_MAX] < max_policy_imcfreq_ps)
                    *ready = EAR_POLICY_TRY_AGAIN;
                else {
                    *ready         = EAR_POLICY_READY;
                    min_time_state = SELECT_CPUFREQ;
    ...
    if (cpu_ready && use_energy_models) {
        compute_policy_savings(cpu_energy_model, sig, freqs, &mt_freq_dom);
    }
...
static state_t int_policy_ok(polctx_t *c, signature_t *curr_sig, signature_t *last_sig, int *ok)
{
    ...
    *ok = 1;
    ...
    if (module_mpi_is_enabled() && enable_load_balance && use_energy_models) {
        ...
        if (state_ok(
            ...
            if (unbalance) {
                ...
                double median;
                mpi_support_select_critical_path(critical_path, percs_mpi, num_processes, mean, &median, &max_mpi,
                                                 &min_mpi);
                ...
                if (!first_time_unbalanced && *ok == 1) {

                    mpi_support_verbose_perc_mpi_stats(2, last_percs_mpi, num_processes, last_mpi_stats_perc_mpi_mean,
                                                       last_mpi_stats_perc_mpi_median, last_mpi_stats_perc_mpi_sd,
                                                       last_mpi_stats_perc_mpi_mag);
                    ...
                    if (mpi_changed) {
                        verbose_master(2, "%sMPI stats changed. Starting again%s", COL_RED, COL_CLR);
                        *ok = 0;
                    }
                } else if (*ok) {
                    first_time_unbalanced = 0;
                }
    ...
    if (*ok == 1) {
        ...
        if (use_energy_models && freq_per_core == 0 &&
            signatures_different(curr_sig, last_sig, "min_time", &cpu_energy_model, min_pstate)) {
            verbose_master(2, "%sSignature is too different from the one used for CPU freq. Starting again%s", COL_RED,
                           COL_CLR);
            *ok = 0;
    ...
    if (*ok == 0) {
        ...
        if (curr_sig->sig_ext->saving < 0) {
            *ok = 0;
            ...
            verbose(2, "%sMIN TIME POLICY OK%s We are not saving energy.", COL_RED, COL_CLR);
        }
#endif
    }
...
state_t policy_ok(polctx_t *c, signature_t *curr_sig, signature_t *last_sig, int *ok /*, node_freqs_t *freqs */)
{
    *ok = 0;
    if ((c == NULL) || (curr_sig == NULL) || (last_sig == NULL))
        return EAR_ERROR;
    return int_policy_ok(c, curr_sig, last_sig, ok /*, freqs */);
}
...
state_t policy_io_settings(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs)
{
    ...
    const uint min_time_io_veb_lvl = 2; // Used to control the verbose level in this function
    verbose_master(min_time_io_veb_lvl, "%sMIN TIME%s CPU I/O settings", COL_BLU, COL_CLR);

    cpu_ready = EAR_POLICY_READY;

    for (uint i = 0; i < num_processes; i++) {
        freqs->cpu_freq[i] = min_time_def_freqs.cpu_freq[i];
        ...
        } else if (mt_top.vendor == VENDOR_INTEL) {
            freqs->cpu_freq[i] = frequency_pstate_to_freq(c->num_pstates - 1);
        }
        ...
        verbose_master(min_time_io_veb_lvl, "%s.[%d] [I/O config: %u, I/O busy waiting config: %u] %s", node_name, i,
                       io_bound, busy_waiting, ssig_buff);
...
state_t policy_busy_wait_settings(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs)
{
    ...
    verbose_master(min_ener_bw_veb_lvl, "%sMIN TIME%s CPU Busy waiting settings", COL_BLU, COL_CLR);
...
static state_t policy_no_models_go_next_mt(int curr_pstate, int *ready, node_freqs_t *freqs, int min_pstate)
{
    ...
    if (curr_pstate == min_pstate) {
        ...
        *best_freq  = frequency_pstate_to_freq(next_pstate);
        debug("Not ready: next %d freq %lu", next_pstate, *best_freq);
    }
    return EAR_SUCCESS;
}
                                               ...
                                               double cpi_ref, double gbs_ref, double freq_ref, double min_eff_gain)
{
    ...
    return (1 - below_perf_min_benefit(time_ref, time_curr, cpi_ref, cpi_curr, gbs_ref, gbs_curr, freq_ref, freq_curr,
                                       min_eff_gain));
#if 0
