...
state_t policy_init(polctx_t *c)
{
    ...
    verbose_master(2, "%sMIN TIME%s Using energy models: %u", COL_BLU, COL_CLR, use_energy_models);
		...
		if (module_mpi_is_enabled()) {
            ...
            verbose_master(2, "%sMIN TIME%s Busy waiting MPI: %u",
                           COL_BLU, COL_CLR, block_type == BUSY_WAITING_BLOCK);
        }
        ...
        verbose_master(2, "      --- eUFS configuration ---\n"
        ...
        verbose_master(2, "%sMIN TIME%s Nominal frequency of the node: %lu kHz",
                       COL_BLU, COL_CLR, nominal_node);
    ...
    if ((min_time_state == SELECT_CPUFREQ) || (eUFS == 0))
    {
        ...
        for (uint i = 0; i < nump; i++)
        {
            ...
            if (use_energy_models)
            {
                ...
                debug("time ref %f | curr pstate %lu | best pstate %lu | best freq %lu | def freq %lu | new freq %lu", time_ref, curr_pstate, best_pstate, best_freq, def_freq, new_freq[i]);
                if (critical_path[i])
                {
    ...
    else if ((min_time_state == SELECT_IMCFREQ) && eUFS)
    {
        ...
        else
        {
            ...
            else
            {
                for (sid = 0; sid < imc_devices; sid++)
                {
                    freqs->imc_freq[sid * IMC_VAL + IMC_MAX] = ((freqs->imc_freq[sid * IMC_VAL + IMC_MAX] < imc_max_pstate[sid]) ? freqs->imc_freq[sid * IMC_VAL + IMC_MAX] + 1 : imc_max_pstate[sid]);
                    freqs->imc_freq[sid * IMC_VAL + IMC_MIN] = imc_max_pstate[sid];
                }
                sid = 0;
...
static state_t int_policy_ok(polctx_t *c, signature_t *curr_sig, signature_t *last_sig, int *ok)
{
    ...
    *ok = 1;
    ...
    if (*ok == 1)
    {
        ...
        if (use_energy_models && freq_per_core == 0 && signatures_different(curr_sig, last_sig, "min_time", &cpu_energy_model, min_pstate)) 
        {
...
state_t policy_io_settings(polctx_t *c, signature_t *my_sig, node_freqs_t *freqs)
{
    ...
    for (uint i = 0; i < num_processes; i++)
    {
        ...
        verbose_master(min_time_io_veb_lvl, "%s.[%d] [I/O config: %u, I/O busy waiting config: %u] %s",
