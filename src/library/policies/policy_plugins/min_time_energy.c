...
state_t policy_apply(polctx_t *c, signature_t *sig, node_freqs_t *freqs, int *ready)
{
    ...
    verbose_master(2, "%sdebug.Min_time_to_solution...........starts.......use models =%d %s", COL_GRE,
                   use_energy_models, COL_CLR);
    ...
    if (pstate_freqtops_upper((pstate_t *) imc_pstates, imc_num_pstates, curr_imc_freq, &tmp_pstate) == EAR_ERROR) {
        verbose_master(2,
    ...
    if ((min_time_state == SELECT_CPUFREQ) || (eUFS == 0)) {
        ...
        for (uint i = 0; i < nump; i++) {
            ...
            if (use_energy_models) {
                ...
                verbose_master(2, "Time ref %lf Power ref %lf Freq ref %lu min_pstate %d max_freq %lu", time_ref,
                               power_ref, best_freq, min_pstate, c->app->max_freq);
...
#define FORCE_DEFAULT 0
#if FORCE_DEFAULT
                for (uint lp = 0; lp < nump; lp++)
                    new_freq[i] = min_time_def_freqs.cpu_freq[0];
                ...
                if (new_freq[i] == min_time_def_freqs.cpu_freq[0]) {
                    compute_cpu_freq_min_energy(my_app, cpu_energy_model, curr_freq, time_ref, power_ref, 0.05,
                                                curr_pstate, curr_pstate, c->num_pstates, &new_freq[i]);
        ...
        *ready         = EAR_POLICY_TRY_AGAIN;
        ...
        /*  SELECT IMCFREQ */
        verbose_master(2, "%sSelecting IMC freq%s: nominal %d - last CPU %u,IMC %u -- current CPU %lu, IMC %u", COL_BLU,
                       COL_CLR, min_pstate, last_cpu_pstate, last_imc_pstate, curr_pstate, curr_imc_pstate);
        if (!imc_set_by_hw) {
            uint decrease_imc;
    ...
    *ok = 1;
    ...
    if (*ok == 1) {
        ...
        if (use_energy_models && freq_per_core == 0 &&
            signatures_different(curr_sig, last_sig, "min_time_energy", &cpu_energy_model, min_pstate)) {
...
state_t policy_ok(polctx_t *c, signature_t *curr_sig, signature_t *last_sig, int *ok)
{
    *ok = 0;
    if ((c == NULL) || (curr_sig == NULL) || (last_sig == NULL))
        return EAR_ERROR;
    return int_policy_ok(c, curr_sig, last_sig, ok);
}
