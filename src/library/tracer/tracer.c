...
const char *trace_syms_nam[] = {"traces_init",
                                "traces_end",
                                "traces_start",
                                "traces_stop",
                                "traces_frequency",
...
void traces_init(settings_conf_t *conf, char *app, int global_rank, int local_rank, int nodes, int mpis, int ppn)
{
    ...
    if (state_fail(utils_build_valid_plugin_path(tracer_path, sizeof(tracer_path), "tracer", traces_plugin, conf))) {
    ...
    if (state_fail(st = symplug_open(tracer_path, (void **) &trace_syms_fun, trace_syms_nam, trace_syms_n))) {
        verbose_error("Unable to load tracer plugin %s (%d)", state_msg, st);
        trace_plugin = 0;
        goto quit;
    }

    verbose_info("Tracer plugin '%s%s%s' has been loaded.", COL_GRE, tracer_path, COL_CLR);
