...
state_t cpu_power_model_load(settings_conf_t *libconf, architecture_t *arch_desc, uint load_dummy)
{
    ...
    state_t st = symplug_open(cpu_pwr_model_path, (void **) &cpu_power_models_syms_fun, cpu_power_models_syms_nam,
                              cpu_power_models_funcs_n);
...
{
    if (!cpu_power_model_loaded)
        return EAR_ERROR;
    if (cpu_power_models_syms_fun.status != NULL)
        return cpu_power_models_syms_fun.status();
    return EAR_SUCCESS;
}
