...
state_t energy_model_init(char *ear_coeffs_path, char *ear_tmp_path, architecture_t *arch_desc)
{
    ...
    if (hack_file == NULL) {
        xsnprintf(coeffs_path, sizeof(coeffs_path), "%s/island%d/coeffs.imc_perc.%s", ear_coeffs_path,
                  system_conf->island, system_conf->tag);
    ...
    if ((spr_imc_perc_model_init) && (valid_range(from, to))) {
        ...
        if (coeff->available) {
            *ptime = sign->time * (coeff->E * sign->avg_imc_f + coeff->F);
            debug("Time %lf = Time(%lf) * (E x MemF(%lf x %lu) + F(%lf))", *ptime, sign->time, coeff->E,
                  sign->avg_imc_f, coeff->F);
        } else {
            *ptime = 0;
            st     = EAR_ERROR;
        }
...
state_t energy_model_project_power(signature_t *sign, ulong from, ulong to, double *ppower)
{
    ...
    if ((spr_imc_perc_model_init) && (valid_range(from, to))) {
        ...
        if (coeff->available) {
            ...
            *ppower      = (coeff->A * power) + (coeff->B * sign->avg_imc_f) + (coeff->C);
            debug("Power %lf = AxPower(%lf x %lf) + BxMemF(%lf x %lu) + C(%lf)", *ppower, coeff->A, power, coeff->B,
                  sign->avg_imc_f, coeff->C);
        } else {
            *ppower = 0;
            st      = EAR_ERROR;
        }
...
{
    if (valid_range(from, to) && coefficients[from][to].available)
        return 1;
    else
        return 0;
}
...
uint energy_model_any_projection_available()
{
    ...
    for (uint ps = 0; ps < num_pstates && !ready; ps++) {
        for (uint pt = 0; pt < num_pstates && !ready; pt++) {
            if ((ps != pt) && (energy_model_projection_available(ps, pt) == EAR_SUCCESS))
                ready = 1;
        }
    }
    if (ready)
