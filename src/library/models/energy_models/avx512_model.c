    ...
    if (basic_model_init && valid_range(from_ps, to_ps)) {
        ...
        if (coeff->available) {
            ...
            *proj_time = ctime;
            ...
            debug("time projection from %lu to %lu time_nosimd %lf time avx512 %lf perc_avx512 %lf TIME %lf", from_ps,
                  to_ps, time_nosimd, time_avx512, perc_avx512, ctime);
        } else {
            *proj_time = 0;
            st         = EAR_ERROR;
        }
...
state_t energy_model_project_power(signature_t *signature, ulong from_ps, ulong to_ps, double *proj_power)
{
    ...
    if (basic_model_init && valid_range(from_ps, to_ps)) {
        ...
        if (coeff->available) {
            ...
            *proj_power = cpower;
            debug("power projection from %lu to %lu power_nosimd %lf power avx512 %lf perc_avx512 %lf POWER %lf",
                  from_ps, to_ps, power_nosimd, power_avx512, perc_avx512, cpower);
        } else {
...
static double avx512_vpi(signature_t *my_app)
{
    return (double) ((my_app->FLOPS[3] / (ullong) 16) + (my_app->FLOPS[7] / (ullong) 8)) /
           (double) my_app->instructions;
}
...
static double project_time(coefficient_t *coeff, signature_t *signature, ulong freq_src, ulong freq_dst)
{
    ...
    debug("((time %lf new_cpi %lf) / orig_cpi %lf) * (freq src %lu / freq_dst %lu)", signature->time, pcpi,
          signature->CPI, freq_src, freq_dst);
...
static uint projection_available(ulong from_ps, ulong to_ps)
{
    // verbose_master(2, "projection_available test: from %lu to %lu aval %lu", from_ps, to_ps,
    // coefficients[from_ps][to_ps].available);
