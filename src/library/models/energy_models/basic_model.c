...
state_t energy_model_project_time(signature_t *signature, ulong from_ps, ulong to_ps, double *proj_time)
{
    ...
    if ((basic_model_init) && (valid_range(from_ps, to_ps))) {
        ...
        if (coeff->available) {
            ...
            *proj_time = ((signature->time * proj_cpi) / signature->CPI) * (freq_src / freq_dst);
        } else {
            *proj_time = 0;
            st         = EAR_ERROR;
        }
...
state_t energy_model_project_power(signature_t *signature, ulong from_ps, ulong to_ps, double *proj_power)
{
    ...
    if ((basic_model_init) && (valid_range(from_ps, to_ps))) {
        coeff = &coefficients[from_ps][to_ps];
        if (coeff->available) {
            *proj_power = (coeff->A * signature->DC_power) + (coeff->B * signature->TPI) + (coeff->C);
            debug("Power %lf = %lf x %lf + %lf x %lf + %lf", *proj_power, coeff->A, signature->DC_power, coeff->B,
                  signature->TPI, coeff->C);
        } else {
            *proj_power = 0;
            st          = EAR_ERROR;
        }
