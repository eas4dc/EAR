...
state_t energy_model_project_time(signature_t *signature, ulong from_ps, ulong to_ps, double *proj_time)
{
  ...
  if ((imc_linear_model_init) && (valid_range(from_ps,to_ps))){
    ...
    if (coeff->available){
      *proj_time= (coeff->D * signature->time) +
       (coeff->E * signature->avg_imc_f) +
       (coeff->F);
       debug("Time %lf = %lf x %lf + %lf x %lf + %lf", *proj_time, coeff->D, signature->time, coeff->E, signature->avg_imc_f, coeff->F);
    }else{
      *proj_time=0;
      st=EAR_ERROR;
    }
...
state_t energy_model_project_power(signature_t *signature, ulong from_ps, ulong to_ps, double *proj_power)
{
	...
	if ((imc_linear_model_init) && (valid_range(from_ps,to_ps))){
		...
		if (coeff->available){
		   ...
		   (coeff->B * signature->avg_imc_f) +
		   (coeff->C);
       debug("Power %lf = %lf x %lf + %lf x %lf + %lf", *proj_power, coeff->A, signature->DC_power, coeff->B, signature->avg_imc_f, coeff->C);
		}else{
			*proj_power=0;
			st=EAR_ERROR;
		}
