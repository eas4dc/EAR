...
static const char *energy_names[] = {"energy_datasize",      "energy_frequency",
                                     "energy_units",         "energy_accumulated",
                                     "energy_to_str",        "energy_dc_read",
                                     "energy_data_alloc",    "energy_data_is_null",
                                     "energy_data_copy",     "energy_not_privileged_init",
                                     "energy_is_privileged", "energy_dispose_not_priv"};
...
state_t energy_node_load(char *path, int eard)
{
    ...
    ret = EAR_ERROR;
#else
    ret = symplug_open(path, (void **) &energy_ops, energy_names, energy_nops);
#endif
    if (state_fail(ret)) {
