...
typedef state_t (*dcgmi_lib_init_fn)(uint api, uint all_events, dcgmi_lib_t *dcgmi_data,
...
static char *dcgmi_ev_names[event_id_max] = {"gr_engine_active", "sm_active",     "sm_occupancy",    "tensor_active",
...
state_t dcgmi_lib_load()
{
    if (USE_GPUS && DCGMI) {
        ...
        if (use_dcgmi) {
            ...
            verbose_info("DCGMI metrics %s by environment variable.", dcgmi_lib_enabled ? "enabled" : "disabled");
        }
    ...
    if (buffer && buff_size) {
        ...
        for (int event_idx = 0; event_idx < event_id_max; event_idx++) {
            for (int gpu_idx = 0; gpu_idx < MAX_GPUS_SUPPORTED; gpu_idx++) {
                ...
                snprintf(gpu_event_field, sizeof(gpu_event_field), ";GPU%d_%s", gpu_idx, dcgmi_ev_names[event_idx]);

                strncat(buffer, gpu_event_field, buff_size - strlen(buffer) - 1);
            }
        }
        return EAR_SUCCESS;
    } else {
        ...
        for (int event_idx = 0; event_idx < event_id_max; event_idx++) {
            ...
            for (int gpu_idx = 0; gpu_idx < MAX_GPUS_SUPPORTED; gpu_idx++) {
                ...
                if (event_set >= 0 && dcgmi_sig->gpu_cnt > gpu_idx) {
                    snprintf(set_event_metric_buff, sizeof(set_event_metric_buff), ";%f",
                             dcgmi_sig->event_metrics_sets[event_set][gpu_idx].values[event_pos]);
                } else {
                    /* Else, we put a zero */
    ...
    for (uint set = 0; set < dcgmi_data.dcgmi_set_count; set++) {
        ...
        for (uint gpu_idx = 0; gpu_idx < gpuprof_api.devs_count; gpu_idx++) {
            for (uint event_idx = 0; event_idx < dcgmi_data.dcgmi_set_sizes[set]; event_idx++) {
                dcgmi_sig->event_metrics_sets[set][gpu_idx].values[event_idx] =
                    agg[gpu_idx].values[event_idx] / elapsed;
            }
        }
        ...
        if (VERB_ON(DCGMI_MET_LVL)) {
            ...
            if (event_metrics_sets_str) {
                ...
                verbose_master(DCGMI_MET_LVL, "DCGMI set %u app metrics %s elapsed %.3lf sec", set,
                               event_metrics_sets_str, elapsed);
            }
...
static state_t static_init()
{
    ...
    debug("Allocating %d sets", dcgmi_data.dcgmi_set_count);
...
static void aggregate_gpuprof_data(gpuprof_t *gpuprof_agg, gpuprof_t *gpuprof_src)
{
    if (gpuprof_src && gpuprof_agg) {
        ...
        for (int dev = 0; dev < gpuprof_api.devs_count; dev++) {
            ...
            debug("Device %d hash: %s, #events: %u, #samples + %f = %f", dev, gpuprof_agg[dev].hash,
                  gpuprof_agg[dev].values_count, gpuprof_src[dev].samples_count, gpuprof_agg[dev].samples_count);
