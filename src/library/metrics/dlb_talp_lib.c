...
static void report_talp_node_metrics(dlb_node_metrics_t *node_metrics)
{
    if (HAVE_DLB_TALP_H && masters_info.my_master_rank >= 0) {
        if (!talp_report) {
            ...
            int ret =
                snprintf(buff, sizeof buff, "%d-%d-%d-talp.csv", my_job_id, my_step_id, masters_info.my_master_rank);
            ...
            size_t fwrite_ret = fwrite((const void *) talp_report_header, sizeof(char), header_len, talp_report);
        ...
        int ret =
            snprintf(talp_node_metrics_sum, sizeof talp_node_metrics_sum,
                     "\n%s,%d,%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 ",%f,%f", time_buff, node_metrics->node_id,
                     node_metrics->total_useful_time, node_metrics->total_mpi_time, node_metrics->max_useful_time,
                     node_metrics->max_mpi_time, node_metrics->load_balance, node_metrics->parallel_efficiency);
...
#define close_and_unlink(fd, name)                                                                                     \
    ...
    if (fd < 0) {
        ...
        if (master_process) {
            snprintf(err_msg, sizeof(err_msg), "Creating DLB TALP shared memory object (%d).", errno);
        } else {
            snprintf(err_msg, sizeof(err_msg), "Opening DLB TALP shared memory object (%d).", errno);
        }
        return_msg(EAR_ERROR, err_msg);
    }

    /* Shared memory map */
    dlb_talp_data = mmap(NULL, sizeof(earl_talp_shd_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
