...
#define APP_NODE 3
#define APP_ACUM                                                                                                       \
    4 // Used to store the accumulated values for metrics which also use
      // LOO_NODE position
#define SIG_BEGIN 0
#define SIG_END   1
...
const metrics_gpus_t *metrics_gpus_get(uint api)
{
    switch (api) {
        ...
        case MET_GPU:
            ...
            _met_gpu.api        = met_gpu.api;
            _met_gpu.ok         = met_gpu.ok;
            _met_gpu.devs_count = met_gpu.devs_count;
            return &_met_gpu;
        default:
            return NULL;
    }
...
{
    if (!met_cpi.ok || !met_bwidth.ok || !met_flops.ok)
        return 0;
    return 1;
}
...
static void metrics_configuration(topology_t *tp)
{
    ...
    if (!mgt_cpufreq.ok) {
        ...
        if (system_conf->def_freq == 0 && earl_config_dummy) {
            ...
            verbose_master(ERROR_VERB, "Using base_freq cpufreq_list[%u] = %llu based on MGT cpufreq", nominal,
                           pst_list[nominal].khz);
...
#define sa(function) function;
    ...
    debug("flops_load");
    sa(flops_load(tp, SCOPE_PROCESS));
    debug("cache_load");
    sa(cache_load(tp, eard));
    debug("cpi_load");
    sa(cpi_load(tp, SCOPE_PROCESS));
    ...
    cpufreq_get_info(&met_cpufreq);
    imcfreq_get_info(&met_imcfreq);
    bwidth_get_info(&met_bwidth);
    flops_get_info(&met_flops);
    temp_get_info(&met_temp);
    cpi_get_info(&met_cpi);

    debug("CPU APIs ready");
    if (met_cpufreq.api == API_DUMMY)
        estimate_met_cpuf = 1;
    if (met_bwidth.api == API_DUMMY)
        estimate_gbs = 1;

    if ((met_bwidth.api == API_DUMMY) || (met_cpi.api == API_DUMMY) || (met_flops.api == API_DUMMY))
        validate_cpu_busy = 0;
    ...
    sa(gpu_get_devices(NULL, &met_gpu.devs_count));
#if USE_CUPTI
    if (read_cupti_metrics) {
    ...
    // Available lists
    sa(mgt_cpufreq_get_available_list(no_ctx, (const pstate_t **) &mgt_cpufreq.avail_list, &mgt_cpufreq.avail_count));
    sa(mgt_imcfreq_get_available_list(no_ctx, (const pstate_t **) &mgt_imcfreq.avail_list, &mgt_imcfreq.avail_count));
    ...
    if (VERB_ON(CPUPRIO_VERB)) {
        verbose_master(CPUPRIO_VERB, "[CPUPRIO] %s / %u priorities / %u devices.", mgt_cpuprio.ok ? "OK" : "DUMMY",
                       mgt_cpuprio.avail_count, mgt_cpuprio.devs_count);

        mgt_cpuprio_data_tostr((cpuprio_t *) mgt_cpuprio.avail_list, (uint *) mgt_cpuprio.current_list, buffer,
                               SZ_BUFFER_EXTRA);
        verbose_master(CPUPRIO_VERB, "CPUPRIO list of available priorities\n %s", buffer);
    }

#if USE_GPUS
    sa(mgt_gpu_freq_list(no_ctx, (const ulong ***) &mgt_gpu.avail_list, (const uint **) &mgt_gpu.avail_count));
    ...
    flops_data_alloc(&flops_read1[LOO]);
    flops_data_alloc(&flops_read1[APP]);
    flops_data_alloc(&flops_read2[LOO]);
    flops_data_alloc(&flops_read2[APP]);
    flops_data_alloc(&flops_diff[LOO]);
    flops_data_alloc(&flops_diff[APP]);
    ...
    cpi_data_alloc(&cpi_read1[LOO]);
    cpi_data_alloc(&cpi_read1[APP]);
    cpi_data_alloc(&cpi_read2[LOO]);
    cpi_data_alloc(&cpi_read2[APP]);
    cpi_data_alloc(&cpi_diff[LOO]);
    cpi_data_alloc(&cpi_diff[APP]);
...
#if METRICS_OVH
    overhead_subscribe("partial_stop", &id_ovh_partial_stop);
    overhead_subscribe("partial_start", &id_ovh_partial_start);
    overhead_subscribe("compute_signature_data", &id_ovh_compute_data);
    overhead_subscribe("DC_energy", &id_ovh_dc_energy);
    overhead_subscribe("RAPL", &id_ovh_rapl);
    overhead_subscribe("AVG_CPU_freq", &id_ovh_cpufreq);
    overhead_subscribe("AVG_IMC_freq", &id_ovh_imcfreq);
#endif
...
static void metrics_global_start()
{
    ...
    if (master) {
        /* Avg CPU freq */
        if (state_fail(s = cpufreq_read(cpufreq_read1[APP]))) {
        ...
        if (mgt_imcfreq.ok) {
            debug("imcfreq_read");
            if (state_fail(s = imcfreq_read(imcfreq_read1[APP]))) {
        ...
        if (state_ok(gpu_read(gpu_metrics_read1[APP]))) {
        ...
        /* To be used in partial_start */
        io_old_copy(&metrics_io_end[LOO_NODE], &metrics_io_init[APP_NODE]);
    ...
    /* OS procs statistics */
    metric_cond(proc_ps_ok, proc_stat_read(&proc_ctx, &metrics_proc_init[APP]), VERB_PROCPS,
                "proc_stat_read failed in global start");
    metric_cond(proc_ps_ok, proc_stat_data_copy(&metrics_proc_end[LOO], &metrics_proc_init[APP]), VERB_PROCPS,
                "proc_stat_data_copy failed in global start");
    ...
    /* Per process IO data */
    if (io_old_read(&ctx_io, &metrics_io_init[APP]) != EAR_SUCCESS) {
        verbose_warning_master("I/O data not available.");
    }
    io_old_copy(&metrics_io_end[LOO], &metrics_io_init[APP]);
...
static void metrics_global_stop()
{
    ...
    if (master) {
        ...
        if (module_mpi_is_enabled()) {
            read_diff_node_mpi_info(lib_shared_region, sig_shared_region, metrics_mpi_info[APP],
                                    metrics_last_mpi_info[APP]);
#if MPI_CALL_TYPES
            ...
            read_diff_node_mpi_types_info(lib_shared_region, sig_shared_region, metrics_mpi_calls_types[APP],
                                          metrics_last_mpi_calls_types[APP]);
#endif
        }

        /* Avg CPU frequency */
        if (state_fail(s = cpufreq_read(cpufreq_read2[APP]))) {
            verbose_warning("cpufreq data read failed in global_stop.");
        }
        cpufreq_data_diff(cpufreq_read2[APP], cpufreq_read1[APP], cpufreq_diff, &cpufreq_avrg[APP]);
        ...
        /* AVG IMC frequency */
        if (mgt_imcfreq.ok) {
            if (state_fail(s = imcfreq_read(imcfreq_read2[APP]))) {
                error_lib("IMC freq. data read in global stop.");
            }
            imcfreq_data_diff(imcfreq_read2[APP], imcfreq_read1[APP], imcfreq_diff[APP], &imcfreq_avrg[APP]);

            verbose(VEARL_INFO2, "Avg. IMC frequency: %.2f.", (float) imcfreq_avrg[APP] / 1000000.0);

            for (i = 0; i < mgt_imcfreq.devs_count; i++) {
                imc_max_pstate[i] = imc_num_pstates - 1;
                imc_min_pstate[i] = 0;
            }
            if (state_fail(s = mgt_imcfreq_set_current_ranged_list(no_ctx, imc_min_pstate, imc_max_pstate))) {
...
#if USE_GPUS
        /* GPU */
        if (state_ok(gpu_read(gpu_metrics_read2[APP]))) {

            gpu_data_diff(gpu_metrics_read2[APP], gpu_metrics_read1[APP], gpu_metrics_diff[APP]);
        ...
        if (state_fail(metrics_compute_total_io(&metrics_io_end[APP_NODE]))) {
            ...
            verbose_warning("Node total I/O could not be read (%s), working with"
                            " the acumulated data read until now.",
                            state_msg);
        } else {
            io_old_diff(&metrics_io_diff[APP_NODE], &metrics_io_init[APP_NODE], &metrics_io_end[APP_NODE]);
        }
    } // master

    /* Per-process IO */
    if (state_fail(io_old_read(&ctx_io, &metrics_io_end[APP]))) {
        verbose_warning_master("I/O data not available.");
    } else {
        io_old_diff(&metrics_io_diff[APP], &metrics_io_init[APP], &metrics_io_end[APP]);
    }

    if (VERB_ON(IO_VERB)) {
        int fd;
        io_old_tostr(&metrics_io_diff[APP], io_info, sizeof(io_info));
        if (verb_path != NULL) {
    ...
    bwidth_read_diff(bwidth_read2[APP], bwidth_read1[APP], NULL, &bwidth_cas[APP], NULL);
    if (master)
        lib_shared_region->cas_counters = bwidth_cas[APP];
    flops_read_diff(flops_read2[APP], flops_read1[APP], NULL, NULL);
    cache_read_diff(&cache_read2[APP], &cache_read1[APP], &cache_diff[APP], &cache_bwidth[APP]);
    cpi_read_diff(cpi_read2[APP], cpi_read1[APP], cpi_diff[APP], &cpi_avrg[APP]);

    metric_cond(
        proc_ps_ok,
        proc_stat_read_diff(&proc_ctx, &metrics_proc_end[APP], &metrics_proc_init[APP], &metrics_proc_diff[APP]),
        VERB_PROCPS, "EARL proc_stat_read_diff failed in global_stop");

#if USE_CUPTI
    if (read_cupti_metrics) {
        if (state_fail(
                gpuproc_read_diff(no_ctx, proc_gpu_data_end[APP], proc_gpu_data_init[APP], proc_gpu_data_diff[APP]))) {
        ...
        verbose(VERBOSE_CUPTI, "GPU_PROC APP DATA for [%d][%d] : %s", masters_info.my_master_rank, my_node_id,
                gpu_proc_buffer);
    }
#endif
}
...
{
    debug("metrics_partial_start starts %p %p %lu", metrics_ipmi[LOO], aux_energy, node_energy_datasize);
    ...
    io_old_copy(&metrics_io_init[LOO], &metrics_io_end[LOO]);
    debug("IO read done");

    // Proc stat
    metric_cond(proc_ps_ok, proc_stat_data_copy(&metrics_proc_init[LOO], &metrics_proc_end[LOO]), VEARL_ERROR,
    ...
    if (master) {
        ...
        /* Per NODE IO data */
        io_old_copy(&metrics_io_init[LOO_NODE], &metrics_io_end[LOO_NODE]);
...
static int metrics_partial_stop(uint where)
{
    ...
    if ((where == SIG_END) && (c_energy == 0) && (master)) {
        ...
        if (dispose)
            fprintf(stderr, "partial stop and EAR_NOT_READY\n");
        if (!EARL_LIGHT)
            return EAR_NOT_READY;
    }
    ...
    verbose_master(2, "EARL[%d][%d] %f = %lu mJ (units = %d) / %lf usecs", ear_my_rank, getpid(), c_power, c_energy,
                   node_energy_units, (float) c_time);
    ...
    if (master) {
#if USE_GPUS
        if (state_ok(gpu_read(gpu_metrics_read2[LOO]))) {
    ...
    if (master) {
        if ((c_power > system_conf->min_sig_power) && (c_power < system_conf->max_sig_power)) {
            ...
            verbose_master(ERROR_VERB,
                           "%sWARNING%s Computed power (%lf) is not correct [%lf-%lf]. Estimate power enabled \n",
    ...
    /* Per process IO data */
    if (io_old_read(&ctx_io, &metrics_io_end[LOO]) != EAR_SUCCESS) {
        verbose_warning_master("I/O data not available");
    }
    io_old_diff(&metrics_io_diff[LOO], &metrics_io_init[LOO], &metrics_io_end[LOO]);

    /* Process statistics */
    metric_cond(
        proc_ps_ok,
        proc_stat_read_diff(&proc_ctx, &metrics_proc_end[LOO], &metrics_proc_init[LOO], &metrics_proc_diff[LOO]),
        VERB_PROCPS, "EARL proc_stat_read_diff failed in partial_stop");
    ...
    if (master) {
        /* MPI statistics */
        if (module_mpi_is_enabled()) {
            read_diff_node_mpi_info(lib_shared_region, sig_shared_region, metrics_mpi_info[LOO],
                                    metrics_last_mpi_info[LOO]);
#if MPI_CALL_TYPES
            ...
            read_diff_node_mpi_types_info(lib_shared_region, sig_shared_region, metrics_mpi_calls_types[LOO],
                                          metrics_last_mpi_calls_types[LOO]);
#endif
        }
        ...
        /* Avg CPU freq */
        if (state_fail(s = cpufreq_read(cpufreq_read2[LOO]))) {
...
#endif
        cpufreq_data_diff(cpufreq_read2[LOO], cpufreq_read1[LOO], cpufreq_diff, &cpufreq_avrg[LOO]);
        // Simple verbose
        if (verb_level >= 3) {
...
#endif
            if (state_fail(s = imcfreq_read(imcfreq_read2[LOO]))) {
                verbose_warning_master("Error IMC freq. data read in partial stop");
            }
#if METRICS_OVH
            overhead_stop(id_ovh_imcfreq);
#endif
            imcfreq_data_diff(imcfreq_read2[LOO], imcfreq_read1[LOO], imcfreq_diff[LOO], &imcfreq_avrg[LOO]);
     ...
     */
    // New metrics
    bwidth_read_diff(bwidth_read2[LOO], bwidth_read1[LOO], bwidth_diff[LOO], &bwidth_cas[LOO], NULL);
    if (master)
        lib_shared_region->cas_counters = bwidth_cas[LOO];
    flops_read_diff(flops_read2[LOO], flops_read1[LOO], flops_diff[LOO], NULL);
    cache_read_diff(&cache_read2[LOO], &cache_read1[LOO], &cache_diff[LOO], &cache_bwidth[LOO]);
    cpi_read_diff(cpi_read2[LOO], cpi_read1[LOO], cpi_diff[LOO], &cpi_avrg[LOO]);

#if USE_CUPTI
    if (read_cupti_metrics) {
        if (state_fail(
                gpuproc_read_diff(no_ctx, proc_gpu_data_end[LOO], proc_gpu_data_init[LOO], proc_gpu_data_diff[LOO]))) {
            verbose_warning("gpuproc_read_diff failed");
        }
        gpuproc_data_tostr(proc_gpu_data_diff[LOO], gpu_proc_buffer, sizeof(gpu_proc_buffer));
        verbose(VERBOSE_CUPTI, "GPU_PROC LOOP DATA for [%d][%d] : %s", masters_info.my_master_rank, my_node_id,
                gpu_proc_buffer);
    }
#endif
...
{
    ullong fops = 0LLU;
    if (metrics->FLOPS[INDEX_512F] > 0)
        fops += metrics->FLOPS[INDEX_512F] / WEIGHT_512F;
    if (metrics->FLOPS[INDEX_512D] > 0)
        fops += metrics->FLOPS[INDEX_512D] / WEIGHT_512D;
    return fops;
}
...
static void estimate_metrics_not_available_by_processes(signature_t *metrics)
{
    ...
    if (estimate_gbs) {
        ...
        lib_shared_region->node_signature.TPI =
            (metrics->GBS * sig_shared_region[my_node_id].period * 1024 * 1024 * 1024) /
            sig_shared_region[my_node_id].sig.instructions;
        lib_shared_region->cas_counters =
            metrics->GBS * 1024 * 1024 * 1024 / (hw_cache_line_size * sig_shared_region[my_node_id].period);
        ...
        verbose_info2_master("NODE GBS estimated %lf GB/s (period %lf)", metrics->GBS,
                             sig_shared_region[my_node_id].period);
    }
}

static void estimate_metrics_not_available_by_node(uint sign_app_loop_idx, signature_t *metrics, ulong procs,
                                                   float gpup)
{
    ...
    if (estimate_met_cpuf) {
        if (mgt_cpufreq.ok) {
            ...
            for (uint i = 0; i < mgt_cpufreq.devs_count; i++)
                cpufreq_diff[i] = metrics->def_f;
            // Copy the average CPU freq. in the shared signature

            memcpy(lib_shared_region->avg_cpufreq, cpufreq_diff, mgt_cpufreq.devs_count * sizeof(cpufreq_diff[0]));
        }
    }
}
...
static void metrics_compute_signature_data(uint sign_app_loop_idx, signature_t *metrics, uint iterations, ulong procs)
{
    ...
    for (uint i = 0; i < FLOPS_EVENTS; i++)
        metrics->FLOPS[i] = metrics->FLOPS[i] * num_th;

    /* CPI */
    metrics->cycles = cpi_diff[sign_app_loop_idx][0].cycles * num_th;
    ;
    metrics->instructions = ear_max(cpi_diff[sign_app_loop_idx][0].instructions, 1) * num_th;
    ;
    metrics->stalls.fetch_decode = cpi_diff[sign_app_loop_idx][0].stalls.fetch_decode * num_th;
    ;
    metrics->stalls.resources = cpi_diff[sign_app_loop_idx][0].stalls.resources * num_th;
    ;
    metrics->stalls.memory = cpi_diff[sign_app_loop_idx][0].stalls.memory * num_th;
    ;
    ...
    if (master) {
        ...
        lib_shared_region->total_cache_bwidth += cache_bwidth[sign_app_loop_idx];
        sem_post(lib_shared_lock_sem);
        // verbose(2,"CACHE_BW[%u] local %lf total %lf", my_node_id, cache_bwidth[sign_app_loop_idx],
        // lib_shared_region->total_cache_bwidth);
    }
    ...
    /* Per process IO data */
    io_old_copy(&(sig_ext->iod), &metrics_io_diff[sign_app_loop_idx]);
    io_old_copy(&sig_shared_region[my_node_id].sig.iod, &metrics_io_diff[sign_app_loop_idx]);
     ...
     */
    if (proc_ps_ok) {
        sig_shared_region[my_node_id].cpu_util = metrics_proc_diff[sign_app_loop_idx].cpu_util;
    ...
    if (master) {
        ...
        if (module_mpi_is_enabled()) {
            ...
            for (int i = 0; i < lib_shared_region->num_processes; i++) {
                ...
                ullong exec_time_usecs =
                    ear_max(((mpi_information_t *) metrics_mpi_info[sign_app_loop_idx])[i].exec_time, 1);
                ...
                verbose_master(MPI_VERB,
                               ...
                               " | Wall time (us): %llu",
                               i, local_mpi_time_perc, mpi_time_usecs, exec_time_usecs);
            }
        ...
        if (sign_app_loop_idx == LOO)
            acum_ipmi[sign_app_loop_idx] = metrics->DC_power * time_s * node_energy_units;
        ...
        if (!lib_shared_region->global_stop_ready) {
            ...
            verbose_master(3, "Accumulating %.2lu J to the application. Total energy %lu J acum_time %.2lf sec",
                           acum_ipmi[LOO], acum_ipmi[APP], total_acum_time);
        }
    ...
    sig_shared_region[my_node_id].period =
        time_s; // TODO: We can ommit this attribute because the shared signature has this metric.
    ...
    verbose_master(INFO_METRICS + 1, "End compute signature for (l. rank) %d. Period %lf secs", my_node_id,
                   sig_shared_region[my_node_id].period);
#if 0
    ...
    if ((sign_app_loop_idx == APP) && (masters_info.my_master_rank >= 0) && sig_ext->telapsed) {
        ...
        verbose_master(ENERGY_EFF_VERB,
                       "Policy savins[%s]. MR[%d] Average estimated energy savings %.2f. Average Power reduction %.2f. "
                       "Time penalty %.2f. Elapsed %.2f",
                       node_name, masters_info.my_master_rank, avg_esaving, avg_psaving, avg_tpenalty,
                       sig_ext->telapsed);
    ...
    if ((sign_app_loop_idx == APP) && (masters_info.my_master_rank >= 0) && (sig_ext != NULL)) {
        ...
        sig_ext->earl_phase[last_earl_phase_classification].elapsed +=
            timestamp_diff(&currt, &init_last_phase, TIME_USECS);
        ...
        if (last_earl_phase_classification == APP_COMP_BOUND) {
            ...
            if (policy_cpu_bound)
                sig_ext->earl_phase[APP_COMP_CPU].elapsed += elapsed_comp;
            else if (policy_mem_bound)
                sig_ext->earl_phase[APP_COMP_MEM].elapsed += elapsed_comp;
            else
                sig_ext->earl_phase[APP_COMP_MIX].elapsed += elapsed_comp;
        }

        for (uint ph = 0; ph < EARL_BASIC_PHASES; ph++)
            ms_total += sig_ext->earl_phase[ph].elapsed;
        for (uint ph = 0; ph < EARL_BASIC_PHASES; ph++) {
            if (sig_ext->earl_phase[ph].elapsed) {
                verbose_master(PHASES_SUMMARY_VERB, "Phase[%s] percentage %f (%llu msec/%llu msec)", phase_to_str(ph),
                               (ms_total && sig_ext->earl_phase[ph].elapsed)
                                   ? (float) sig_ext->earl_phase[ph].elapsed / (float) ms_total
                                   : 0.0,
                               sig_ext->earl_phase[ph].elapsed, ms_total);
...
int metrics_load(topology_t *topo)
{
    ...
    if (state_ok(io_old_init(&ctx_io, getpid()))) {
    ...
    if (master) {
        ...
        cimc_max_khz =
            (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_MAX_IMCFREQ) : NULL; // Max. IMC freq. (kHz)
        cimc_min_khz =
            (system_conf->user_type == AUTHORIZED) ? ear_getenv(FLAG_MIN_IMCFREQ) : NULL; // Min. IMC freq. (kHz)
        ...
        if (cimc_max_khz != (char *) NULL) {
            ...
            if (state_ok(pstate_freqtops(imc_pstates, imc_num_pstates, tmp_def_imc, &tmp_def_imc_pstate))) {
                for (i = 0; i < mgt_imcfreq.devs_count; i++)
                    imc_min_pstate[i] = tmp_def_imc_pstate.idx;
        ...
        if (cimc_min_khz != (char *) NULL) {
            ...
            if (state_ok(pstate_freqtops(imc_pstates, imc_num_pstates, tmp_def_imc, &tmp_def_imc_pstate))) {
                for (i = 0; i < mgt_imcfreq.devs_count; i++)
                    imc_max_pstate[i] = tmp_def_imc_pstate.idx;
...
void metrics_dispose(signature_t *metrics, ulong procs, uint job_node_ratio)
{
    ...
    wait_time = 0;
    exited    = num_processes_exited(lib_shared_region, sig_shared_region);
    while ((exited != lib_shared_region->num_processes) &&
           (are_signatures_ready(lib_shared_region, sig_shared_region, &num_ready) == 0) &&
...
float metrics_iter_per_second()
{
    ...
    if (!last_sig_elapsed || !last_iterations)
        return 1;
                                         ...
                                         llong *passed_time)
{
    ...
    debug("%s :metrics_compute_signature_finish iterations %u ready %u master_ready %u", node_name, iterations,
          sig_shared_region[my_node_id].ready, lib_shared_region->master_ready);
        ...
        *passed_time = elap_time;
        ...
        if (elap_time < min_time_us && !time_eq) {
            ...
            debug("%s : EAR_NOT_READY because of time %lld, iters %u (start %lld end %lld)\n", node_name, elap_time,
                  iterations, metrics_usecs[LOO], metrics_time());
#if METRICS_OVH
        ...
        if (state_fail(report_misc(&report_id, MPITRACE, (cchar *) &sig_shared_region[my_node_id], 0))) {
            verbose_error("Reporting mpitrace for process");
        }

        // Report node metrics
        if (state_fail(report_misc(&report_id, MPI_NODE_METRICS, (cchar *) metrics->sig_ext,
...
void metrics_app_node_signature(signature_t *master, signature_t *ns)
{
    ...
    if (state_fail(compute_job_node_cpuutil(sig_shared_region, lib_shared_region->num_processes, &total_cpuutil))) {
        verbose_warning("Error computing application's node total CPU util: %s", state_msg);
    } else {
    ...
    if (master) {
        for (uint lp = 0; lp < lib_shared_region->num_processes; lp++) {
            ...
            if (valid_period) {
                ...
                ns->avg_f += (sig_shared_region[lp].sig.accum_avg_f * sig_shared_region[lp].num_cpus) / valid_period;
            }
            tcpus += sig_shared_region[lp].num_cpus;
            accesses += sig_shared_region[lp].sig.accum_mem_access;
            if (valid_period) {
                debug(" p[%u]: Power %lf GBs %lf avg_f %lu dram %lf pck %lf  period %lf", lp,
                      (double) (sig_shared_region[lp].sig.accum_energy / valid_period),
                      (double) (sig_shared_region[lp].sig.accum_mem_access / (valid_period * 1024 * 1024 * 1024)),
                      sig_shared_region[lp].sig.accum_avg_f / (ulong) valid_period, ns->DRAM_power, ns->PCK_power,
                      valid_period);
            }
        }
...
state_t metrics_new_iteration(signature_t *sig)
{
    ...
    if (last_phase_io_bnd || last_phase_bw) {
        ...
        if (last_phase_io_bnd) {
            ...
            io_old_diff(&metrics_io_diff[LOO_NODE], &metrics_io_init[LOO_NODE], &metrics_io_end[LOO_NODE]);
            ...
            verbose_info2_master("IO phase and current IO rate %.2lf (IO? %u)", sig->IO_MBS,
                                 sig->IO_MBS > phases_limits.io_th);
            return EAR_SUCCESS;
        }
        ...
        if (last_phase_bw) {
            ...
            verbose_master(3, "Busy Waiting phase and current CPI %.2lf (BW? %u)", sig->CPI,
                           sig->CPI < phases_limits.cpi_busy_waiting);
            return EAR_SUCCESS;
        }
        ...
        debug("--- Computing flops because of application phase ---\n"
              ...
              "%-26s: %lld\n%-26s: %lld\n"
              "----------------------------------------------------",
              "IO", last_phase_io_bnd, "Busy waiting", last_phase_bw, "Time elapsed", elap_sec, "Last elapsed",
              last_elap_sec);

        /* FLOPS are used to determine the CPU BUSY waiting
         *  WARNING: Currently below code won't be executed never. */
        flops_read_diff(flops_read2[LOO], flops_read1[LOO], flops_diff[LOO], &sig->Gflops);
    ...
    if (last_gpu_state & _GPU_Idle) {
        ...
        /* GPUs */
        if (state_fail(gpu_read(gpu_metrics_busy))) {
...
void metrics_start_cpupower()
{
    if (master && report_power_trace) {
        ...
        if (!traces_are_on()) {
            snprintf(filename, sizeof(filename), "csv_cpu_power.%lu.%lu.%s.csv", application.job.id,
                     application.job.step_id, application.node_id);
...
void metrics_read_cpupower()
{
    if (master && report_power_trace) {
        ...
        energy_cpu_data_diff(no_ctx, metrics_rapl_fine_monitor_start, metrics_rapl_fine_monitor_end,
                             metrics_rapl_fine_monitor_diff);
        ...
        if (!traces_are_on()) {
            snprintf(csv_str, sizeof(csv_str), "%s;%u;%lld;", application.node_id, report_power_id,
                     timestamp_getconvert(TIME_USECS));
        }
        report_power_id++;
        ...
        curr = energy_cpu_compute_power((double) metrics_rapl_fine_monitor_diff[(num_packs * 2) - 1],
                                        (double) elap / 1000.0);
        lpck += (ullong) curr * 1000;
        total += curr;
        if (traces_are_on()) {
...
static state_t metrics_compute_total_io(io_data_t *total)
{
    if (total != NULL) {
        ...
        for (int i = 0; i < lib_shared_region->num_processes; i++) {
            ...
            if (state_ok(io_old_init(&local_io_ctx, sig_shared_region[i].pid))) {

                io_data_t io;
                io_old_read(&local_io_ctx, &io);
...
static void read_metrics_options()
{
    ...
    if (cuse_cupti != NULL)
        read_cupti_metrics = atoi(cuse_cupti);
...
static state_t energy_lib_init(settings_conf_t *conf)
{
    ...
    char *hack_energy_plugin_path = (read_env) ? ear_getenv(HACK_ENERGY_PLUGIN) : NULL;
    ...
    if (hack_energy_plugin_path) {
        ret =
            utils_build_valid_plugin_path(my_plug_path, sizeof(my_plug_path), "energy", hack_energy_plugin_path, conf);
    }
    ...
    verbose_master(2, "Energy plugin %s loaded with success: %u", my_plug_path, state_ok(ret));
