...
static state_t earl_periodic_actions_mpi_opt(void *no_arg)
{
    if (module_mpi_is_enabled() && (ear_mpi_opt || traces_are_on())) {
        ...
        uint new_pstate;
        if (mpi_opt_symbols.periodic != NULL)
            mpi_opt_symbols.periodic(&new_pstate);
        ...
        if (traces_are_on()) {
            ...
            traces_generic_event(ear_my_rank, my_node_id, TIME_BLOCK_CALLS,
                                 (ullong) my_mpi_types[2].mpi_block_call_time);
