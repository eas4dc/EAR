...
#define NEW_CPUFREQ_API                                                                                                \
    1                                      // Enables the usage of the new metrics API,
...
#define states_verbose2_master(msg, ...) verbose_info2_master("States: " msg, ##__VA_ARGS__);
...
static uint id_ovh_ev_sig, id_ovh_stable, id_ovh_compute_signature, id_ovh_policy_apply, id_ovh_update_affinity_mask,
    id_ovh_metrics_new_iteration;
...
/**TODO */
static void compute_perf_count_period(llong n_iters_time, llong n_iters_time_min, uint *loop_perf_count_period,
...
// #define FAKE_PASSED_TIME passed_time = 0;
#define FAKE_PASSED_TIME

static void recompute_lib_period(llong passed_time, uint dynais_used)
{
    double ptime = ((passed_time == 0) ? (double) 0.001 : (double) passed_time);
    if (dynais_used) {
        perf_count_period     = (uint) ((double) perf_accuracy_min_time / ptime) * perf_count_period;
        perf_count_period     = ear_max(perf_count_period, 1);
        perf_count_period_10p = perf_count_period;
        // verbose_master(0,"%s Using new period %u accel %u (iters per second %f)", node_name,
        // perf_count_period, perf_count_period_10p, (float)N_iter / (float)passed_time);
    } else if (ear_guided != TIME_GUIDED) {
        lib_period = (uint) ((double) perf_accuracy_min_time / ptime) * lib_period;
        verbose_master(2, "Library period updated %u.", lib_period);
    }
}
                          ...
                          uint dynais_used)
{
    ...
    verbose_master(3, "%s Iters %u period %u lib_period %u mpi_calls_iter %lu", node_name, iterations,
                   perf_count_period, lib_period, mpi_calls_iter);
    ...
    if (system_conf != NULL) {
        if ((master) && (resched_conf->force_rescheduling)) {
            ...
            verbose_master(
                1, "--- [EAR] Re-scheduling forced by %s ---\n%-20s: %u\n%-20s: %lu\n%-20s: %lu\n%-20s: %lf\n",
                (resched_conf->force_rescheduling == 1 ? "EARD" : "External"), "Curr. state", EAR_STATE, "Max CPUfreq",
                system_conf->max_freq, "Def. CPUfreq", system_conf->def_freq, "Policy th", system_conf->settings[0]);
    ...
    switch (EAR_STATE) {
        ...
        case FIRST_ITERATION:
            ...
            // TODO: Should we use lib_period?
            compute_perf_count_period(comp_N_time, (llong) perf_accuracy_min_time, &perf_count_period,
                                      &perf_count_period_10p);
        ...
        case EVALUATING_LOCAL_SIGNATURE:
            ...
            if (!lib_shared_region->master_ready) {
                ...
                if (dynais_used) {
                    ...
                    // verbose_master(0, "Conditions are Iters %u Iters2 %u", ((iterations - 1) % perf_count_period),
                    // (iterations == 1));
            ...
            if (result == EAR_NOT_READY) {
                ...
                if (!lib_shared_region->master_ready && (passed_time < perf_accuracy_min_time)) {
                    sig_enlarged = 0;
                    // We just set a minimum time
                    recompute_lib_period(passed_time, dynais_used);
                }
                overhead_stop(id_ovh_ev_sig);
                return;
            }
            ...
            */
            ...
            /* VERBOSE */
            if (master) {
                traces_generic_event(ear_my_rank, my_node_id, JOB_POWER,
                                     (ullong) lib_shared_region->job_signature.DC_power * 1000);
                traces_generic_event(ear_my_rank, my_node_id, NODE_POWER,
                                     (ullong) lib_shared_region->node_signature.DC_power * 1000);
            }
            if (traces_are_on()) {
                signature_from_ssig(&tracer_signature, &sig_shared_region[my_node_id].sig);
                tracer_signature.def_f = sig_shared_region[my_node_id].new_freq;
            ...
            state_verbose_signature(&curr_loop, masters_info.my_master_rank, ear_app_name, application.node_id,
                                    iterations, prev_cpufreq_khz, policy_cpufreq_khz, use_case);
        ...
        case SIGNATURE_STABLE:
            ...
            if (master) {
                ...
                signature_copy(&lib_shared_region->job_signature,
                               &curr_loop.signature); // lib_shared_region->job_signature <- curr_loop.signature
            }
            if (master) {
                traces_generic_event(ear_my_rank, my_node_id, JOB_POWER,
                                     (ullong) lib_shared_region->job_signature.DC_power * 1000);
            ...
            state_verbose_signature(&curr_loop, masters_info.my_master_rank, ear_app_name, application.node_id,
                                    iterations, prev_cpufreq_khz, 0, use_case);
            /* END VERBOSE */
            ...
            if (pol_ok) {
                ...
                if ((master) && (traces_are_on() == 0) && !sig_enlarged) {
                    if (dynais_used) {
                        ...
                        perf_count_period_10p = perf_count_period;
                        // verbose_master(0,"%s Using new period %u accel %u ", node_name, perf_count_period,
                        // perf_count_period_10p);
                    } else if (ear_guided != TIME_GUIDED) {
        ...
        case PROJECTION_ERROR:
            ...
            if (master && traces_are_on()) {
                ...
                if (((iterations - 1) % perf_count_period))
                    return;
                prev_cpufreq_khz = ear_frequency; // TODO: Why khz are prev, but pstate is curr?
                curr_pstate      = frequency_closest_pstate(ear_frequency);
                ...
                state_verbose_signature(&curr_loop, masters_info.my_master_rank, ear_app_name, application.node_id,
                                        iterations, prev_cpufreq_khz, 0, "D-PE");
            }
...
static void check_dynais_off(ulong mpi_calls_iter, uint period, uint level, ulong event)
{
    debug("[%d] Dynais: Time %f ov %f limit %f", my_node_id, loop_signature.time,
          ((float) mpi_calls_iter / 1000000.0) * 100.0 / loop_signature.time, MAX_DYNAIS_OVERHEAD_DYN);
    if (loop_signature.time > 0) {
        ...
        if (dynais_enabled == DYNAIS_DISABLED) {
            debug("    --- DynAIS disabled ---\n%15s: %lf s\n%15s: %lu us\n%15s: %lu (%lf %%)\n%15s: %lu\n%15s: %lu\n",
                  "Total time", loop_signature.time, "DynAIS overhead", dynais_overhead_usec, "MPI calls",
                  mpi_calls_iter, dynais_overhead_perc, "Event", event, "Min. time", perf_accuracy_min_time);
        }
#endif
    }
                                      ...
                                      uint *loop_perf_count_period_accel)
{
    ...
    sig_enlarged = 0;
    // verbose_master(0,"%s min_time %llu period %u period_accel %u", node_name, n_iters_time_min,
    // *loop_perf_count_period, *loop_perf_count_period_accel);
}
...
static void accumulate_energy_savings(signature_t *loops, signature_t *apps)
{
    ...
    static char *verb_savs_pens_hdr; // Header for savings and penalties verbose

    sig_ext_t *loop_sig_ext = loops->sig_ext;
    sig_ext_t *app_sig_ext;

    if (loop_sig_ext == NULL)
        return;
...
#if USE_GPUS
    app_sig_ext->gpu_saving += loop_sig_ext->gpu_saving;
    app_sig_ext->gpu_psaving += loop_sig_ext->gpu_psaving;

    verb_savs_pens_hdr = "    --- Savings and penalties ---\n"
                         "Scope GPU E.save  GPU P.save  E.save  P.save  T.pen   T.elapsed\n"
                         "----- ----------  ----------- ------  ------  -----   ---------";
    verbose_master(
        VERB_SAV_PEN,
        "%s\n Loop %-7.2lf %-7.2f %-7.2lf %-7.2f %-7.2f %-9.2lf\n  App %-7.2lf %-7.2f %-7.2lf %-7.2f %-7.2f %-9.2lf\n",
        verb_savs_pens_hdr, loop_sig_ext->gpu_saving, loop_sig_ext->gpu_psaving, loop_sig_ext->saving,
        loop_sig_ext->psaving, loop_sig_ext->tpenalty, loop_sig_ext->elapsed, app_sig_ext->gpu_saving,
        app_sig_ext->gpu_psaving, app_sig_ext->saving, app_sig_ext->psaving, app_sig_ext->tpenalty,
        app_sig_ext->telapsed);
#else
    verb_savs_pens_hdr = "    --- Savings and penalties ---\n"
                         "Scope E.save  P.save  T.pen   T.elapsed\n"
                         "----- ------  ------  -----   ---------";
    verbose_master(VERB_SAV_PEN, "%s\n Loop %-7.2lf %-7.2f %-7.2f %-9.2lf\n  App %-7.2lf %-7.2f %-7.2f %-9.2lf\n",
                   verb_savs_pens_hdr, loop_sig_ext->saving, loop_sig_ext->psaving, loop_sig_ext->tpenalty,
                   loop_sig_ext->elapsed, app_sig_ext->saving, app_sig_ext->psaving, app_sig_ext->tpenalty,
                   app_sig_ext->telapsed);
#endif
}
