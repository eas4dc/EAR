...
/* If set to 1, the structs will be compatible with EAR v5.0 daemons */
#define V5_COMPAT 1
...
typedef struct request {
    ...
    int32_t num_nodes;
    int32_t *nodes;
    req_data_t my_req; // all new variables must be specified after time_code so the first portion aligns with
                       // internal_request_t
} request_t;
...
typedef struct status_node_info {
    uint32_t avg_freq; // In KH
    uint32_t max_freq; // In KH
    uint16_t temp; // In degres, 16 bits should be more than enough (temp ranges from 0 to 100 usually, but 16 is safer)
    uint16_t
        power; // In Watts, 16 bits should be more than enough (power should never peak above 65k for a single node)
    uint16_t rep_power; // In Watts, same as above
} status_node_info_t;
...
#ifdef V5_COMPAT
// Based on the old status, notice the different ordering
typedef struct full_status {
    int ip;
    char ok;
...
typedef struct status {
    ...
    status_node_info_t node;
    app_info_t app;
    char eardbd_connected;
    char ok;
} status_t;

typedef struct app_status {
    int32_t ip;
    int32_t job_id, step_id; // need to be signed so we can set an invalid job_id (-1) to know when there is no job
    uint32_t nodes, master_rank;
    signature_t signature;
} app_status_t;

#endif

typedef struct policy_status {
    int32_t ip;
...
typedef struct power_check {
    uint64_t power;    // this is a cluster-wide accumulator, hence the 64 bits size
    int64_t num_nodes; // this could be smaller, but the compiler will add padding anyway so best be safe
} power_check_t;
...
#define EAR_RC_POLICY_STATUS     603
#define EAR_RC_NODE_PURGE        604
#define EAR_RC_SEND_MESSAGE      605
...
#define EAR_TYPE_POLICY_STATUS 2006
#define EAR_TYPE_EARGM_STATUS  2007
///  |||||
///  vvvvv
/******************* IMPORTANT ***********************/
// UPDATE MAX_TYPE_VALUE FOR EACH NEW TYPE OR THE NEW TYPE WON'T BE ACCEPTED
