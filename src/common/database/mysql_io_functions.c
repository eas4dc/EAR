...
#define AVG_SIGNATURE_QUERY_FULL                                                                                       \
    "INSERT INTO Signatures_avg (DC_power, DRAM_power, PCK_power, EDP,"                                                \
...
#define POWER_SIGNATURE_MYSQL_QUERY                                                                                    \
    "INSERT INTO Power_signatures (DC_power, DRAM_power, PCK_power, EDP, max_DC_power, min_DC_power, "                 \
    "time, avg_f, def_f) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"

#if USE_GPUS
#define GPU_SIGNATURE_MYSQL_QUERY                                                                                      \
    "INSERT INTO GPU_signatures(GPU_power, GPU_freq, GPU_mem_freq, GPU_util, GPU_mem_util, GPU_gflops) VALUES (?, ?, " \
    "?, ?, ?, ?)"
#endif

#if USE_GPUS
#define PERIODIC_METRIC_QUERY_DETAIL                                                                                   \
    "INSERT INTO Periodic_metrics (start_time, end_time, DC_energy, node_id, job_id, step_id, avg_f, temp, "           \
    "DRAM_energy, PCK_energy, GPU_energy)"                                                                             \
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
#else
#define PERIODIC_METRIC_QUERY_DETAIL                                                                                   \
    "INSERT INTO Periodic_metrics (start_time, end_time, DC_energy, node_id, job_id, step_id, avg_f, temp, "           \
    "DRAM_energy, PCK_energy)"                                                                                         \
    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
#endif

#define PERIODIC_METRIC_QUERY_SIMPLE                                                                                   \
    "INSERT INTO Periodic_metrics (start_time, end_time, DC_energy, node_id, job_id, step_id)"                         \
    "VALUES (?, ?, ?, ?, ?, ?)"

#define PERIODIC_AGGREGATION_MYSQL_QUERY                                                                               \
    "INSERT INTO Periodic_aggregations (DC_energy, start_time, end_time, eardbd_host) VALUES (?, ?, ?, ?)"

#define EAR_EVENT_MYSQL_QUERY                                                                                          \
    "INSERT INTO Events (timestamp, event_type, job_id, step_id, value, node_id) VALUES (?, ?, ?, ?, ?, ?)"

#if EXP_EARGM
#define EAR_WARNING_MYSQL_QUERY                                                                                        \
    "INSERT INTO Global_energy2 (energy_percent, warning_level, inc_th, p_state, GlobEnergyConsumedT1, "               \
...
#define MYSQL_APPS_FALLBACK 1

int32_t mysql_insert_applications_fallback(MYSQL *connection, application_t *app, int32_t num_apps, long long *sigs_ids,
                                           long long *pow_sigs_ids)
{
    if (app == NULL) {
    ...
    state_t gret     = EAR_SUCCESS;

    char *query;
    if (!is_learning) {
        // APPLICATION_MYSQL_QUERY already has the params for ONE application
        query = xmalloc(strlen(APPLICATION_MYSQL_QUERY) + 1);
        strcpy(query, APPLICATION_MYSQL_QUERY);
    } else {
        query = xmalloc(strlen(LEARNING_APPLICATION_MYSQL_QUERY) + 1);
        strcpy(query, LEARNING_APPLICATION_MYSQL_QUERY);
    }
    MYSQL_BIND bind[APPLICATION_ARGS] = {0};
    ...
    for (int32_t i = 0; i < num_apps; i++) {
        ...
        if (!is_mpi) {
            ...
            bind[4].is_null     = (ear_my_bool) 1;
        }

        // string types
        bind[3].buffer_type   = MYSQL_TYPE_VAR_STRING;
        bind[3].buffer_length = strlen(app[i].node_id);

        // storage variable assignation
        bind[0].buffer = (char *) &app[i].job.id;
        bind[1].buffer = (char *) &app[i].job.step_id;
        bind[2].buffer = (char *) &app[i].job.local_id;
        bind[3].buffer = (char *) &app[i].node_id;
        if (is_mpi)
            bind[4].buffer = (char *) &sigs_ids[i];
        else
            bind[4].buffer = (char *) NULL;
        bind[5].buffer = (char *) &pow_sigs_ids[i];
...
int mysql_batch_insert_applications(MYSQL *connection, application_t *app, int num_apps)
{
    ...
    for (i = 0; i < num_apps; i++) {
        ...
        bind[0 + offset].buffer_type = bind[1 + offset].buffer_type = bind[2 + offset].buffer_type =
            bind[4 + offset].buffer_type = bind[5 + offset].buffer_type = MYSQL_TYPE_LONG;
        bind[0 + offset].is_unsigned = bind[1 + offset].is_unsigned = bind[2 + offset].is_unsigned =
            bind[4 + offset].is_unsigned = bind[5 + offset].is_unsigned = 1;
        if (!is_mpi) {
            bind[4 + offset].buffer_type = MYSQL_TYPE_NULL;
            bind[4 + offset].is_null     = (ear_my_bool) 1;
        }

        // string types
        bind[3 + offset].buffer_type   = MYSQL_TYPE_VAR_STRING;
        bind[3 + offset].buffer_length = strlen(app[i].node_id);
        ...
        bind[3 + offset].buffer = (char *) &app[i].node_id;
        if (is_mpi)
            bind[4 + offset].buffer = (char *) &sigs_ids[i];
        else
            bind[4 + offset].buffer = (char *) NULL;
        bind[5 + offset].buffer = (char *) &pow_sigs_ids[i];
    }
    ...
    if (ret == EAR_SUCCESS) {
        if (mysql_stmt_execute(statement)) {
            ret = mysql_statement_error(statement);
#if MYSQL_APPS_FALLBACK
            warning("Application insertion failed, using fallback function");
            mysql_insert_applications_fallback(connection, app, num_apps, sigs_ids, pow_sigs_ids);
#endif
        }
    }
...
int mysql_batch_insert_applications_no_mpi(MYSQL *connection, application_t *app, int num_apps)
{
    ...
    for (i = 0; i < num_apps; i++) {
        ...
        // string types
        bind[3 + offset].buffer_type   = MYSQL_TYPE_STRING;
        bind[3 + offset].buffer_length = strlen(app[i].node_id);
...
int mysql_retrieve_applications(MYSQL *connection, char *query, application_t **apps, char is_learning)
{
    ...
    if (mysql_stmt_prepare(statement, query, strlen(query)))
        return mysql_statement_error(statement);
    MYSQL_BIND bind[APPLICATION_ARGS];
    unsigned long job_id, step_id, pow_sig_id, local_id;
    long long sig_id = 0;
    int num_apps;
    ...
    bind[3].buffer = &app_aux->node_id;
    bind[4].buffer = &sig_id;
    bind[5].buffer = &pow_sig_id;
...
int mysql_batch_insert_loops(MYSQL *connection, loop_t *loop, int num_loops)
{
    ...
    for (i = 0; i < num_loops; i++) {
        ...
        // string types
        bind[offset + 6].buffer_type   = MYSQL_TYPE_VAR_STRING;
        bind[offset + 6].buffer_length = strlen(loop[i].node_id);
        ...
        bind[offset + 5].buffer = (char *) &loop[i].local_id;
        bind[offset + 6].buffer = (char *) &loop[i].node_id;
        bind[offset + 7].buffer = (char *) &loop[i].total_iterations;
        bind[offset + 8].buffer = (char *) &sigs_ids[i];
    }
...
int mysql_retrieve_loops(MYSQL *connection, char *query, loop_t **loops)
{
    ...
    bind[5].buffer = &loop_aux->local_id;
    bind[6].buffer = &loop_aux->node_id;
    bind[7].buffer = &loop_aux->total_iterations;
    bind[8].buffer = &sig_id;
...
long long mysql_batch_insert_signatures(MYSQL *connection, signature_container_t cont, char is_learning, int num_sigs)
{
    ...
    if (full_signature) {
        params     = ", (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        num_params = FULL_SIGNATURE_ARGS;
    } else {
        params     = ", (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        num_params = SIMPLE_SIGNATURE_ARGS;
    }
#else
    if (full_signature) {
        params     = ", (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
int mysql_retrieve_signatures(MYSQL *connection, char *query, signature_t **sigs)
{
...
#endif
    if (full_signature)
        num_params = FULL_SIGNATURE_ARGS + 1; // we add 1 because we also read the signature_id
    else
        num_params = SIMPLE_SIGNATURE_ARGS + 1;
    ...
    while (status == 0 || status == MYSQL_DATA_TRUNCATED) {
        ...
        if (min_gpu_sig_id > 0 && max_gpu_sig_id > 0) {
            ...
            sprintf(gpu_sig_query, "SELECT * FROM GPU_signatures WHERE id <= %ld AND id >= %ld", max_gpu_sig_id,
                    min_gpu_sig_id);
...
long long mysql_batch_insert_gpu_signatures(MYSQL *connection, signature_container_t cont, int num_sigs)
{
    ...
    for (i = 0; i < num_sigs; i++) {
        ...
        for (j = 0; j < gpu_sig->num_gpus; j++) {
            ...
            bind[5 + offset].buffer_type = MYSQL_TYPE_FLOAT;
            ...
            bind[5 + offset].buffer = (char *) &gpu_sig->gpu_data[j].GPU_GFlops;

            offset += GPU_SIGNATURE_ARGS;
        }
    }

    if (offset / GPU_SIGNATURE_ARGS < num_gpu_sigs) {
        verbose(VMYSQL, "Assigned less signatures than allocated");
    } else if (offset / GPU_SIGNATURE_ARGS > num_gpu_sigs) {
        verbose(VMYSQL, "Assigned more signatures than allocated");
    }
...
int mysql_retrieve_gpu_signatures(MYSQL *connection, char *query, gpu_signature_t *gpu_sig)
{
    ...
    MYSQL_BIND bind[GPU_SIGNATURE_ARGS + 1];
    gpu_app_t base_data;
    memset(bind, 0, sizeof(bind));
    ...
    bind[6].buffer = &base_data.GPU_GFlops;
...
int mysql_insert_periodic_aggregation(MYSQL *connection, periodic_aggregation_t *per_agg)
{
    ...
    if (mysql_stmt_prepare(statement, PERIODIC_AGGREGATION_MYSQL_QUERY, strlen(PERIODIC_AGGREGATION_MYSQL_QUERY)))
        return mysql_statement_error(statement);
...
int mysql_batch_insert_periodic_metrics(MYSQL *connection, periodic_metric_t *per_mets, int num_mets)
{
    ...
    char *params;
    int num_args = node_detail ? FULL_PERIODIC_METRIC_ARGS : SIMPLE_PERIODIC_METRIC_ARGS;
...
int mysql_batch_insert_ear_events(MYSQL *connection, ear_event_t *ear_ev, int num_evs)
{
    ...
    for (i = 0; i < num_evs; i++) {
        ...
        bind[2 + offset].is_unsigned = bind[3 + offset].is_unsigned = 1;
...
int mysql_insert_gm_warning(MYSQL *connection, gm_warning_t *warning)
{
    ...
    if (mysql_stmt_prepare(statement, EAR_WARNING_MYSQL_QUERY, strlen(EAR_WARNING_MYSQL_QUERY)))
        return mysql_statement_error(statement);
