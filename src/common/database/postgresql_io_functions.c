...
#define PERIODIC_AGGREGATION_PSQL_QUERY                                                                                \
...
// htonl() always reverses, regardless of the endianess of the input, same as ntohl(), it might be useful to change this
// calls to __builtin_bswap64 in the future (GCC ONLY) double_swap() must be used for floating point values, as htonl
// does not work properly with them the procedure to workaround the lack of INSERT IGNORE in postgres <9.5 is: create a
// temp table, insert into it, filter repetitions and insert into actual table
//
...
int postgresql_retrieve_jobs(PGconn *connection, char *query, job_t **jobs)
{
    ...
    // for every row, we read the data and query the job, power_signature and signature, the latter only if the job has
    // one
...
int postgresql_retrieve_gpu_signatures(PGconn *connection, char *query, gpu_signature_t *gpu_sig)
{
    ...
    for (i = 0; i < num_rows; i++) {
        ...
        base_data.GPU_GFlops   = htonl(*((ulong *) PQgetvalue(res, i, 7)));
...
{
    char *full_query =
        calloc(strlen("SELECT last_values FROM ") + strlen(table_name) + strlen("_id_seq") + 1, sizeof(char));
    strcpy(full_query, "SELECT last_value FROM ");
    strcat(full_query, table_name);
    strcat(full_query, "_id_seq");
    PGresult *res =
        PQexecParams(connection, full_query, 0, NULL, NULL, NULL, NULL, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_ear_events(PGconn *connection, ear_event_t *events, int num_events)
{
    ...
    PGresult *res =
        PQexecParams(connection, query, EAR_EVENTS_ARGS * num_events, NULL, (const char *const *) param_values,
                     param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_gm_warnings(PGconn *connection, gm_warning_t *warns, int num_warns)
{
    ...
    PGresult *res = PQexecParams(connection, query, 10 * num_warns, NULL, (const char *const *) param_values,
                                 param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_periodic_aggregations(PGconn *connection, periodic_aggregation_t *per_aggs, int num_aggs)
{
    ...
    query =
        calloc((strlen(PERIODIC_AGGREGATION_PSQL_QUERY) + (num_aggs * PERIODIC_AGGREGATION_ARGS * 10)), sizeof(char));
    ...
    PGresult *res =
        PQexecParams(connection, query, PERIODIC_AGGREGATION_ARGS * num_aggs, NULL, (const char *const *) param_values,
                     param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_periodic_metrics(PGconn *connection, periodic_metric_t *per_mets, int num_mets)
{
    ...
    PGresult *res = PQexecParams(connection, query, per_met_args * num_mets, NULL, (const char *const *) param_values,
                                 param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_power_signatures(PGconn *connection, power_signature_t *sigs, int num_sigs)
{
    ...
    PGresult *res =
        PQexecParams(connection, query, POWER_SIGNATURE_ARGS * num_sigs, NULL, (const char *const *) param_values,
                     param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_gpu_signatures(PGconn *connection, signature_t *sigs, int num_sigs)
{
    ...
    for (i = 0; i < num_sigs; i++) {
        for (j = 0; j < sigs[i].gpu_sig.num_gpus; j++) {
            ...
            param_values[5 + offset] = (char *) &sigs[i].gpu_sig.gpu_data[j].GPU_GFlops;
            ...
            param_lengths[5 + offset] = sizeof(sigs[i].gpu_sig.gpu_data[j].GPU_GFlops);
    ...
    PGresult *res =
        PQexecParams(connection, query, GPU_SIGNATURE_ARGS * num_sigs, NULL, (const char *const *) param_values,
                     param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_signatures(PGconn *connection, signature_t *sigs, char is_learning, int num_sigs)
{
    ...
    query = calloc((strlen(SIGNATURE_QUERY_FULL) + (num_sigs * sig_args * 10) + strlen("ON CONFLICT DO NOTHING")),
                   sizeof(char));
#if USE_GPUS
    ...
    if (starter_gpu_sig_id != EAR_SUCCESS) {
        ...
        if ((starter_gpu_sig_id = postgresql_get_current_autoincrement_val(connection, "signatures")) < 1) {
            ...
            for (i = 0; i < num_sigs; i++)
                if (sigs[i].gpu_sig.num_gpus > 0)
                    num_gpu_sigs += sigs[i].gpu_sig.num_gpus;
    ...
    PGresult *res = PQexecParams(connection, query, sig_args * num_sigs, NULL, (const char *const *) param_values,
                                 param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_jobs(PGconn *connection, application_t *apps, int num_jobs)
{
    ...
    query =
        calloc((strlen(JOB_PSQL_QUERY) + (num_jobs * JOB_ARGS * 10) + strlen("ON CONFLICT DO NOTHING")), sizeof(char));
    ...
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        verbose(VMYSQL, "ERROR while creating transaction block: %s\n", PQresultErrorMessage(res));
    PQclear(res); // This free may not be correct, if it sigsegv's in this function all but the last PQclear should be
                  // suspects

    res =
        PQexecParams(connection, CREATE_TEMP_JOBS, 0, NULL, NULL, NULL, NULL, 1); // CREATE temp table to store all jobs
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        verbose(VMYSQL, "ERROR while creating temp table: %s\n", PQresultErrorMessage(res));
    PQclear(res);

    res = PQexecParams(connection, lock_query, 0, NULL, NULL, NULL, NULL, 1); // LOCK main table
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        verbose(VMYSQL, "ERROR while creating locking main table: %s\n", PQresultErrorMessage(res));
    PQclear(res);

    res = PQexecParams(connection, query, num_jobs * JOB_ARGS, NULL, (const char *const *) param_values, param_lengths,
                       param_formats, 1); // INSERT all jobs into temp table
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        verbose(VMYSQL, "ERROR while inserting to temp table: %s\n", PQresultErrorMessage(res));
    PQclear(res);

    res = PQexecParams(connection, insert_query, 0, NULL, NULL, NULL, NULL,
                       1); // INSERT new jobs into Jobs, ignore the rest
    if (PQresultStatus(res) != PGRES_COMMAND_OK)
        verbose(VMYSQL, "ERROR while merging with main table: %s\n", PQresultErrorMessage(res));
    PQclear(res);

    debug("running query %s\n", "COMMIT");
    res = PQexecParams(connection, "COMMIT", 0, NULL, NULL, NULL, NULL,
                       1); // COMMIT and end transaction block, dropping the table
...
int postgresql_batch_insert_loops(PGconn *connection, loop_t *loops, int num_loops)
{
    ...
    query         = calloc((strlen(LOOP_PSQL_QUERY) + (num_loops * LOOP_ARGS * 10) + strlen("ON CONFLICT DO NOTHING")),
                           sizeof(char));
    ...
    PGresult *res = PQexecParams(connection, query, num_loops * LOOP_ARGS, NULL, (const char *const *) param_values,
                                 param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
...
int postgresql_batch_insert_applications(PGconn *connection, application_t *apps, int num_apps)
{
    ...
    query         = calloc((strlen(LEARNING_APPLICATION_PSQL_QUERY) + (num_apps * APPLICATION_ARGS * 10) +
                    strlen("ON CONFLICT DO NOTHING")),
                           sizeof(char));
    ...
    PGresult *res =
        PQexecParams(connection, query, num_apps * APPLICATION_ARGS, NULL, (const char *const *) param_values,
                     param_lengths, param_formats, 1); // 0 indicates text mode, 1 is binary
