...
{
    if (state_fail(user_ruid_get(&user->ruid, user->ruid_name)))
        return EAR_ERROR;
    if (state_fail(user_euid_get(&user->euid, user->euid_name)))
        return EAR_ERROR;
    if (state_fail(user_rgid_get(&user->rgid, user->rgid_name)))
        return EAR_ERROR;
    if (state_fail(user_rgid_get(&user->egid, user->egid_name)))
        return EAR_ERROR;
    return EAR_SUCCESS;
}
