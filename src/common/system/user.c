...
{
    if (!user_get_uid(getuid(), &user->ruid, user->ruid_name))
        return EAR_ERROR;
    if (!user_get_uid(geteuid(), &user->euid, user->euid_name))
        return EAR_ERROR;
    if (!user_get_gid(getgid(), &user->rgid, user->rgid_name))
        return EAR_ERROR;
    if (!user_get_gid(getegid(), &user->egid, user->egid_name))
        return EAR_ERROR;
