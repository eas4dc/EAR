...
{
    if (conf_ear_app_name != NULL)
        free(conf_ear_app_name);
...
int getenv_ear_power_policy()
{
    ...
    if (my_policy != NULL) {
        ...
        if (conf_ear_power_policy == EAR_ERROR)
            conf_ear_power_policy = DEFAULT_POWER_POLICY;
    }
...
double getenv_ear_power_policy_th()
{
    ...
    switch (conf_ear_power_policy) {
        ...
        case MIN_TIME_TO_SOLUTION:
            ...
            if (my_th != NULL) {
                conf_ear_power_policy_th = strtod(my_th, NULL);
                if ((conf_ear_power_policy_th < DEFAULT_MIN_TIME_TH) || (conf_ear_power_policy_th > 1.0))
...
int getenv_ear_reset_freq()
{
    ...
    if (my_reset != NULL) {
        if (strcmp(my_reset, "1") == 0)
            conf_ear_reset_freq = 1;
    }
...
double getenv_ear_performance_accuracy()
{
    ...
    if (my_perf_acu != NULL) {
        ...
        if (conf_ear_performance_accuracy < 0)
            conf_ear_performance_accuracy = DEFAULT_PERFORMANCE_ACURACY;
    }
...
int getenv_ear_num_nodes()
{
    ...
    if (my_num_nodes == NULL) {
        ...
        if (my_num_nodes == NULL)
            conf_ear_num_nodes = 1;
        else
            conf_ear_num_nodes = atoi(my_num_nodes);
    } else
        conf_ear_num_nodes = atoi(my_num_nodes);
...
int getenv_ear_dynais_levels()
{
    ...
    if (dynais_levels != NULL) {
        ...
        if (dyn_level > 0)
            conf_ear_dynais_levels = dyn_level;
    }
...
int getenv_ear_dynais_window_size()
{
    ...
    if (dynais_size != NULL) {
        ...
        if (dyn_size > 0)
            conf_ear_dynais_window_size = dyn_size;
    }
...
{
    if (path != NULL) {
        if (conf_ear_coeff_db_pathname != NULL)
            free(conf_ear_coeff_db_pathname);
