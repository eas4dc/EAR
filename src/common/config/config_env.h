...
#define HACK_POWER_POLICY                                                                                              \
    "HACK_POWER_POLICY" // This var forces to load a specific power policy. Replaces (SCHED_PREFIX)_EAR_POWER_POLICY.
#define HACK_GPU_POWER_POLICY                                                                                          \
    "HACK_GPU_POWER_POLICY" // This var forces to load a specific gpu power policy. Replaces
                            // (SCHED_PREFIX)_EAR_GPU_POWER_POLICY.
#define HACK_GPU_POLICY_PENALTY                                                                                        \
    "HACK_GPU_POLICY_PENALTY" // Sets the threshold penalty tolered by Min-GPU-Ebnergy policy.. Replaces
                              // (SCHED_PREFIX)_EAR_GPU_POLICY_PENALTY.
#define HACK_POWER_MODEL                                                                                               \
    "HACK_POWER_MODEL" // This var forces to load a specific power model plugin. Replaces
                       // (SCHED_PREFIX)_EAR_POWER_MODEL.
#define HACK_GPU_ENERGY_MODEL                                                                                          \
    "HACK_GPU_ENERGY_MODEL" // This var forces to load a specific GPU energy model plugin. Replaces
                            // (SCHED_PREFIX)_EAR_GPU_POWER_MODEL.
...
#define HACK_EARL_GPU_COEFF_FILE                                                                                       \
    "HACK_EARL_GPU_COEFF_FILE" // This var forces to load a specific coefficient file for GPU models
#define HACK_DYNAIS_WINDOW_SIZE                                                                                        \
    "HACK_DYNAIS_WINDOW_SIZE" // This var forces a specific dynais windows size. Replaces
                              // (SCHED_PREFIX)_EAR_DYNAIS_WINDOW_SIZE.
#define HACK_DEF_FREQ                                                                                                  \
    "HACK_DEF_FREQ" // This var forces a different default frequency than the predefined for a policy. Replaces
                    // (SCHED_PREFIX)_EAR_DEF_FREQ.
#define HACK_EARL_VERBOSE "HACK_EARL_VERBOSE" // This var forces an EARL verbosity.
#define HACK_EARL_INSTALL_PATH                                                                                         \
    "HACK_EARL_INSTALL_PATH" // This variable HACKS all the library environment, simplifies the utilization of a
                             // privatized environment.
#define HACK_PROCS_VERB_FILES                                                                                          \
    "HACK_PROCS_VERB_FILES" // This variable makes all processes having a log verbose file each one.
#define HACK_ROOFLINE_COEFF_PATH                                                                                       \
    "HACK_ROOFLINE_COEFF_PATH" // This var forces to load a specific roofline coefficient file
...
#define FLAG_GPU_DCGMI_ENABLED                                                                                         \
    "EAR_GPU_DCGMI_ENABLED" // if DCGMI is enable at compile time, this flag allows to explicitly disable it: Default 1.
                            // (Use 1/0)
#define FLAG_DCGMI_SAMPLING_PERIOD                                                                                     \
    "EAR_DCGMI_SAMPLING_PERIOD" // Set the period of the DCGMI metrics sampling monitor. Be careful, as very low values
                                // can introduce considerable overhead. This variable is read as a single precision
                                // floating point number.
#define FLAG_DCGM_ALL_EVENTS "EAR_DCGM_ALL_EVENTS" // Request to collect and report all DCGM events.

#define FLAG_LOADER_APPLICATION                                                                                        \
    "EAR_LOADER_APPLICATION" // This variable forces to load the NON MPI version of the library for a given application
                             // (i.e. SLURM_LOADER_LOAD_NO_MPI_LIB=gromacs).
#define FLAG_LOAD_MPI_VERSION                                                                                          \
    "EAR_LOAD_MPI_VERSION" // This variable is used for not detected MPI application (python) to specify which version
                           // it is: intel, ompi, etc
...
#define FLAG_EARL_PHASES                                                                                               \
    "USE_EARL_PHASES" // Allows EARL to detect application phases (e.g., compute bound, memory bound, mpi bound) and
                      // improve policy default decisions.
#define FLAG_LOAD_BALANCE                                                                                              \
    "EAR_LOAD_BALANCE" // Force changing policies' default decisions if they detect the application is load unbalanced.
...
#define FLAG_NTWRK_IMC                                                                                                 \
    "EAR_NTWRK_IMC" // Tells the IMC policy the application uses IMC freq for network operations, making the policy less
                    // aggressive.
#define FLAG_DISABLE_NODE_METRICS                                                                                      \
    "EAR_DISABLE_NODE_METRICS" // If defined, This job will not be considered for node metrics and policy will not be
                               // applied : To be used in MASTER/WORKED use cases
#define FLAG_NTASK_WORK_SHARING                                                                                        \
    "EAR_NTASK_WORK_SHARING" // Tells EAR to consider the N tasks created by the scheduler as a single job (for non MPI
                             // use cases).
#define FLAG_TRY_TURBO                                                                                                 \
    "EAR_TRY_TURBO" // If policies have make CPU frequency be at maximum, try to boost the CPU frequency.
#define FLAG_REPORT_ADD "EAR_REPORT_ADD"
#define FLAG_USE_ENERGY_MODELS                                                                                         \
    "EAR_USE_ENERGY_MODELS" // Tells policies to use power models instead of probe and error DVFS.
...
#define FLAG_MPI_SAMPLING_ENABLED                                                                                      \
    "EAR_MPI_SAMPLING_ENABLED" // Allows MPI sampling monitoring to be enabled(1) or disabled (0). Default is 1.
#define FLAG_LOOP_STRATEGY  "EAR_LOOP_STRATEGY"  // Specifies is EARL is dynamic or time guided
#define FLAG_MPI_MONITORING "EAR_MPI_MONITORING" // Specifies if EARL has to take actions in MPI or not

#define FLAG_NO_AFFINITY_MASK                                                                                          \
    "EARL_NO_AFFINITY_MASK"                   // Prevents EARL from using the affinity mask. Only for special use cases
...
#define FLAG_PYTHON_MULTIPROC                                                                                          \
    "EARL_PYTHON_MULTIPROC" // Enables features supporting applications which use Python's multiprocessing module.
                            // Currently, this variable has the same effect as FLAG_NO_SIGHANDLER
#define FLAG_NO_SIGHANDLER                                                                                             \
    "EARL_NO_SIGHANDLER" // Configures EARL to not configure any signal handler. This is useful to avoid conflicts with
                         // applications which want to handle signals.
#define FLAG_GPU_MASTER_OPTIMIZE                                                                                       \
    "EARL_GPU_MASTER_OPTIMIZE" // Enforces just the first master process to apply the GPU optimization. This flag is
 ...
 * @{
 */
#define SCHED_LOADER_LOAD_NO_MPI_LIB                                                                                   \
    "SLURM_LOADER_APPLICATION" // TODO: Deprectated. This variable is maintaned here in order to avoid errors in users'
                               // scripts.
#define SCHED_LOAD_MPI_VERSION                                                                                         \
    "SLURM_LOAD_MPI_VERSION" // TODO: Deprectated. This variable is maintaned here in order to avoid errors in users'
                             // scripts.
