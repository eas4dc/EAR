...
unsigned long long ullong_diff_overflow(unsigned long long begin, unsigned long long end)
{
    ...
    if (begin < max_16 && end < max_16) {
        ret = max_16 - begin + end;
               ...
               max_32 < max_64) // some architectures have the same size for ul and ull
    {
        ret = max_32 - begin + end;
...
{
    return ((n - src_range_min) /
            (src_range_max - src_range_min)); // *(target_range_max - target_range_min) + target_range_min;
}
