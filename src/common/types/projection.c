...
{
    return (coeff->D * sign->CPI) + (coeff->E * sign->TPI) + (coeff->F);
}
...
double basic_project_time(signature_t *sign, coefficient_t *coeff)
{
    ...
    if (coeff->available) {
        ...
        ptime = ((sign->time * proj_cpi) / sign->CPI) * (freq_src / freq_dst);
    }
    return ptime;
}
...
double basic_project_power(signature_t *sign, coefficient_t *coeff)
{
    double ppower = 0;
    if (coeff->available) {
        ppower = (coeff->A * sign->DC_power) + (coeff->B * sign->TPI) + (coeff->C);
    }
    return ppower;
}
