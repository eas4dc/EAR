...
void power_signature_print_fd(int fd, power_signature_t *power_signature)
{
    ...
    dprintf(fd, "%lf;%lf;%lf;%lf;%lf;%lf;%lu;%lu", power_signature->DC_power, power_signature->DRAM_power,
            power_signature->PCK_power, power_signature->max_DC_power, power_signature->min_DC_power,
...
{
    if (!isnormal(ps->DC_power))
        ps->DC_power = 0;
    if (!isnormal(ps->DRAM_power))
        ps->DRAM_power = 0;
    if (!isnormal(ps->PCK_power))
        ps->PCK_power = 0;
    if (!isnormal(ps->EDP))
        ps->EDP = 0;
    if (!isnormal(ps->max_DC_power))
        ps->max_DC_power = 0;
    if (!isnormal(ps->min_DC_power))
        ps->min_DC_power = 0;
    if (ps->DC_power > limit)
        ps->DC_power = limit;
    if (ps->DRAM_power > limit)
        ps->DRAM_power = 0;
    if (ps->PCK_power > limit)
        ps->PCK_power = 0;
}
...
void power_signature_clean_before_db(power_signature_t *sig, double pwr_limit)
{
    ...
    if (!isnormal(sig->PCK_power))
        sig->PCK_power = 0;
    if (!isnormal(sig->max_DC_power))
        sig->max_DC_power = 0;
    if (!isnormal(sig->min_DC_power))
        sig->min_DC_power = 0;
