...
void print_job_fd(int fd, job_t * job)
{
	...
	sprintf(job_buff,
		...
		job->start_mpi_time, job->end_mpi_time, job->policy, job->th,
	...
	sprintf(job_buff,
		...
		job->end_mpi_time, job->policy, job->th, job->procs, job->type,
		job->def_f);
#endif
...
void report_job(job_t * job)
{
	...
	verbose(VTYPE,
		...
		"etag %s\n",
		job->id, job->step_id, job->local_id, job->user_id,
		job->group_id, job->app_id, job->user_acc, job->energy_tag);
#else
	verbose(VTYPE,
		"Job: ID %lu step %lu  user %s group %s name %s account %s etag %s\n",
		job->id, job->step_id, job->user_id, job->group_id, job->app_id,
		job->user_acc, job->energy_tag);
#endif
	verbose(VTYPE,
		"start time %ld end time %ld start mpi %ld end mpi %ld policy %s th "
		"%lf def_f %lu\n",
		job->start_time, job->end_time, job->start_mpi_time,
		job->end_mpi_time, job->policy, job->th, job->def_f);
}
