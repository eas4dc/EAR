...
void periodic_metrict_print_channel(FILE *file, periodic_metric_t *pm)
{
    ...
    fprintf(file, "periodic_metric_t: start '%lu', end '%lu', energy '%lu' avg_freq %lu temp %lu\n", pm->start_time,
            pm->end_time, pm->DC_energy, pm->avg_f, pm->temp);
}
...
{
    if (pm->DC_energy > INT_MAX)
        pm->DC_energy = INT_MAX;
    if (pm->avg_f > INT_MAX)
        pm->avg_f = INT_MAX;
    if (pm->temp > INT_MAX)
        pm->temp = INT_MAX;
    if (pm->DRAM_energy > INT_MAX)
        pm->DRAM_energy = INT_MAX;
    if (pm->PCK_energy > INT_MAX)
        pm->PCK_energy = INT_MAX;
#if USE_GPUS
    if (pm->GPU_energy > INT_MAX)
        pm->GPU_energy = INT_MAX;
#endif
}
