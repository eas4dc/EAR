...
void application_print_channel(FILE *file, application_t *app)
{
    ...
    fprintf(file, "application_t: pow dc '%lf', pow ram '%lf', pow pack '%lf'\n", app->signature.DC_power,
            app->signature.DRAM_power, app->signature.PCK_power);
}
...
int scan_application_fd(FILE *fd, application_t *app, char is_extended)
{
    ...
    if (is_extended)
        ret = fscanf(fd,
                     ...
                     "%llu;%llu;%llu;%llu;"
                     "%llu;%llu;%llu;%llu\n",
                     a->node_id, &a->job.id, &a->job.step_id, a->job.user_id, a->job.group_id, a->job.app_id,
                     a->job.user_acc, a->job.energy_tag, a->job.policy, &a->job.th, &a->signature.avg_f,
                     &a->signature.def_f, &a->signature.time, &a->signature.CPI, &a->signature.TPI, &a->signature.GBS,
                     &a->signature.DC_power, &a->signature.DRAM_power, &a->signature.PCK_power, &a->signature.cycles,
                     &a->signature.instructions, &a->signature.Gflops, &a->signature.ps_sig.cpu_util,
                     &a->signature.L1_misses, &a->signature.L2_misses, &a->signature.L3_misses, &a->signature.FLOPS[0],
                     &a->signature.FLOPS[1], &a->signature.FLOPS[2], &a->signature.FLOPS[3], &a->signature.FLOPS[4],
                     &a->signature.FLOPS[5], &a->signature.FLOPS[6], &a->signature.FLOPS[7]);
...
void verbose_application_data(uint vl, application_t *app)
{
    ...
    if (ret < 0) {
        ...
        verbose(vl, "%sWARNING%s The application summary message was truncated. Required size: %d", COL_YLW, COL_CLR,
                ret);
    }
    ...
    if (app->is_mpi) {
        ...
        snprintf(mpi_info_txt, sizeof(mpi_info_txt),
                 "(Appl. sign.) Wall time: %0.3lf s, CPU util.: %u %% nominal freq.: %0.2f GHz, avg. freq: %0.2f GHz, "
                 "avg imc freq: %0.2f GHz\n"
                 ...
                 app->signature.time, app->signature.ps_sig.cpu_util, def_f, avg_f, avg_imc_f, app->signature.perc_MPI,
                 app->job.procs, app->signature.CPI, app->signature.TPI, app->signature.GBS, app->signature.Gflops,
                 app->signature.IO_MBS, app->signature.DC_power, app->signature.DRAM_power, app->signature.PCK_power,
                 app->signature.Gflops / app->signature.DC_power);
    ...
    int app_summ_hdr_dec_len =
        (int) ((max_str_len - strlen(app_summ_hdr_txt)) / 2); // Compute the length of the decorator, e.g. "---"
    ...
    */
    ...
    if (app_sig->gpu_sig.num_gpus) {
        ...
        if (gpu_sig_buff != NULL) {
            ...
            verbose(vl, "\n%s%s%s\n%s\n%s\n%s", app_summ_hdr_dec_txt, app_summ_hdr_txt, app_summ_hdr_dec_txt,
                    job_info_txt, gpu_sig_buff, app_summ_ftr_dec); // header + job info + gpu info + footer
            free(gpu_sig_buff);
        }
    } else {
        verbose(vl, "\n%s%s%s\n%s\n%s", app_summ_hdr_dec_txt, app_summ_hdr_txt, app_summ_hdr_dec_txt, job_info_txt,
                app_summ_ftr_dec); // header + job info + footer
    }
#else
    verbose(vl, "\n%s%s%s\n%s\n%s", app_summ_hdr_dec_txt, app_summ_hdr_txt, app_summ_hdr_dec_txt, job_info_txt,
            app_summ_ftr_dec); // header + job info + footer
...
void report_mpi_application_data(int vl, application_t *app)
{
    ...
    verbose(vl, "%s%s%s\n%s\n", app_summ_hdr_dec_txt, app_summ_hdr_txt, app_summ_hdr_dec_txt,
            job_info_txt); // header and job info
                                      ...
                                      char is_extended, int single_column)
{
    ...
    for (uint s = 0; s < num_sockets; s++) {
        char temp_hdr[33]; // Assumming MAX_SOCKETS_SUPPORTED HAVE AT MOST 2 digits + null byte
    ...
    char *HEADER_POWER_SIG = ";NODEMGR_DC_NODE_POWER_W;NODEMGR_DRAM_POWER_W;NODEMGR_PCK_POWER_W;NODEMGR_MAX_DC_POWER_W;"
                             "NODEMGR_MIN_DC_POWER_W;NODEMGR_TIME_SEC;NODEMGR_AVG_CPUFREQ_KHZ;NODEMGR_DEF_FREQ_KHZ";

    size_t header_len = strlen(HEADER_JOB) + strlen(HEADER_BASE) + strlen(HEADER_SIGN) +
                        num_gpus * USE_GPUS * strlen(HEADER_GPU_SIG) + strlen(ext_header) + 1 + strlen(cpu_sig_hdr) +
                        strlen(HEADER_POWER_SIG);
    if (header_prefix) {
