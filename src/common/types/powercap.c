...
pc_device_types_map_t device_map[] = {{"DRAM_0", DRAM0},  {"DRAM_1", DRAM1}, {"CPU_0", CPU0}, {"CPU_1", CPU1},
                                      {"GPU_", GPUSTART}, {"PCK_0", CPU0},   {"PCK_1", CPU1}, {"DRAM0", DRAM0},
                                      {"DRAM1", DRAM1},   {"CPU0", CPU0},    {"CPU1", CPU1},  {"PCK0", CPU0},
                                      {"PCK1", CPU1},     {"GPU", GPUSTART}};
        ...
        if (strncmp(device_name, device_map[i].name, len) == 0) {
            if (i == GPUSTART) {
                ...
                if (device_name[len] >= '0' && device_name[len] <= '9') {
                    int tmp = device_map[i].type;
                    tmp += (device_name[len] - 48);
                    return (powercap_device_types) tmp;
                } else
                    return NO_TYPE;
            } else
                return device_map[i].type;
        }
    }
    return NO_TYPE;
}
