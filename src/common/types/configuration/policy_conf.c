...
int get_default_policies(cluster_conf_t *conf, policy_conf_t **policies, int tag_id)
{
    ...
    for (i = 0; i < conf->num_policies; i++) {
        ...
        if (uses_def_tag) {
            if (!strcmp(conf->tags[tag_id].id, conf->power_policies[i].tag) ||
                strlen(conf->power_policies[i].tag) < 1) {
...
void print_policy_conf(policy_conf_t *p)
{
    ...
    verbosen(VCCONF, "---> policy %s id %d p_state %u def_freq %.3f NormalUsersAuth %u", p->name, p->policy, p->p_state,
             p->def_freq, p->is_available);
    for (i = 0; i < MAX_POLICY_SETTINGS; i++) {
        if (p->settings[i] > 0) {
