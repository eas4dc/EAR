...
{
    if (strcasestr(token, "EARD") != NULL)
        return EAR_SUCCESS;
    if (strcasestr(token, "NodeDaemon") != NULL)
        return EAR_SUCCESS;
    return EAR_ERROR;
}
...
void set_default_eard_conf(eard_conf_t *eardc)
{
    ...
    eardc->period_powermon   = POWERMON_FREQ;
