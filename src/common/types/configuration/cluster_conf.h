...
typedef struct cluster_conf {
    ...
    uint auth_users_count;
    char **auth_users;
    uint auth_groups_count;
    char **auth_groups;
    uint auth_acc_count;
    char **auth_accounts;
    ...
    uint num_edcmons_tags;
    edcmon_t *edcmon_tags;
    /**@}*/
} cluster_conf_t;
...
/** returns the ear.conf path. It checks first at /etc/ear/ear.conf and, it is not available, checks at
 * $EAR_INSTALL_PATH/etc/sysconf/ear.conf */
int get_ear_conf_path(char *ear_conf_path);

/** returns the eardbd.conf path. It checks first at /etc/ear/eardbd.conf and, it is not available, checks at
 * $EAR_INSTALL_PATH/etc/sysconf/eardbd.conf */
...
/** Returns whether the user, group and/or acc is present in the list of
 * authorized users, groups or accounts, respectively. */
int is_authorized_usr_grp_acc(cluster_conf_t *my_conf, char *user, char *group, char *acc);

/** Returns whether the user is in the list of admin users.
 * If some of my_conf or user arguments is NULL, returns a negative number. */
int is_admin_usr(cluster_conf_t *my_conf, char *user);

/** returns the user type: NORMAL, AUTHORIZED, ENERGY_TAG */
uint get_user_type(cluster_conf_t *my_conf, char *energy_tag, char *user, char *group, char *acc,
                   energy_tag_t **my_tag);
...
/** given a node name, get all ips of its range. Allocates memory.*/
...
/* Same as get_ip_ranges, but also copies the hostnames in *names. Allocates memory.*/
