...
{
    if (strcasestr(token, "EARGM") != NULL)
        return EAR_SUCCESS;
    if (strcasestr(token, "GlobalManager") != NULL)
        return EAR_SUCCESS;
    return EAR_ERROR;
}
    ...
    while (token != NULL) {
        ...
        if (!strcmp(token, "EARGMID")) {
            ...
            for (i = 0; i < conf->num_eargms; i++)
                if (conf->eargms[i].id == aux)
                    idx = i;

            if (idx < 0) {
                local_idx    = conf->num_eargms;
                conf->eargms = realloc(conf->eargms, sizeof(eargm_def_t) * (conf->num_eargms + 1));
                memset(&conf->eargms[local_idx], 0,
                       sizeof(eargm_def_t)); // this could be replaced by a default init function
            ...
            while (token != NULL) {
                ...
                conf->eargms[local_idx].subs =
                    realloc(conf->eargms[local_idx].subs, sizeof(int) * (conf->eargms[local_idx].num_subs + 1));
...
void print_eargm_conf(eargm_conf_t *conf)
{
    ...
    verbosen(VCCONF, "\t eargm: defcon levels [%u,%u,%u] grace period %u\n", conf->defcon_limits[0],
             conf->defcon_limits[1], conf->defcon_limits[2], conf->grace_periods);
    ...
    verbosen(VCCONF, "\t power limit for action %lu and for lower %lu\n", conf->defcon_power_limit,
             conf->defcon_power_lower);
    verbosen(VCCONF, "\t powercap_limit_action %s and powercap_lower_action %s\n", conf->powercap_limit_action,
             conf->powercap_lower_action);
