...
{
    if (strcasestr(token, "ENERGYTAG") != NULL)
        return EAR_SUCCESS;
    return EAR_ERROR;
}
...
state_t ETAG_parse_token(unsigned int *num_etagsp, energy_tag_t **e_tagsl, char *line)
{
    ...
    while (token != NULL) {
        ...
        if (!strcmp(token, "ENERGYTAG")) {
            ...
            token = strtok_r(NULL, "=", &secondary_ptr);
            if (LIST_parse_token(token, &e_tags[num_etags - 1].num_users, &e_tags[num_etags - 1].users) != EAR_SUCCESS)
                return EAR_ERROR;
        } else if (!strcmp(token, "GROUPS")) {
            token = strtok_r(NULL, "=", &secondary_ptr);
            if (LIST_parse_token(token, &e_tags[num_etags - 1].num_groups, &e_tags[num_etags - 1].groups) !=
                EAR_SUCCESS)
                return EAR_ERROR;
        } else if (!strcmp(token, "ACCOUNTS")) {
            token = strtok_r(NULL, "=", &secondary_ptr);
            if (LIST_parse_token(token, &e_tags[num_etags - 1].num_accounts, &e_tags[num_etags - 1].accounts) !=
                EAR_SUCCESS)
                return EAR_ERROR;
        }
