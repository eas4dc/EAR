...
my_node_conf_t *get_my_node_conf(cluster_conf_t *my_conf, char *nodename)
{
            ...
            if (my_conf->islands[i].ranges[range_id].num_tags > 0) {
                for (j = 0; j < my_conf->islands[i].ranges[range_id].num_tags; j++) {
                    ...
                    if (tag_id >= 0) {
                        ...
                        debug("tag %s could not be found",
                              my_conf->islands[i].specific_tags[my_conf->islands[i].ranges[range_id].specific_tags[j]]);
                    }
                }
            }
    ...
    if (tag_id < 0)
        tag_id = def_tag_id; // if no tag found, we apply the default tag
    if (tag_id >= 0) {
        ...
        warning("No tag found for current node in ear.conf\n");
    ...
    if (tag_id >= 0 && tag_id != def_tag_id) // policies with the default tag have already been added
    {
        for (i = 0; i < my_conf->num_policies; i++) {
            ...
            debug("comparing current tag (%s) with policy tag (%s)\n", my_conf->tags[tag_id].id,
                  my_conf->power_policies[i].tag);
            if (!strcmp(my_conf->tags[tag_id].id,
                        my_conf->power_policies[i].tag)) // we only need the policies for our current tag
            {
                debug("match found!\n");
                for (j = 0; j < n->num_policies; j++) {
                    if (!strcmp(my_conf->power_policies[i].name,
                                n->policies[j].name)) // two policies are the same if they are have the same name, so we
                                                      // replace them
                    {
                        if (i == my_conf->default_policy) {
                            debug("Found default policy and changing it!");
                            my_conf->default_policy =
                                j; // if the default policy is the one we are replacing, change the pointer
                        }
    ...
    switch (n->powercap) {
        ...
        case POWER_CAP_UNLIMITED:
            ...
            if (my_conf->eargm.num_eargms > my_conf->islands[island_idx].ranges[range_id].eargm_id) {
                if (my_conf->eargm.eargms[my_conf->islands[island_idx].ranges[range_id].eargm_id].power ==
                    POWER_CAP_AUTO_CONFIG) {
                    verbose(
                        VCCONF + 2,
                        "warning: EARGM is set to autoconfigure and node powercap is unlimited, this is unsupported");
                }
            }
            break;
        case POWER_CAP_AUTO_CONFIG: {
            // When autoconfiguring, if the configuration is not valid we are currently setting the powercap to
            // UNLIMITED. Another option would be setting it to the DEFAULT value (but it could be -1 which would cause
            // issues) or set it to DISABLED.
            ...
            if (eargm_idx > 0) {
                ...
                if (my_conf->eargm.eargms[eargm_idx].power == 0) {
                    verbose(VCCONF, "\tEARGM powercap is DISABLED (0), this configuration is not supported. Setting "
                                    "powercap to UNLIMITED.");
                    n->powercap = POWER_CAP_UNLIMITED;
                    break;
                }
                if (my_conf->eargm.eargms[eargm_idx].power == 1) {
                    verbose(VCCONF, "\tEARGM powercap is UNLIMITED (1), this configuration is not supported. Setting "
                                    "powercap to UNLIMITED.");
                    n->powercap = POWER_CAP_UNLIMITED;
                    break;
                }
                if (my_conf->eargm.eargms[eargm_idx].power == -1) {
                    verbose(VCCONF, "\tEARGM powercap is AUTOCONFIGURE (-1), this configuration is not supported. "
                                    "Setting powercap to UNLIMITED.");
                    n->powercap = POWER_CAP_UNLIMITED;
                    break;
                }
...
int is_privileged(cluster_conf_t *my_conf, char *user, char *group, char *acc)
{
    ...
    if (user != NULL) {
        ...
        if (getgrouplist(pwd.pw_name, pwd.pw_gid, groups, &ngroups) == -1) {
            ...
            if (getgrouplist(pwd.pw_name, pwd.pw_gid, groups, &ngroups) ==
                -1) { // Get all the groups which the user belongs
...
energy_tag_t *can_use_energy_tag(char *user, char *group, char *acc, energy_tag_t *my_tag)
{
    ...
    if (user != NULL) {
        ...
        while ((i < my_tag->num_users) && (!found)) {
            ...
            if (strcmp(user, my_tag->users[i]) == 0)
                found = 1;
            else if (strcmp(my_tag->users[i], "all") == 0)
                found = 1;
            else
                i++;
        }
    ...
    if (group != NULL) {
        ...
        while ((i < my_tag->num_groups) && (!found)) {
            debug("Can_use_energy_tag group %s (%lu) vs %s (%lu)\n", group, strlen(group), my_tag->groups[i],
                  strlen(my_tag->groups[i]));

            if (strcmp(group, my_tag->groups[i]) == 0)
                found = 1;
            else
                i++;
        }
    ...
    if (acc != NULL) {
        ...
        while ((i < my_tag->num_accounts) && (!found)) {
            ...
            if (strcmp(acc, my_tag->accounts[i]) == 0)
                found = 1;
            else
                i++;
        }
...
/* returns  the energy tag entry if the username, group and/or accounts is in the list of the users/groups/acc
 * authorized to use the given energy-tag, NULL otherwise */
...
energy_tag_t *energy_tag_exists(cluster_conf_t *my_conf, char *etag)
{
    ...
    i = 0;
    while ((i < my_conf->num_etags) && (!found)) {
        if (strcmp(etag, my_conf->e_tags[i].tag) == 0)
            found = 1;
        else
            i++;
    }
    if (found)
        return &my_conf->e_tags[i];
    return NULL;
}
...
int get_node_server_mirror(cluster_conf_t *conf, const char *hostname, char *mirror_of)
{
    ...
    for (i = 0; i < conf->num_islands && !found_both; ++i) {
        ...
        for (k = 0; k < is->num_ranges && !found_both; k++) {
            ...
            if (!found_server && p != NULL && ((strncmp(p, n, strlen(n)) == 0) || (strncmp(p, a, strlen(a)) == 0))) {
                found_server = 1;
            }
            ...
            if (!found_mirror && is->ranges[k].sec_ip >= 0) {
                ...
                if (p != NULL && ((strncmp(p, n, strlen(n)) == 0) || (strncmp(p, a, strlen(a)) == 0))) {
...
int policy_name_to_id(char *my_policy, cluster_conf_t *conf)
{
    ...
    for (i = 0; i < conf->num_policies; i++) {
        if (strcmp(my_policy, conf->power_policies[i].name) == 0)
            return i;
    }
    return EAR_ERROR;
}
...
int is_valid_policy(unsigned int p_id, cluster_conf_t *conf)
{
    int i;
    for (i = 0; i < conf->num_policies; i++)
        if (conf->power_policies[i].policy == p_id)
            return 1;
...
int validate_configuration(cluster_conf_t *conf)
{
    ...
    for (i = 0; i < conf->num_islands; i++)
        if (conf->islands[i].num_ips < 1 || conf->islands[i].num_backups < 1)
            return EAR_ERROR;
    for (i = 0; i < conf->num_etags; i++)
        if (conf->e_tags[i].num_users < 1 && conf->e_tags[i].num_groups < 1 && conf->e_tags[i].num_accounts < 1)
            return EAR_ERROR;
    ...
    for (i = 0; i < conf->num_islands; i++) {
        for (j = 0; j < conf->islands[i].num_ranges; j++) {
            ...
            for (k = 0; k < conf->islands[i].ranges[j].num_tags; k++) {
                ...
                // each range of an island stores the ids in the array of specific_tags that they have assigned
                tag_id =
                    tag_id_exists(conf, conf->islands[i].specific_tags[conf->islands[i].ranges[j].specific_tags[k]]);
                if (tag_id >= 0) {
                    debug("found tag: %s\n",
            ...
            if (conf->tags[tag_id].powercap == ret)
                continue;
            else
                ret += conf->tags[tag_id].powercap;
        }
    }
    return ret;
}
...
uint tags_are_unlimited(cluster_conf_t *conf)
{
    ...
    for (i = 0; i < conf->num_islands; i++) {
        for (j = 0; j < conf->islands[i].num_ranges; j++) {
            ...
            if (conf->tags[tag_id].powercap != 1)
                return 0; // is this the correct way to get it? what if some are set to 0 or -1 (automatic?)
        }
    }
    return 1;
}
...
static void _free_island_contents(node_island_t *is)
{
    ...
    if (is->num_specific_tags > 0)
        free(is->specific_tags);
    ...
    for (i = 0; i < conf->num_islands; i++) {
        for (j = 0; j < conf->islands[i].num_ranges; j++) {
            ...
            if (!tag_found) {
                ...
                conf->islands[i].ranges =
                    realloc(conf->islands[i].ranges, sizeof(node_range_t) * conf->islands[i].num_ranges);
                j--;
            }
        }
        ...
        if (conf->islands[i].num_ranges == 0) {
            ...
            if (i + 1 < conf->num_islands)
                memmove(&conf->islands[i], &conf->islands[i + 1],
                        sizeof(node_island_t) * (conf->num_islands - (i + 1)));
            conf->num_islands--;
...
void remove_islands_by_eargm(cluster_conf_t conf[static 1], eargm_def_t *e_def)
{
    ...
    for (i = 0; i < conf->num_islands; i++) {
        for (j = 0; j < conf->islands[i].num_ranges; j++) {
            ...
            {
                debug("Swapping range index %d with %d. target id %d current id %d ", j,
                      conf->islands[i].num_ranges - 1, conf->islands[i].ranges[j].eargm_id, e_def->id);
                conf->islands[i].num_ranges--;
                if (conf->islands[i].ranges[j].num_tags > 0)
                    free(conf->islands[i].ranges[j].specific_tags);
                memcpy((void *) &conf->islands[i].ranges[j],
                       (void *) &conf->islands[i].ranges[conf->islands[i].num_ranges], sizeof(node_range_t));
                // pointer_swap((void *)&conf->islands[i].ranges[j], (void
                // *)&conf->islands[i].ranges[conf->islands[i].num_ranges]);
                conf->islands[i].ranges =
                    realloc(conf->islands[i].ranges, sizeof(node_range_t) * conf->islands[i].num_ranges);
...
state_t serialize_cluster_conf(cluster_conf_t *conf, char **ear_conf_buf, size_t *conf_size)
{
    ...
    if (conf->eargm.num_eargms)
        serial_add_elem(&ear_conf_serial, (char *) conf->eargm.eargms, sizeof(eargm_def_t) * conf->eargm.num_eargms);
    debug("serial + eargm");
    ...
    if (conf->power_policies)
        serial_add_elem(&ear_conf_serial, (char *) &conf->default_policy, sizeof(conf->default_policy));
    ...
    if (conf->num_nodes)
        serial_add_elem(&ear_conf_serial, (char *) conf->nodes, sizeof(node_conf_t) * conf->num_nodes);
    ...
    if (conf->num_etags)
        serial_add_elem(&ear_conf_serial, (char *) conf->e_tags, sizeof(energy_tag_t) * conf->num_etags);
    ...
    if (conf->num_islands)
        serial_add_elem(&ear_conf_serial, (char *) conf->islands, sizeof(node_island_t) * conf->num_islands);
