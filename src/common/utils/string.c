...
char **strtoa(const char *string, char separator, char ***rows, uint *rows_count)
{
    ...
    for (prows_count = i = 0; i < strlen(pstring) + 1; ++i) {
        debug("'%c'", pstring[i]);
        if (pstring[i] == separator || pstring[i] == '\n' || pstring[i] == '\0') {
...
void *strtoat(const char *string, char separator, void **list, uint *list_count, int id_type)
{
    ...
    // We are adding
    if (id_type == ID_INT) {
        plist = calloc(plist_count, sizeof(int));
    }
    if (id_type == ID_UINT) {
        plist = calloc(plist_count, sizeof(uint));
    }
    if (id_type == ID_LONG) {
        plist = calloc(plist_count, sizeof(long));
    }
    if (id_type == ID_ULONG) {
        plist = calloc(plist_count, sizeof(ulong));
    }
    if (id_type == ID_LLONG) {
        plist = calloc(plist_count, sizeof(llong));
    }
    if (id_type == ID_ULLONG) {
        plist = calloc(plist_count, sizeof(ullong));
    }
    if (id_type == ID_FLOAT) {
        plist = calloc(plist_count, sizeof(float));
    }
    if (id_type == ID_DOUBLE) {
        plist = calloc(plist_count, sizeof(double));
    }
    // Its a NULL terminated list
    for (i = 0; alist[i] != NULL; ++i) {
        if (id_type == ID_INT) {
            ((int *) plist)[i] = (int) atoi(alist[i]);
        }
        if (id_type == ID_UINT) {
            ((uint *) plist)[i] = (uint) atoi(alist[i]);
        }
        if (id_type == ID_LONG) {
            ((long *) plist)[i] = (long) strtol(alist[i], NULL, 10);
        }
        if (id_type == ID_ULONG) {
            ((ulong *) plist)[i] = (ulong) strtoul(alist[i], NULL, 10);
        }
        if (id_type == ID_LLONG) {
            ((llong *) plist)[i] = (llong) strtoll(alist[i], NULL, 10);
        }
        if (id_type == ID_ULLONG) {
            ((ullong *) plist)[i] = (ullong) strtoull(alist[i], NULL, 10);
        }
        if (id_type == ID_FLOAT) {
            ((float *) plist)[i] = (float) atof(alist[i]);
        }
        if (id_type == ID_DOUBLE) {
            ((double *) plist)[i] = (double) atof(alist[i]);
        }
    }
    // Freeing
    strtoa_free(alist);
