...
static uint32_t power_list_max[FAKE_N];
static uint32_t power_list_min[FAKE_N];
#else
...
state_t mgt_gpu_dummy_get_devices(ctx_t *c, gpu_devs_t **devs_in, uint *devs_count_in)
{
    uint devs_count = 1;
    uint i;
    #if FAKE_GPUS
    ...
    *devs_in = calloc(devs_count, sizeof(gpu_devs_t));
    for (i = 0; i < *devs_count_in; ++i) {
        (*devs_in)[i].index        =  i;
        (*devs_in)[i].index_device = -1;
        (*devs_in)[i].serial       =  i;
        (*devs_in)[i].is_readable  =  1;
        #if FAKE_GPUS
        sprintf((*devs_in)[i].name, "Fake Device %d", i);
        sprintf((*devs_in)[i].uuid, "fake-%d", i);
        #else
        sprintf((*devs_in)[i].name, "Dummy Device %d", i);
        sprintf((*devs_in)[i].uuid, "dummy-%d", i);
        #endif
    }
    return EAR_SUCCESS;
}
