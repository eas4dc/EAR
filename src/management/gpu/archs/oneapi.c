    ...
    for (dv = 0; dv < devs_count; ++dv) {
        ...
        for (dm = 0; dm < devs[dv].spowers_count; ++dm) {
            ...
            for (de = 0; de < descs_count; ++de) {
                ...
                debug("POWER DEV%d DOM%d DESC%d: limit %03d %s at %04d mS interval (%d%d, %s, %s) (sub %d)", dv, dm, de,
                      descs[de].limit / 1000, units[descs[de].limitUnit], descs[de].interval,
                      descs[de].limitValueLocked, descs[de].intervalValueLocked, levels[descs[de].level],
...
static state_t get_available_frequencies()
{
    ...
    for (dv = 0; dv < devs_count; ++dv) {
        ...
        for (d = 0; d < devs[dv].sfreqs_count; ++d) {
            ...
            debug("DEV%d FREQ DOM%d: %lu to %lu KHz (%u P_STATEs)", dv, d, freqs_avail_list[dv][0],
                  freqs_avail_list[dv][freqs_avail_count[dv] - 1], freqs_avail_count[dv] - 1);
...
void mgt_gpu_oneapi_load(mgt_gpu_ops_t *ops)
{
    ...
    // Checking if OneAPI is capable to run commands
...
state_t mgt_gpu_oneapi_freq_limit_set(ctx_t *c, ulong *khz)
{
    ...
    for (dv = 0; dv < devs_count; ++dv) {
        for (d = 0; d < devs[dv].sfreqs_count; ++d) {
            ...
            // In documentation of zesFrequencySetRange -> zes_freq_range_t:
            // - The min frequency in MHz. On input, setting to 0 will permit
            //   the frequency to go down to the hardware minimum.
            range.min = 0;
