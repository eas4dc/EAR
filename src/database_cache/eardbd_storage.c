...
#define sync_option(option, type)           ((option & type) > 0)

#define sync_option_m(option, type1, type2) ((sync_option(option, type1)) || (sync_option(option, type2)))
...
void metrics_print()
{
    ...
    if (n > 0) {
        ...
        for (i = 0; i < EDB_NTYPES; ++i) {
            ...
            tprintf("%s||%lu/%lu||%2.2f||%0.2fs||%0.2fs", type_name[i], samples_index[i], type_alloc_len[i], percent,
                    time_insert, time_recv);
        }
...
{
    debug("trying to insert type %d (simple: '%d')", i, !conf_clus.database.report_sig_detail);
    // Insert time update
    time(&time_insert1[i]);
}
...
void storage_sample_receive(int fd, packet_header_t *header, char *content)
{
    ...
    if (type == EDB_TYPE_APP_MPI) {
        ...
        storage_sample_add(type_chunk[index], type_alloc_len[index], &samples_index[index], content, type_sizeof[index],
                           EDB_SYNC_TYPE_APPS_MPI);
    } else if (type == EDB_TYPE_APP_SEQ) {
        // print_application_fd_binary(0, (application_t *) content);
        storage_sample_add(type_chunk[index], type_alloc_len[index], &samples_index[index], content, type_sizeof[index],
                           EDB_SYNC_TYPE_APPS_SEQ);
    } else if (type == EDB_TYPE_APP_LEARN) {
        // print_application_fd_binary(0, (application_t *) content);
        storage_sample_add(type_chunk[index], type_alloc_len[index], &samples_index[index], content, type_sizeof[index],
                           EDB_SYNC_TYPE_APPS_LEARN);
    } else if (type == EDB_TYPE_EVENT) {
        storage_sample_add(type_chunk[index], type_alloc_len[index], &samples_index[index], content, type_sizeof[index],
                           EDB_SYNC_TYPE_EVENTS);
