...
// By now disabled
    ...
    if (state_fail(process_pid_file_save(&proc_server))) {
        edb_error("while testing the temporal directory '%s' (%s)", conf_clus->install.dir_temp, state_msg);
    }
    ...
    if (server_exists)
        server_pid = other_server_pid;
    if (mirror_exists)
        mirror_pid = other_mirror_pid;
...
static void init_types_single(int i, char *name, size_t size, uint percent)
{
    ...
    // Verbosity
    tprintf("%s||%0.2f MBs||%lu Bs||%lu||%0.2f", type_name[i], type_alloc_mbs[i], type_sizeof[i], type_alloc_len[i],
            batch_100);
}
...
static void init_process_configuration(int argc, char **argv, cluster_conf_t *conf_clus)
{
    ...
    // Setting master name
    if (server_iam) {
        xsprintf(master_name, "%s-s", master_nick);
    } else {
        xsprintf(master_name, "%s-m", master_nick);
    }
    ...
    float mb_total = type_alloc_mbs[index_appsm] + type_alloc_mbs[index_appsn] + type_alloc_mbs[index_appsl] +
                     type_alloc_mbs[index_loops] + type_alloc_mbs[index_evens] + type_alloc_mbs[index_enrgy] +
