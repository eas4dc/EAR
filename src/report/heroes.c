...
static char *extra_header     = "HEROES_INSTANCE_WORKFLOW_ID;HEROES_TEMPLATE_WORKFLOW_ID;HEROES_USER_ID;HEROES_"
                                "ORGANIZATION_NAME;HEROES_ORGANIZATION_ID;";
static char *extra_header_app = "HEROES_INSTANCE_WORKFLOW_ID;HEROES_TEMPLATE_WORKFLOW_ID;HEROES_USER_ID;HEROES_"
                                "ORGANIZATION_NAME;HEROES_ORGANIZATION_ID;MAX_POWER_W;";
...
state_t report_init(report_id_t *id, cluster_conf_t *cconf)
{
    ...
    if (heroes_wrf_id == NULL)
        heroes_wrf_id = NULL_WRF_ID;
    if (heroes_her_id == NULL)
        heroes_her_id = NULL_HER_ID;
    if (heroes_usr_id == NULL)
        heroes_usr_id = NULL_USR_ID;
    if (heroes_prj_id == NULL)
        heroes_prj_id = NULL_PRJ_ID;
    if (heroes_org_id == NULL)
        heroes_org_id = NULL_ORG_ID;

    snprintf(heroes_ids, sizeof(heroes_ids), "%s;%s;%s;%s;%s;", heroes_wrf_id, heroes_her_id, heroes_usr_id,
             heroes_prj_id, heroes_org_id);
...
state_t report_loops(report_id_t *id, loop_t *loops, uint count)
{
    ...
    debug("header created %u , %s is regular %d", file_loop_created, csv_loop_log_file,
          ear_file_is_regular(csv_loop_log_file));
