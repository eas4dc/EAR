...
state_t report_init(report_id_t *id, cluster_conf_t *cconf)
{
    ...
    sprintf(log_file_name_pm, "%s/%s.%s.periodic_metrics.txt", cconf->install.dir_temp, program_invocation_short_name,
            nodename);
    sprintf(log_file_name_loops, "%s/%s.%s.loops.txt", cconf->install.dir_temp, program_invocation_short_name,
            nodename);
    sprintf(log_file_name_apps, "%s/%s.%s.apps.txt", cconf->install.dir_temp, program_invocation_short_name, nodename);
    sprintf(log_file_name_pa, "%s/%s.%s.periodic_agg_metrics.txt", cconf->install.dir_temp,
            program_invocation_short_name, nodename);
    sprintf(log_file_name_events, "%s/%s.%s.events.txt", cconf->install.dir_temp, program_invocation_short_name,
            nodename);
...
{
    dprintf(fd, "power (node %.1lf/cpu %.1lf/dram %.1lf) EDP %.lf time %.1lf avg. CPU_freq %lu ", ps->DC_power,
            ps->PCK_power, ps->DRAM_power, ps->EDP, ps->time, ps->avg_f);
}
...
state_t report_applications(report_id_t *id, application_t *apps, uint count)
{
    ...
    for (uint i = 0; i < count; i++) {
        ...
        dprintf(fd_apps,
                ...
                i, apps[i].job.id, apps[i].job.step_id, lid, apps[i].job.user_id, (uint) apps[i].job.start_time,
                (uint) apps[i].job.end_time, apps[i].job.policy, apps[i].job.th, apps[i].job.procs, apps[i].job.def_f,
                apps[i].is_mpi, apps[i].node_id);
...
state_t report_events(report_id_t *id, ear_event_t *eves, uint count)
{
    ...
    for (uint i = 0; i < count; i++) {
        ...
        dprintf(fd_events, "event[%d]: jid %lu stepid %lu node %s event %s value %ld time %u\n", i, eves[i].jid,
                eves[i].step_id, eves[i].node_id, event_desc, eves[i].value, (uint) eves[i].timestamp);
    }
    return EAR_SUCCESS;
}
