...
state_t report_periodic_metrics(report_id_t *id, periodic_metric_t *mets, uint count)
{
    ...
    for (i = 0; i < count; i++) {
        // format the query
#if USE_GPUS
        sprintf(tmp_query, base_query, mets[i].start_time, mets[i].end_time, mets[i].DC_energy, mets[i].node_id,
                mets[i].job_id, mets[i].step_id, mets[i].avg_f, mets[i].temp, mets[i].DRAM_energy, mets[i].PCK_energy,
                mets[i].GPU_energy);
#else
        sprintf(tmp_query, base_query, mets[i].start_time, mets[i].end_time, mets[i].DC_energy, mets[i].node_id,
                mets[i].job_id, mets[i].step_id, mets[i].avg_f, mets[i].temp, mets[i].DRAM_energy, mets[i].PCK_energy);
