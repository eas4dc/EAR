...
state_t report_init(report_id_t *id, cluster_conf_t *cconf)
{
    if (id->master_rank >= 0) {
        ...
        dcgmi_verbose(1, "DCGMI report: %s", (must_report) ? "ON" : "OFF");
    ...
    if (must_report) {
        ...
        if (!csv_log_file_env) {
            ...
            snprintf(csv_log_file, sizeof(csv_log_file) - 1, "%s/dcgmi_app_%s.csv", csv_log_file_env, nodename);

            snprintf(csv_loop_log_file, sizeof(csv_loop_log_file) - 1, "%s/dcgmi_loops_%s.csv", csv_log_file_env,
                     nodename);
        } else {
            snprintf(csv_log_file, sizeof(csv_log_file) - 1, "%s_dcgmi_app_%s.csv", csv_log_file_env, nodename);

            snprintf(csv_loop_log_file, sizeof(csv_loop_log_file) - 1, "%s_dcgmi_loops_%s.csv", csv_log_file_env,
                     nodename);
        }
