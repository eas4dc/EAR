...
state_t report_init(report_id_t *id, cluster_conf_t *cconf)
{
    ...
    sprintf(log_file_name_sensors, "%s/%s.%s.edcmon_sensors.txt", cconf->install.dir_temp,
    ...
    for (uint i = 0; i < count; i++) {
        switch (type) {
            case NODESENSORS_TYPE:
                dprintf(fd_sensors, "time[%s] type %s node %s power %.1lf W\n", date_str(data[i].timestamp),
                        pdu_type_to_str(data[i].type), data[i].nodename, data[i].power);
                break;

            default:
                dprintf(fd_sensors, "time[%s] node %s data not recognized", date_str(data[i].timestamp),
                        data[i].nodename);
        }
    }
    return EAR_SUCCESS;
