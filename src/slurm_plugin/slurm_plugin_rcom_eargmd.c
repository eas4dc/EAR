...
int plug_rcom_eargmd_job_start(spank_t sp, plug_serialization_t *sd)
{
    ...
    if (sd->job.nodes_count < sd->pack.eargmd.min) {
        plug_verbose(sp, 2, "EARGMD is not connected because not enough nodes (%d < %d)", sd->job.nodes_count,
                     sd->pack.eargmd.min);
        return ESPANK_SUCCESS;
    }
    // Pack deserialization
    if (getenv_agnostic(sp, Var.con_eargmd.mod, buffer, SZ_PATH)) {
        sd->pack.eargmd.secured = atoi(buffer);
    }
    //
    if (!sd->pack.eargmd.enabled || sd->pack.eargmd.secured) {
        plug_error(sp, "connection with EARGMD not enabled or secured (%d,%d)", sd->pack.eargmd.enabled,
                   sd->pack.eargmd.secured);
        return ESPANK_ERROR;
    }
    //
    ...
    // Verbosity
    plug_verbose(sp, 2, "connecting EARGMD with host '%s', port '%u', and nnodes '%u'", sd->pack.eargmd.host,
                 sd->pack.eargmd.port, sd->job.nodes_count);
    // Connection
    ...
    // Enabling protection
    setenv_agnostic(sp, Var.con_eargmd.mod, "1", 1);
...
int plug_rcom_eargmd_job_finish(spank_t sp, plug_serialization_t *sd)
{
    ...
    plug_verbose(sp, 2, "trying to disconnect EARGMD with host '%s', port '%u', and nnodes '%u'", sd->pack.eargmd.host,
                 sd->pack.eargmd.port, sd->job.nodes_count);
    ...
    setenv_agnostic(sp, Var.con_eargmd.mod, "0", 1);
