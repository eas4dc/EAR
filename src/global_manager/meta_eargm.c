...
void print_eargm_status(eargm_status_t *status, uint num_status)
{
    ...
    for (i = 0; i < num_status; i++) {
        verbose(VGM, "%sid %d powercap %lu power %lu requested %lu freeable %lu extra %lu status %u%s", COL_CLR,
...
void meta_aggregate_data(eargm_status_t *status, uint num_status)
{
    ...
    // reset values
    total_power = total_req = total_freeable = total_extra = total_req_no_extra = total_powercap = total_req_ask_def =
        0;
    ...
    if (default_powercap < total_powercap) {
        ...
        verbose(VGM, "powercap left to allocate %lu (default %lu current %lu)", current_free_power, default_powercap,
                total_powercap);
    }
}
...
void reduce_freeable_power(eargm_status_t *status, uint num_status, eargm_table_t *egm_table, ulong *pending,
                           ulong *freeable)
{
    int i         = 0, egm_id;
    ulong tmp_var = 0;
    while (*pending && i < num_status) {
        if (status[i].freeable_power) {
            egm_id = get_eargm_position(egm_table, status[i].id);
            if (egm_id >= 0) {
                tmp_var = ear_min(*pending, status[i].freeable_power); // store it in an auxiliary value since we will
                                                                       // need to remove it from both variables
...
void reduce_extra_power(eargm_status_t *status, uint num_status, eargm_table_t *egm_table, ulong *pending)
{
    ...
    while (*pending && i < num_status) {
        if (status[i].extra_power) {
            egm_id = get_eargm_position(egm_table, status[i].id);
            if (egm_id >= 0) {
                tmp_var = ear_min(*pending, status[i].extra_power); // store it in an auxiliary value since we will need
                                                                    // to remove it from both variables
...
void assign_extra_power(eargm_status_t *status, uint num_status, eargm_table_t *egm_table, ulong *free_power,
                        ulong *pending, uint has_extra)
{
    int i         = 0, egm_id;
    ulong tmp_var = 0;
    ...
    for (i = 0; (i < num_status) && *free_power; i++) {
        if (status[i].status == PC_STATUS_GREEDY) {
            ...
            if ((status[i].extra_power > 0) && (has_extra == 0))
                continue;
            if ((status[i].extra_power == 0) && (has_extra == 1))
                continue;

            egm_id = get_eargm_position(egm_table, status[i].id);
            if (egm_id >= 0) {
                tmp_var = ear_min(*free_power, status[i].requested); // store it in an auxiliary value since we will
                                                                     // need to remove it from both variables
                ...
                *pending -= tmp_var;    // remove it from the pool of pending to assign power
                status[i].requested -=
                    tmp_var; // remove it from the individual node requested power (since it has already been granted)
            }
        }
    }
}
...
void meta_reallocate_power(eargm_status_t *status, uint num_status, eargm_table_t *egm_table)
{
    ...
    if (num_ask_def > 0) {
        ...
        if (current_free_power >=
            total_req_ask_def) { // if there is power not assigned to any EARGM and is enough to cover PC_STATUS_ASK_DEF
            ...
            if (pending) { // if still needed, remove extra power given to other EARGMS
                ...
                if (pending > 0) { // this should never happen
                    verbose(VGM, "ERROR: NOT ENOUGH POWER TO ASSIGN PC_STATUS_ASK_DEF TO EARGMS (pending %lu)",
                            pending);
                }
                return;
            }
    ...
    if (num_greedy_no_extra) {
        ...
        if (total_req_no_extra > 0 && total_freeable) {
            pending = pending_2 = ear_min(
                total_freeable, total_req_no_extra); // the function modifies pending, that's why we have pending_2
            reduce_freeable_power(status, num_status, egm_table, &pending, &total_freeable);
    ...
    if (num_greedy) {
        ...
        if (total_req > 0 && total_freeable) {
            pending = pending_2 =
                ear_min(total_freeable, total_req); // the function modifies pending, that's why we have pending_2
            reduce_freeable_power(status, num_status, egm_table, &pending, &total_freeable);
...
void *meta_eargm(void *config)
{
    ...
    if (pthread_setname_np(pthread_self(), "meta_eargm"))
        verbose(0, "Setting name for meta_eargm thread %s", strerror(errno));
