...
#define INDIRECT_PERIODIC_QUERY                                                                                        \
    "SELECT * FROM Periodic_metrics WHERE end_time >= %s AND start_time <= %s AND node_id IN (%s)"

void usage(char *argv)
{
    printf("Usage: %s [OPTIONS]\n", argv);
    printf("\nOptions:\n");
    printf("\t-j\t--job-id  <id.step>\tSpecifies which job-id the periodic metrics will be retrieved from. It can be a "
           "list of jobs.\n");
    printf("\t-n\t--node-id <\"id\">\tSpecifies which nodes the periodic metrics will be retrieved from. It can be a "
           "list of nodes\n");
...
int main(int argc, char *argv[])
{
    ...
    static struct option long_options[] = {
        ...
        {"no-filter", no_argument, 0, 'x'},     {"job-id", required_argument, 0, 'n'},
        {"node-id", required_argument, 0, 'j'}, {"file", required_argument, 0, 'f'},
    };
