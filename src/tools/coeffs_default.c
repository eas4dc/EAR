...
int find(coefficient_t *coeff)
{
    ...
    for (i = 0; i < 200; ++i) {
        ...
        found = (coeffs_accum[i].pstate_ref == coeff->pstate_ref) && (coeffs_accum[i].pstate == coeff->pstate);
...
static void print()
{
    ...
    //
    for (i = 0; i < coeffs_max; ++i) {
        tprintf("%lu||%lu||%d||%d||| %0.3lf||%0.3lf||%0.3lf||%0.3lf||%0.3lf||%0.3lf", coeffs_accum[i].pstate_ref,
                coeffs_accum[i].pstate, coeffs_num[i], coeffs_accum[i].available, coeffs_accum[i].A, coeffs_accum[i].B,
                coeffs_accum[i].C, coeffs_accum[i].D, coeffs_accum[i].E, coeffs_accum[i].F);
