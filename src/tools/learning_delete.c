...
#define POW_SIG_QUERY                                                                                                  \
    "DELETE FROM Power_signatures WHERE id IN (SELECT power_signature_id from Learning_applications "                  \
    "INNER JOIN Learning_jobs ON Learning_jobs.job_id=Learning_applications.job_id AND "                               \
    "Learning_jobs.step_id=Learning_applications.step_id "                                                             \
    "WHERE "
#define NODE_QUERY                                                                                                     \
    "DELETE Learning_signatures, Learning_applications, Learning_jobs FROM Learning_jobs "                             \
    "INNER JOIN Learning_applications INNER JOIN Learning_signatures ON "                                              \
    "Learning_jobs.job_id=Learning_applications.job_id "                                                               \
    "AND Learning_jobs.step_id=Learning_applications.step_id AND Learning_signatures.id=signature_id "                 \
    "WHERE "
...
void usage(char *name)
{
    ...
    printf("Available options:\n"
           "\t-n [node_name1, nn2]\t Specifies from which nodes the learning phase will be deleted from, or all for "
           "every entry. [default: all]\n"
           "\t-j [job_id1, job_id2] \t Specifies which job_ids will be deleted from the learning phase. Only available "
           "for specific nodes.\n"
           "\t-f [freq1, freq2]\t Specifies what frequencies will be deleted from the learning phase. Only available "
           "for specific nodes.\n"
...
int main(int argc, char *argv[])
{
    ...
    if (!to_run) {
        ...
        if (!mysql_real_connect(connection, my_conf.database.ip, "root", passw, my_conf.database.database,
                                my_conf.database.port, NULL, 0)) {
            verbose(0, "Error connecting to the database(%d):%s", mysql_errno(connection),
