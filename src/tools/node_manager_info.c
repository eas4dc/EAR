...
int main(int argc, char *argv[])
{
    ...
    for (uint i = 0; i < max_jobs; i++) {
        if (ejoblist_shmem[i] != 0) {
            ...
            eard_jobs_found++;
            printf("PMAPP: ID.STEPID %lu.%lu is_job %u signature_reported %d CPU freq %lu\n", pmapp_shmem->app.job.id,
                   pmapp_shmem->app.job.step_id, pmapp_shmem->is_job, pmapp_shmem->sig_reported,
                   pmapp_shmem->current_freq);
