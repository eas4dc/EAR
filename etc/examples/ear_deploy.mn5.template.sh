...
extra_flags="--with-mysql=/apps/GPP/MYSQL/8.0.36/GCC"
...
core_modules="bsc/1.0 mkl"
intel_modules="intel/2023.2.0 impi/2021.10.0 oneapi/2023.2.0 mkl/2023.2.0"
nvidia_modules="cuda/12.2"
openmpi_modules="openmpi/4.1.5-gcc"
...
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MAKE_NAME=intelmpi.$tag   USER=$US GROUP=$GR --docdir=$DOCPATH --with-cuda=$CUDA_HOME $extra_disable $extra_flags
...
echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MAKE_NAME=nompi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --disable-mpi --with-cuda=$CUDA_HOME $extra_disable $extra_flags
...
my_cflags='-march=native -Wall'
echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MPI_VERSION=ompi MAKE_NAME=openmpi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --with-cuda=$CUDA_HOME $extra_disable $extra_flags
