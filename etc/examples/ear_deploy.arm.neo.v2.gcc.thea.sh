...
export INSTALL_ROOT=/global/scratch/users/$USER/EAR
...
core_modules="openmpi/4.1.6-gcc-12.3.0-wftkmyd"
nvidia_modules="cuda/12.4.0-gcc-12.3.0-vggnjvw"
...
# System types options: Lenovo, Grace_Hooper, Cray_HPE, EAR_Node_Mgr
...
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MAKE_NAME=nompi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --disable-mpi --with-cuda=$CUDA_ROOT $extra_disable --with-slurm=$SLURM_PATH 
...
eval $EAR_FEATURES make -f Makefile.nompi.$tag etc.install
...
my_cflags='-march=armv8-a+sve -Wall'
my_cflags='-mcpu=native -Wall -g'
my_mpi_cflags='-mcpu=native -Wall -g'

echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_mpi_cflags" MPICC=mpicc MPI_VERSION=ompi MAKE_NAME=openmpi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --with-cuda=$CUDA_ROOT $extra_disable --with-slurm=$SLURM_PATH
