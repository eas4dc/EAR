...
export system_type=Grace_Hooper
...
export extra_disable="--enable-arm64 --disable-avx512"
# End disable section
...
export tag="JAMMY-Neo2"
# Installation prefix path
export INSTALL_ROOT=/hpc/base/EAR
...
core_modules="GCC/13.2.0 GCCcore/13.2.0"
nvidia_modules=""
openmpi_modules="gompi/2023b"
...
# With gcc
my_cflags='-march=armv8-a+sve -Wall'
my_cflags='-mcpu=native -Wall -g'

echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MAKE_NAME=nompi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --disable-mpi  $extra_disable --with-slurm=$SLURM_PATH
...
DCGMI=1 FEAT_EARL_LITE=1 COMPONENT_DISABLE="database_cache commands tools" make -f Makefile.nompi.$tag $2
DCGMI=1 FEAT_EARL_LITE=1 COMPONENT_DISABLE="database_cache commands tools" make -f Makefile.nompi.$tag install
DCGMI=1 FEAT_EARL_LITE=1 COMPONENT_DISABLE="database_cache commands tools" make -f Makefile.nompi.$tag etc.install
...
echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MPI_VERSION=ompi MAKE_NAME=openmpi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH $extra_disable --with-slurm=$SLURM_PATH
...
DCGMI=1 FEAT_EARL_LITE=1 COMPONENT_DISABLE="database_cache commands tools" make -f Makefile.openmpi.$tag $2
DCGMI=1 FEAT_EARL_LITE=1 COMPONENT_DISABLE="database_cache commands tools" make -f Makefile.openmpi.$tag earl.install
