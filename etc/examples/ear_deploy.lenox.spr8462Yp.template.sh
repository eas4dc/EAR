...
#### Extra configuration paths section. Not included by default
  ...
  #--with-slurm=PATH       Specify path to SLURM installation. Use no to disable.
...
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=icx CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpiicx MAKE_NAME=intelmpi.$tag   USER=$US GROUP=$GR --docdir=$DOCPATH --with-oneapi=/usr $extra_disable --with-slurm=$SLURMPATH
...
echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MAKE_NAME=nompi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --disable-mpi --with-oneapi=/usr $extra_disable --with-slurm=$SLURMPATH
...
my_cflags='-march=native -Wall'
echo "Using CFLAGS="$my_cflags
./configure --prefix=$INS_PATH EAR_TMP=$ETMP EAR_ETC=$ETC CC=gcc CC_FLAGS="$my_cflags" MPICC_FLAGS="$my_cflags" MPICC=mpicc MPI_VERSION=ompi MAKE_NAME=openmpi.$tag  USER=$US GROUP=$GR --docdir=$DOCPATH --with-oneapi=/usr $extra_disable --with-slurm=$SLURMPATH
